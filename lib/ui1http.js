...
var basic_style_respond = function( question ){
  ...
  if( PendingResponse.fast_redirect ){
    if( PendingResponse.request.kudo_session.can_history ){
...
function handle_basic_style_request_input( req, res ){
  ...
  if( session ){
    ...
    if( l8.client
    || session.can_script !== true // Tracking bot initiated bugs
    ){
      var referer = req.headers[ "referer" ];
      trace( "URL: " + req.url, "referer: " + referer );
    }
    ...
    if( session.get_cookie( "can_script" ) ){
      if( session.can_script === "init" ){
        ...
        console.log( "BUG? noscript client set 'can_script' cookie" );
        session.can_script = true;
      }
    }
