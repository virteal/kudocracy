...
var basic_style_respond = function( question ){
  ...
  var html_tag = "<html>";
  // Insert link to offline manifest file in index page (it uses style.css)
  if( head ){
    ...
    var head_head = head.substring( 0, 100 );
    var idx_style = head_head.indexOf( "style.css" );
    if( idx_style !== -1 ){
      var idx_kudo_style = head_head.indexOf( "kudocracy.css" );
      if( idx_kudo_style !== -1 ){
        trace( "BUG? both style.css & kudocracy.css", idx_style, idx_kudo_style );
        if( idx_style < idx_kudo_style ){
          html_tag = '<html x-kudo="offline" manifest="manifest.appcache">';
        }
      }else{
        html_tag = '<html x-kudo="offline" manifest="manifest.appcache">';
      }
