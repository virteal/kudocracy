...
function start_local_server( kudo_scope ){
  ...
  server.get_stored_changes = function(){
    ...
    var domain  = kudo_ctx.domain;
    if( !storage )return 0;
    var version = storage.getItem( "change_version" );
    var stored_count = storage.getItem( "change_count_" + domain );
    var count = stored_count ? parseInt( stored_count, 10 ) : 0;
    if( version !== window.Ephemeral.version ){
      count = 0;
      storage.setItem( "change_count_" + domain, 0 );
      storage.setItem( "change_version", window.Ephemeral.version );
    }
  ...
  // Loads changes coming from the server, then call callback
    ...
    if( storage ){
      ...
      for( var ii = 0 ; ii < changes.length ; ii++ ){
        change = changes[ ii ];
        index = storage.getItem( "change_count_" + domain ) || "0";
        index = parseInt( index, 10 );
        storage.setItem(
          "change_" + domain + "_" + index,
          JSON.stringify( change )
        );
        storage.setItem( "change_count_" + domain, index + 1 );
  ...
  if( !window.kudo_ctx || window.kudo_offline_loader ){
    if( !offline ){
      ...
      if( offline ){
        ...
        console.info( "OFFLINE Kudocracy, back online" );
        debugger;
      }
    }
  ...
  // This callback is called once ephemeral database is restored
    ...
    // Offline without any data is not very usefull...
      ...
      if( kudo_ctx && kudo_ctx.session ){
        if( !kudo_ctx.session.i18n ){
          debugger;
          kudo_ctx.session.i18n = function( x ){ return x; }
        }
  ...
  if( !stored_count ){
    ...
    l8.tick( function(){
      ...
      for( var ii = 0 ; ii < stored_count ; ii++ ){
         changes.push( JSON.parse( 
           storage.getItem( "change_" + domain + "_" + ii )
         ) );  
      }
      Ephemeral.restore(
        changes,
