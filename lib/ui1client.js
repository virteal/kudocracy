...
function start_local_server( kudo_scope ){
  ...
  server.inject = function( type, parameters ){
    ...
    if( !window.kudo_ctx.authentic ){
      console.log( "Ignored change from not authentic visitor" );
      return;
    }
  ...
  // Loads changes coming from the server
    if( changes === "offline" ){
      if( !window.kudo_offline_loader ){
        console.log( "BUG? offline change yet no window.kudo_offline_loader" );
        debugger;
  ...
  ||  window.kudo_ctx && window.kudo_ctx.changes === "offline"
    ...
    console.log( "Switching to 'offline' loader mode" );
    if( window.kudo_offline_loader ){
    ...
    window.kudo_offline_loader = true;
    if( window.kuo_ctx && window.kudo_ctx.changes === "offline" ){
  ...
  function when_data_ready(){
    ...
    if( offline && stored_count ){
      console.log( "OFFLINE entry page" );
      ...
      ui1_server({
        ...
        writeHead: function(){},
          ...
          var scripts = "";
          if( false ){ // This version does not work.
            // Change content of whole current page. Note: scripts are not run
            try{
              console.log( html.substring( 0, 200 ) );
              window.$('html') // .empty().html( html );
              .get( 0 ).innerHTML = html;
            }catch( err ){
              console.log( "Error when loading html in page", err );
              $("boby").text( ":(" );
              debugger;
            }
            // Dynamic loading of script src=... stuff
            r.replace( /<script[\s\S]*?<\/script>/g, function( s ){
              var front = s.substring( 0, 300 );
              // console.log( "script: " + front );
              var idx_src = front.indexOf( 'src="http' );
              if( idx_src > 0 ){
                var tag = document.createElement( "script" );
                var src = s.substring( idx_src + 'src="'.length );
                src = src.substring( 0, src.indexOf( '"' ) );
                tag.setAttribute( "type", "text/javascript" );
                tag.setAttribute( "src", src );
                console.log( "Dynamic loading of script at", src );
                document.getElementsByTagName( "head" )[ 0 ].appendChild( tag );
              }else{
                scripts += s;
              }
            });
            // Run other inlined scripts
            try{
              // $("#progress_bar").html( scripts );
            }catch( err ){
          ...
          try{
            ...
            console.log( "BUG? error when setting new 'body' element", err );
            debugger;
          }
      ...
      if( kudo_ctx && kudo_ctx.session ){
        ...
        document.write( "data :(" );
        console.log( "BUG? missing session" );
      }
    }
  }
...
if( !window.ui1_server ){
  try{
    // When loaded from /offline home page, start a local UI
    if( window.kudo_offline_loader ){
      console.log( "This is the offline version starting..." );
      if( false ){
