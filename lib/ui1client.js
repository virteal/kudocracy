...
function start_local_server( kudo_scope ){
  ...
  function ajax_inject( type, parameters ){
    ...
    if( kudo_ctx.session.is_offline ){
      console.log( "Offline, retry later", retry_delay );
      retry();
      return;
    }
    ...
    if( !kudo_ctx.authentic ){
      console.log( "Online but not authenticated, keep waiting", retry_delay );
      retry();
      return;
    }
  ...
  server.inject = function( type, parameters ){
    ...
    if( !window.kudo_ctx.authentic ){
      console.log( "Change injection from not authentic visitor" );
    }
    
    console.log( "Queue change to send to server", arguments );
  ...
  // Loads changes coming from the server, then call callback
    ...
    if( restore_in_progress ){
      ...
      if( queued_load ){
        console.log( "BUG? invalid multiple calls to load()" );
        return;
      }
