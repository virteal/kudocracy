...
// If persona is specified and never voted the topic, it doubles the heat
  ...
  if( persona && !Vote.find( "" + persona.name + "." + this.name ) ){
    touched = touched * 2;
    if( age < vote.ONE_MONTH )return touched;
  }
...
Topic.prototype.add_comment = function( comment ){
  ...
  // Also update the "best" comment to describe the topic
  if( !this.comment() || comment.persona === this.comment().vote.persona ){
    ...
    if( !last_vote ){
      ...
      var age = vote.now() - last_vote.snaptime;
      if( age < vote.ONE_MONTH ){
        ok = true;
      }else if( age < vote.ONE_HOUR ){
        if( ante_last_vote ){
          age = vote.now() - last_vote.snaptime;
          if( age < vote.ONE_MONTH ){
            ok = true;
          }
        }
...
// propositions, filtered.
  ...
  if( Session.current.has_filter() ){
    ...
    var comment = tag_topic ? tag_topic.comment() : "";
    if( comment ){
