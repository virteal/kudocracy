...
Topic.prototype.check = function( dont_delay ){
  ...
  function dump(){
    ...
    votes.forEach( function( vote ){
      if( !result._added_votes[ vote.persona.id ] ){
        ...
        trace( "Result does not know about " + vote );
        if( !vote.valid() ){
    ...
    result_votes_keys.forEach( function( persona_id ){
      ...
      if( !persona ){
        trace( "Result includes vote from invalid persona", persona_id );
        return;
      }
      ...
      if( vote ){
        ...
        trace( "Result includes invalid vote from persona " + persona );
        var found = false;
        votes.forEach( function( vote ){
          if( vote.persona === persona ){
            trace( "Problematic vote: " + vote );
            found = vote;
          }
        });
        if( !found ){
          trace( "Problematic vote from " + persona, "not found in votes..." );
        }else{
          if( !found.valid() ){
  ...
  votes.forEach( function( vote ){
    ...
    if( orientation === Vote.agree ){
      ...
      trace( "BUG? unknown orientation", orientation );
      debugger;
    }
    count_votes++;
...
Topic.prototype.computed_tags = function( persona ){
  ...
  if( persona ){
    ...
    voted = true;
    if( vote && vote.orientation() !== Vote.neutral ){
      buf.push( "#vote" );
    }
...
Topic.prototype.get_persona = function(){
  var label = "@" + this.label.substring( 1 );
  return Persona.find( label );
};
...
function Vote( options ){
  ...
  if( proposition.id === "xdebat_f2_melencho" ){
    debugger;
  }
  ...
  this.previous_privacy = plasma( options.previous_privacy || Vote.public );
  ...
  // Trigger on orientation, delegation or privacy
  ...
  function update(){
    ...
    try{
      ...
      var was_privacy     = was_vote && was_vote.privacy;
      
      var orientation = vote.orientation();
      var delegation  = vote.delegation();
      var privacy     = vote.privacy();
      ...
      var privacy_changed = typeof was_privacy === "undefined"
      || privacy !== was_privacy;
      ...
      vote.previous_privacy( was_privacy );
...
// At expiration vote becomes public direct neutral for a while
  if( !this.orientation ){
    if( !this.buried ){
...
Vote.prototype.add = function(){
  ...
  if( vote.delegation() !== Vote.direct ){
    ...
    // Also make sure that the vote last for as long as the delegation does
    vote.outlive( vote.delegation() );
  }
...
function Result( options ){
  ...
  this.secret      = plasma( options.secret    || 0 );
  this.private     = plasma( options.private   || 0 ),
  this.count       = plasma( 0 );
  this._added_votes = {};
  this.proposition.touch();
  this.touch();
...
function Delegation( options ){
  ...
  this.privacy  = plasma( options.privacy );
  this.tags     = options.tags;
  ...
  function update(){
    ...
    if( inactive !== delegation.was_inactive ){
      ...
      if( !inactive ){
        ...
        // I could remove votes due to the delegation but user can either
        // vote directly or set another delegation to override the previous
        // vote. Hence, I do nothing.
      }
    }
...
exports.start = function( starter, is_local ){
  l8.begin.step( function(){ main( starter, is_local ); } ).end;
};
