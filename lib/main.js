...
// january 2015 by jhr, first alpha, 4530 LOC
//
// This file is included by the server side application (ui1.js) and also
// by the browser when the application run in "app mode", ie also on the
// client side (ui1client.js). In that case, some actions are forwarded to
// the server in addition to running locally and changes on the
// server side are sent to all the clients to keep them updated.
//
// ToDo: implement "realtime" updates. As of today, changes are sent when the
// client asks for them only.

"use strict";

// Global (kind of) "Scope". A lot of exported stuff is stored in it.
var Kudo = { store: "vote.json.log" }; // ToDo: "file://vote.json.log"

// Inject the "ephemeral" vocabulary into the Kudo global scope.
...
var Machine   = Kudo.Ephemeral.Machine;
...
function Persona( options ){
  ...
  // ToDo: test update()
  if( this.is_update() ){
    trace( "BUG? persona update is unexpected" );
    debugger;
    return persona;
    return persona.update( this );
  }
...
Persona.prototype.expiration = function(){
  trace( "Persona expiration: " + this );
  return Persona.super.prototype.expiration.call( this );  
};
...
Persona.prototype.short_label = function(){
  return this.label.substring( 1 );
};
...
// If there is a vote by persona on said topic, return it, or null/undef
  ...
  return Vote.find( this.id + "." + proposition.id );
};
...
Persona.prototype.get_alias = function( dflt ){
// Optional "alias" is @xxx at start of persona's topic's comment, if any.
// Returns "" (or optional default) if none.
// Note: Persona::set_volatile_alias() is used to import aliases from twitter.
  if( arguments.length < 1 ){
    dflt = "";
  }
  var alias = this._cached_alias;
  if( alias !== undefined )return alias || dflt;
...
// "volatile aliases" are aliases that are not stored locally but stored in
// the persona's twitter profile. They are the "full name" of the profile.
// Note: the data is shared by all the domains / ephemeral machines.
// They are volatile because they are not "persisted" by Ephemeral.
var VolatileAliases = map();


Persona.prototype.set_volatile_alias = function( alias ){
  this._cached_alias = alias;
  VolatileAliases[ this.label ] = alias;
};


Persona.set_volatile_aliases_map = function( map ){
  VolatileAliases = map;
};


Persona.get_volatile_aliases_map = function(){
  return VolatileAliases;
};
...
Persona.prototype.is_domain_owner = function(){
// A persona is a domain owner if the domain name matches the persona's name.
  // For demos and support
  var id = this.id.substring( 1 );
  if( id === "jhr" || id === "kudocracy" )return true;
  if( id === Ephemeral.Machine.current.id )return true;
  var domain = Ephemeral.Machine.find_id( id );
  if( !domain )return false;
  if( domain === Ephemeral.Machine.current
  ||  domain === Ephemeral.Machine.main
  )return true;
  return false;
};
...
// Return the potential Comment object associated to the personal tag of
// the user.
...
// Return a Persona, if there is a comment
...
// Return an array of delegations that include said proposition
  ...
  Ephemeral.each( this.delegations, function( delegation ){
    ...
    var agent = Persona.valid( delegation.agent );
    if( !agent )return;
    // If that agent delegates to the persona, skip that delegation loop
    // ToDo: manage multiple delegations
    if( agent.delegates_to( delegation.persona, delegation.tags ) )return;
    if( !agent.get_non_neutral_vote_on( proposition ) )return;
...
// Called by Vote constructor. Add the vote to the list of votes of the
// persona.
  De&&mand( vote.persona === this );
...
// Remove vote from list of votes of persona.
  var votes = this.votes();
...
Persona.prototype.is_member_of = function( group ){
  ...
  // return group._members_indexed_by_persona( this.key );
...
Persona.prototype.last_measure = function(){
  return this._measure;
}


Persona.prototype.measure_name = function(){
  return this._measure = this.key;
};
...
function Topic( options ){
  ...
  if( options.comment && !Comment.valid( options.comment ) ){
    trace( "BUG? Invalid comment for Topic " + this );
    debugger;
...
Topic.prototype.is_proposition = function(){ 
// A topic is either a proposition or a tag. User can vote on both types.
  return this.label[0] !== "#"; 
};


Topic.prototype.short_label = function(){
  return this.is_proposition() ? this.label : this.label.substring( 1 );
};
...
Topic.prototype.check = function( dont_delay ){
  ...
  if( !this.result ){
    de&&bug( "check() on resultless object" );
    return;
  }
...
// Collect random agent votes from at most n distinct agents
  ...
  var ii;
  var list = [];
  var avoid_map  = agents_avoid_map || map();
  var agent_vote;
  for( ii = 0 ; ii < many ; ii++ ){
...
// If persona is specified and never voted the topic, it doubles the heat
  ...
  if( age > Kudo.ONE_YEAR ){
    this.measure = 0;
    return age;
  }
  if( age > Kudo.ONE_WEEK ){
  ...
  // Double the heat if not voted by persona
  if( persona && !Vote.find( "" + persona.name + "." + this.name ) ){
    touched = touched * 2;
  }
  return this._measure = touched;
};
...
Topic.prototype.measure_age_modified = function( persona ){
  ...
  if( persona ){
    ...
    if( vote ){
      age = vote.measure_age_modified();
    }
  }
  if( !age ){
    age = Ephemeral.prototype.measure_age_modified.call( this );
  }
  return this._measure = age;
};


Topic.prototype.measure_heat = function( persona ){
  return this._measure = this.heat( persona );
};


Topic.prototype.measure_trust = function( persona ){
  if( !persona )return this.measure_direct_votes();
  return this._measure = persona.trust_level_on( this );
};


Topic.prototype.measure_activity = function(){
  return this.measure = this.votes_log().length;
};


Topic.prototype.measure_comments = function(){
  return this._measure = this.comments().length;
};


Topic.prototype.measure_author = function(){
  var author = this.get_comment_author();
  this._measure = author ? author.name : "";
...
// participation
  ...
  return this._measure = this.result.direct() / this.result.total();
};
...
// ToDo: increase weight of prostest votes?
  return this._measure
  = ( this.result.blank() + this.result.protest() ) / this.result.total();
};


Topic.prototype.measure_success = function(){
// Success is proportion of winning vote, whatever the agree/disagree result
  var total = this.result.total();
  if( total === 1 ){
    this._measure = 0;
  }
  var agree = this.result.agree();
  var disagree = this.result.disagree();
  if( agree > disagree ){
    return this._measure = agree / total;
  }else{
    return this._measure = disagree / total;
  }
};
...
Topic.prototype.measure_orientation = function(){
  ...
  if( orientation === Vote.neutral  ){ this.measure = 0; return 1; }
  if( orientation === Vote.blank    )return this._measure = 2;
  if( orientation === Vote.protest  )return this._measure = 3;
  if( orientation === Vote.disagree )return this._measure = 4;
  if( orientation === Vote.agree    )return this._measure = 5;
  return this._measure = 6;
};
...
Topic.reserved_tags_comments = {
  ...
  agree:      'with an "agree" vote from you',
  disagree:   'with a "disagree" vote from you',
  protest:    'with a "protest" vote from you',
  direct:     "with a direct vote from you",
  indirect:   "with an indirect vote from you",
  comment:    "with a comment from you",
  expire:     "with a vote that will expire soon (within less than a week)",
  expired:    "with a vote from an expired delegation",
  inactive:   "with a vote from a delegation that became inactive",
  trust:      "with at least a vote by a delegate",
  quorum:     "with more than 50% participation",
  quorum1:    "with 1 to 10% participation",
  quorum10:   "with 10 to 20% participation",
  quorum20:   "with 20 to 25% participation",
  quorum25:   "with 25 to 33% participation",
  quorum33:   "with 33 to 50% participation",
  quorum50:   "with 50 to 66% participation",
  quorum66:   "with 66 to 80% participation",
  quorum80:   "with 80 to 90% participation",
  quorum90:   "with 90 to 100% participation",
  ...
  problematic: "with more than 1% of protest votes",
  orphan:     "propositions without much votes",
  ...
  tie:        "with an agree/disagree equality",
...
Topic.prototype.computed_tags = function( persona ){
  ...
  var vote;
  if( persona ){
    vote = Vote.find( persona.id + "." + this.id );
    if( vote ){
      voted = true;
      if( !vote.orientation ){
        ...
        if( orientation !== Vote.neutral ){
          ...
          if( delegation === Vote.direct ){
            buf.push( "#direct" );
            if( vote.life_expectancy() <= Kudo.ONE_WEEK ){
              buf.push( "#expire" );
            }
          }else{
            buf.push( "#indirect" );
            var valid_delegation = Delegation.valid( delegation );
            if( !valid_delegation ){
              buf.push( "#expired" );
              trace( "BUG? indirect vote for invalid delegation " + vote );
              debugger;
            }else{
              if( vote.delegation().life_expectancy() <= Kudo.ONE_WEEK ){
                buf.push( "#expire" );
              }
              if( vote.delegation().is_inactive() ){
                buf.push( "#inactive" );
              }
            }
          }
  ...
  if( this.is_tag() ){
    ...
    // A proposition without tags is a draft
    if( !all_tags.length ){
      buf.push( "#draft" );
    // A #xxxx where @xxxx exists is a draft if there is only one tag
    }else if( all_tags.length === 1 ){
      var first_tag = this.tags()[0];
      if( !first_tag.key ){
        trace( "BUG? missing key for first tag of " + this );
        debugger
      }else{
  ...
  // #problematic if protest votes > 1% of agree votes
  if( this.result.is_problematic() ){
    buf.push( "#problematic" );
  }
  
  // Quorums
  var count_votes = this.result.total();
  
  // #orphan if no votes after a week
  if( count_votes <= 1 && this.age() > Kudo.ONE_WEEK ){
    buf.push( "#orphan" );
    
  // #referendum if 1% of people voted (at least 2!) and no vote from visitor
  }else if( this.result.is_referendum() && !voted && persona ){
    buf.push( "#referendum" );
  }
  
  var count_persona = Persona.count;
  var quorum = Math.floor( ( count_votes / count_persona ) * 10000 );
...
// Note: anyone (but abusive users) is the author when there is no currently
// valid author.
  if( !persona )return true;
  if( persona.is_abuse() )return false;
  var author = this.get_comment_author();
...
// True if proposition pass thru the filter, ie proposition not filtered out
  ...
  if( !this.is_tagged( filter, persona ) )return false;
  
  // Check the keywords, if any
  if( !query )return true;
  var full_text = this.full_text();
  return query.split( " " ).every( function( keyword ){
    // Check presence, with word boundaries, or start/end of text
...
Topic.prototype.tags_string = function( persona, with_abuses, not_computed ){
  ...
  var sorted_tags = this.tags().sort( function( a, b ){
    if( !a.heat || !b.heat ){
      trace( "BUG? missing .heat() for a: " + a, "or b:" + b );
      debugger;
      return 0;
    }
...
Topic.prototype.get_data = function( key ){
  if( !this.key || !this.data )return; // undefined
  return this.data[ key.toString() ];
};


Topic.prototype.set_data = function( key, value, persona ){
  Store.inject( {
    proposition: this,
    key: key,
    value: value,
...
Topic.prototype.count_voters = function( span ){
  
  var time_limit;
  var now = l8.now;
  var duration = Kudo.ONE_YEAR;
  
  if( !span ){
    time_limit = 0;
  }else{
    time_limit = l8.now - Kudo[ "ONE_" + span ];
  }
  ...
  if( !time_limit ){
    ...
    duration = now - time_limit;
  }
  
  var all_votes = this.votes_log();
  var log_size = all_votes.length;
  var cache_id = "_cached_count_voters_" + span;
  var cached = this[ cache_id ];
  
  if( !cached ){
    this[ cache_id ] = cached = { timestamp: 0, count: 0, log_size: 0 };
    
  }else{
    var age = now - cached.timestamp;
    // Use cache unless new votes or too old
    if( cached.log_size === log_size
    && age < duration / 52 // 26m/day, 2hr/week, 1week/year
    )return this._measure = cached.count;
  }
  ...
  var count = 0;
  while( ii-- ){
    vote_value = all_votes[ ii ];
    if( vote_value.timestamp < time_limit )break;
    persona_id = vote_value.persona;
    // Skip neutral first vote
...
Topic.prototype.last_measure = function( measure_name ){
  if( measure_name ){
    var first_char = measure_name[ 0 ];
    if( first_char === "+" || first_char === "-" ){
      measure_name = measure_name.substring( 1 );
    }
    var measurer = this[ "measure_" + measure_name ];
    if( !measurer ){
      trace( "BUG, cannot find last measure ", measure_name, "for" + this );
      return undefined;
    }
    return measurer.call( this );
  }
  return this._measure;
};


Topic.prototype.measure_voters_today = function(){
  return this.count_voters( "DAY" );
};


Topic.prototype.measure_voters_this_week = function(){
  return this.count_voters( "WEEK" );
};


Topic.prototype.measure_voters_this_month = function(){
  return this.count_voters( "MONTH" );
};


Topic.prototype.measure_voters_this_year = function(){
  return this.count_voters( "YEAR" );
};
...
function Tagging( options ){
  ...
  if( this.comment && !Comment.valid( this.comment ) ){
    trace( "BUG? Invalid comment for Tagging " + this );
    debugger;
    this.comment = undefined;
  }
  this.comment_text = options.comment_text;
  ...
  this.detags.forEach( function( tag ){
    var tag_entity = ( tag.entity_type && tag ) || Topic.find( tag );
    if( !tag_entity ){
  ...
  this.tags.forEach( function( tag ){
    ...
    var tag_entity;
    if( tag.entity_type ){
      tag_entity = Topic.valid( tag );
      if( !tag_entity ){
        trace( "Invalid tag entity in Tagging" );
        return;
      }
    }else{
  ...
  // Inject the potential comment
  if( this.comment_text && this.persona ){
    var vote_entity = this.persona.get_vote_on( this.proposition );
    // Either using an existing vote by persona
    if( vote_entity ){
      Comment.inject({
        vote: vote_entity,
        text: this.comment_text
      });
    // Or using a newly created vote (if persona is known, else comment lost)
    }else{
      // Inject an "agree" vote on the proposition, with comment
      if( this.persona ){
        Vote.inject({
...
/*
 *  Store event - simple key/value store
 *
 *  This event is created typically when some UI changes the data for a
 *  proposition/topic.
 *  Potential huge side effects...
 *  Only the owner of the proposition is supposed to have such a power!
 *
 ...
 *
 *  Attributes
 *    - proposition -- the proposition being changed 
 *    - key         -- string name of the data  
 *    - value       -- data itself, undefined to erase, JSON compatible
 *    - persona     -- optional author of the event, null if system origin
 */
...
function Store( options ){
  ...
  var data = this.proposition.data;
  if( !data ){
    data = this.proposition.data = map();
  }
...
Store.prototype.valid = function(){
  return ( !this.expired() && this.proposition.valid() ) ? this : null;
};
...
function Comment( options ){
  ...
  if( options.vote !== Vote.find( options.vote.key ) ){
    ...
    trace( "options.vote.type: " + options.vote.entity_type );
 ...
 *  Vote is either "direct" or "indirect" with via a delegation.
 */
...
function Vote( options ){
  ...
  if( this.is_update() ){
    if( !vote.buried ){
      ...
      if( vote.expired() )return vote;
      this.persona.outlive( vote );
      this.proposition.outlive( vote );
  ...
  function update(){
    ...
    try{
      ...
      = was_orientation === undefined || orientation != was_orientation;
      
      var delegation_changed 
      = was_delegation === undefined || delegation !== was_delegation;
      ...
      &&  !delegation_changed
        ...
        if( time_delta < 10 * 1000 ){
          // trace( "Ignored duplicated update on vote " + vote );
          return;
        }
      }
...
Vote.share( "log", function(){ return []; } );
Vote.share( "some_expired_votes", false );
...
Vote.prototype.last_measure = function(){
  return this._measure;
}
...
Vote.prototype.measure_orientation = function(){
  ...
  if( orientation === Vote.neutral  ){ this._measure = 0; return 1; }
  if( orientation === Vote.blank    )return this._measure = 2;
  if( orientation === Vote.protest  )return this._measure = 3;
  if( orientation === Vote.disagree )return this._measure = 4;
  if( orientation === Vote.agree    )return this._measure = 5;
...
Vote.prototype.measure_topic = function(){
  return this._measure = this.proposition.measure_name();
};
Vote.prototype._measure_topic_is_alpha = true;


Vote.prototype.measure_voter = function(){
  return this._measure =this.persona.name;
};
Vote.prototype._measure_voter_is_alpha = true;


Vote.prototype.measure_total_votes = function(){
  return this._measure = this.proposition.measure_total();
};
...
Vote.prototype.update = function( other, options ){
  ...
  this.persona.outlive( this );
  this.proposition.outlive( this );
  this.proposition.check();
  this.set_duration( other.duration = options.duration );
  this.previous_orientation( 
    other.previous_orientation  = options.previous_orientation
  );
...
// At expiration vote becomes direct neutral for a while
  ...
  if( !this.is_neutral() ){
    de&&bug( "Pre-expiration for " + this );
    if( this.persona.expired() ){
      trace( "Expired " + this.persona + " at expiration " + this );
      return this;
    }
    if( this.proposition.expired() ){
      trace( "Expired " + this.proposition + " at expiration " + this );
    }
    this.resurrect();
    this.renew();
...
Vote.prototype.snap_value = function(){
  var val = {
    id: this.id,
    key: this.key, // ToDo: useless?
    label: this.label,
    persona: this.persona.id,
    proposition: this.proposition.id,
    orientation: this.orientation(),
    previous_orientation: this.previous_orientation(),
    delegation: ( this.delegation() && this.delegation().id ) || null,
    duration: this.duration,
    time_touched: this.time_touched,
    timestamp: this.timestamp
  };
  if( !this.comment ){
    trace( "BUG? missing .comment" );
    debugger;
  }else
  if( this.comment() ){
    val.comment = this.comment().id;
    if( this.comment().text ){
      val.comment_text = this.comment().text;
    }
  }
  if( de ){
    var val_t = this.value();
    var ok = true;
    for( var attr in val ){
      if( val[ attr ] !== val_t[ attr ] ){
        // ToDo: issue with comment_text
        if( attr === "comment_text" )continue;
        debugger;
        ok = false;
      }
    }
    for( attr in val_t ){
      if( val_t[ attr ] !== val[ attr ] ){
        ok = false;
        debugger;
      }
    }
    if( !ok ){
      trace( "BUG? fast value is not equal to true value" );
      val = val_t;
    }
  }
...
Vote.prototype.remove = function( was ){
  //debugger;
  De&&mand( !was.entity_type );
  var vote = this;
  de&&bug( "Remove vote " + this, "was " + was.oriention );
  //de&&bugger();
  vote.proposition.remove_vote( was );
  var delegation = Delegation.find( was.delegation );
  if( delegation ){
    delegation.untrack_vote( this );
  }
  return this;
};
...
// Return true if a vote occured.
  ...
  if( delegations.length === 1 ){
    return this.vote_using_delegation( delegations[0] );
  }
  // If multiple delegations apply, select the most recently touched active one
  // amoung the ones with the most specific tags (the bigger number of tags)
  ...
  // True unless every delegation failed
  var that = this;
  return !delegations.every( function( delegation ){
    return !that.vote_using_delegation( delegation );
  });
};
...
// Try to delegate vote using delegation, return true if done
  ...
  if( agent_orientation === Vote.neutral ){
    ...
    trace( "BUG? todo: handle neutral vote for delegation" );
    debugger;
  }
  ...
  // Dont vote if delegation loops
  if( delegation.agent.delegates_to( persona, delegation.tags ) ){
    de&&bug( 
      "No vote, looping delegation " + delegation,
      "persona " + persona,
      "proposition " + this.proposition
    );
    // ToDo: provide UI info about presence of a loop
    return false;
  }
...
Vote.prototype.get_comment = function( allow_abuser ){
  var comment = Comment.valid( this.comment() );
  if( !comment )return null;
  if( this.persona.is_abuse() )return null;
  return comment;
};


Vote.prototype.get_comment_author = function( allow_abuser ){
// Return a Persona, if there is a comment
  var comment = this.get_comment( allow_abuser );
  if( !comment )return Persona.valid( this.persona() );
  return comment.vote.persona;
...
Vote.prototype.get_comment_text = function( allow_abuser ){
// Return a string
  var comment = this.get_comment( allow_abuser );
  return comment ? comment.get_text() : "";
};
...
Vote.prototype.set_comment = function( comment ){
  ...
  if( persona ){
    // Because comment contains the new alias
...
function Result( options ){
  ...
  this.orientation = function(){
    ...
    if( r.expired() ){
      now = Vote.neutral;
    }else if( r.agree() > r.disagree() ){
      // Won
      if( r.agree() > r.blank() + r.protest() ){
        // agree > blank/protest, > disagree
        now = Vote.agree;
      }else{
        // blank/protest > agree, > disagree
        now = Vote.blank;
        if( r.protest() > r.blank() ){
          now = r.protest() > 0 ? Vote.protest : Vote.neutral;
        }
      }
    }else{
      // Lost
      if( r.disagree() > r.blank() + r.protest() ){
        now = Vote.disagree;
      }else{
    ...
    return _;
  }.when( this.agree, this.protest, this.blank );
...
Result.prototype.is_tie = function(){
  return ( this.agree() === this.disagree() ) && this.total();
};
...
function DelegationFilter( tags ){
  ...
  nde&&bug( "New DelegationFilter", label );
  return this;
}


DelegationFilter.all = map();
...
function DelegationExpertize( delegation ){
  ...
  nde&&bug( "New DelegationExpertize", this.label );
  return this;
}


DelegationExpertize.all = map();
...
DelegationExpertize.entity_type = "DelegationExpertize";
...
DelegationExpertize.prototype.expired = function(){
// Hack to look like an entity
  return this.delegation.expired();
};
 ...
 *
 *  Attributes:
 *     id: @persona_id.@agent_id.#tag1_id.#tag2_id...
 *     persona
 *     agent
 *     tags: list of tag entities
 *     inactive: plasma boolean
 *     votes: plasma list of votes
 *
...
function Delegation( options ){
  ...
  && agent.delegates_to( persona, options.tags || delegation.tags )
    ...
    // ToDo: should provide a flag that signals loops
    ...
    if( inactive !== delegation.was_inactive ){
      ...
      if( !inactive ){
        ...
        if( delegation.agent.delegates_to( delegation.persona, delegation.tags ) ){
          de&&bug( "Activate a looping delegation " + delegation );
          // ToDo: should provide info about the presence of a loop
        }
...
Delegation.prototype.is_active = function(){
  if( !this.inactive ){
    trace( "BUG? invalid Delegation " + this );
    debugger;
    return false;
  }
...
// Compute the "heat" of a delegation. "Hot delegations" should come first.
  ...
  // Recently touched are hot
  var age = Kudo.now() - touched;
  if( age < Kudo.ONE_WEEK )return touched;
...
// Update all votes based on current delegation state.
// When delegation is inactive, there should be no votes.
  ...
  Ephemeral.each( delegation.votes, function( vote ){
    ...
    if( vote.delegation() !== delegation ){
      trace( "BUG? invalid delegation's vote" );
      return;
    }
    if( vote.orientation() === Vote.neutral ){
      trace( "BUG? invalid neutral delegated vote" );
      debugger;
    }
...
Delegation.prototype.untrack_vote = function( vote ){
  var votes = this.votes();
  var idx = votes.indexOf( vote );
  De&&mand( idx !== -1 );
  votes.splice( idx, 1 );
...
Delegation.prototype.expiration = function(){
  if( !this.inactive ){
    trace( "BUG? invalid delegation at expiration" );
  }
  if( this.inactive && !this.inactive() ){
    ...
    var len = list.length;
    if( len ){
      trace( "BUG? non empty list of vote at expiration of " + this, len );
      debugger;
    }
    de&&mand( !this.votes().length );
...
/*
 *  For UI.
 *  ToDo: this is unused at this point.
 */
...
var replized_verbs      = Kudo.replized_verbs      = map();
var replized_verbs_help = Kudo.replized_verbs_help = map();
...
// no log file of persisted changes.
  ...
  //   ex: e( Vote, Persona, @jhr, e( Topic, "Hulot president" ) );
    if( arguments.length === 1 && type && type.entity_type )return entity = type;
...
if( false && de ){
  // ToDo: per machine, not global, it works for the main machine only.
