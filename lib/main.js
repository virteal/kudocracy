  ...
  if( this.orientation() === Vote.neutral ){
    ...
    if( this.delegation() === Vote.direct ){
      ...
      if( this.delegation() !== Vote.direct ){
        ...
        // Only record neutral vote once, if persona wants to be neutral
        // she can expire the current vote
...
function Delegation( options ){
  ...
  if( this.is_update() ){
    // ToDo: avoid double processing of duration for updates
    delegation.duration( options.duration );
...
// propositions, filtered.
  ...
  if( Session.current.has_filter() ){
    ...
    var comment;
    if( comment = Topic.reserved_comment( Session.current.filter.trim() ) ){
