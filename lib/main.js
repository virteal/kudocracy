...
Persona.prototype.agents = function( proposition ){
// Return alpha sorted list of all agents, among all delegations
  ...
  Ephemeral.each( this.delegations(), function( delegation ){
    ...
    if( proposition ){
      ...
      if( !vote )return;
      if( vote.is_neutral() )return;
    }
    if( list.indexOf( agent ) !== -1 )return;
    list.push( agent );
  });
...
Topic.prototype.is_abuse = function(){ return this.result.is_abuse(); };
...
Topic.prototype.measure_heat = function(){ return this.heat(); };
...
Topic.prototype.get_comment = function(){
  ...
  return ( comment && comment.valid() ) ? comment : null;
};
...
Vote.prototype.measure_age = function(){ return this.age(); };
...
Vote.prototype.measure_topic = function(){ return this.propostion.measure_name(); };
...
Vote.prototype.filtered = function( filter, persona ){
  ...
  return this.proposition.filtered( filter, persona );
};
