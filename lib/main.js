...
// Success is proportion of winning vote, whatever the for/against result
  ...
  if( agree > against ){
    ...
    return this._measure = against / total;
  }
};
...
Topic.reserved_tags_comments = {
  ...
  disagree:   'with a "disagree" or "protest" vote from you',
  ...
  tie:        "with a for/against equality",
      ...
      if( !vote.orientation ){
        ...
        if( orientation !== Vote.neutral ){
          ...
          if( orientation === Vote.agree ){
            ...
            ||      orientation === Vote.protest
          ){
            buf.push( "#disagree" );
            if( orientation == Vote.protest ){
              buf.push( "#protest" );
            }
          }
...
function Vote( options ){
  ...
  this.source  = plasma( options.source );
  if( options.comment ){
    if( !Comment.valid( options.comment ) ){
      trace( "BUG? Invalid comment for Topic " + this );
      debugger;
    }else{
      this.comment = plasma( options.comment );
...
// Update all vote based on current delegation state.
