...
Session.prototype.has_filter = function(){
  return this.visitor && !!this.filter.length;
}
...
// Focus on one proposition
  ...
  if( Session.current.visitor ){
    buf.push( ['\n<form name="proposition" url="/">',
      '<input type="hidden" name="input" maxlength="140" value="change_proposition"/>',
      '<input type="hidden" name="input3" value="' + proposition.label + '"/>',
      '<input type="input" placeholder="tags" name="input4" />',
      ' <input type="submit" name="input2" value="Tag"/>',
      '</form>\n'
    ].join( "" ) );
  }
  buf.push( '<br>' );
...
Kudo.extend( http_repl_commands, {
  ...
  change_proposition: function( name, proposition_name ){
