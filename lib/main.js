...
// other personas, on the same proposition.  de&&mand( vote.persona === this );
  var persona     = this;
...
function Tweet( options ){
  ...
  if( !Person.valid( options.persona ) )return null;
...
Vote.prototype.agent = function(){
  ...
  if( !Vote.valid( this.agent_vote )
  || this.agent_vote.persona !== agent
  ){
    this.agent_vote = agent.get_vote_on( this.proposition );
  }
  if( !this.agent_vote ){
  ...
  if( !this.agent_vote.is_public() ){
    trace( "BUG? non public agent vote " + this.agent_vote + " for " + this );
    debugger;
    this.agent_vote = null;
    return null;
  }
  return this.agent_vote.persona;
};
