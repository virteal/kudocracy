...
Topic.prototype.add_comment = function( comment ){
  ...
  ||  comment.vote.persona === main_comment.vote.persona
  ){
...
Topic.prototype.get_comment = function(){
  ...
  if( comment.vote.persona.is_abuse() )return null;
  return comment;
};


Topic.prototype.get_comment_author = function(){
  var comment = this.comment();
  var vote = Vote.valid( comment && comment.vote );
  var author = Persona.valid( vote && vote.persona );
  return author;
};


Topic.prototype.get_comment_text = function(){
  var comment = this.get_comment();
...
Vote.prototype.snap_value = function(){
  ...
  val.comment_text = this.comment() && this.comment().text;
...
Vote.prototype.add = function(){
  ...
  if( vote.delegation() !== Vote.direct ){
    vote.delegation().count_indirectionsld++;
...
// Direct neutral vote triggers delegations
  ...
  while( true ){
    ...
    if( recent_delegation ){
      ...
      if( this.vote_using( recent_delegation ) )break;
    }else{
      break;
    }
  }
...
Vote.prototype.vote_using = function( delegation ){
...
// Called by topic.remove_vote()
  ...
  if( !this.added_votes[ was.persona ] ){
    trace( "Bug, removed was was never added, vote: " + was.persona );
    debugger;
    return this;
  }else{
...
function DelegationExpert( agent, tags ){
  var delegation_filter = new DelegationFilter( tags );
  var id = agent.id + "." + delegation_filter.id;
  var found = DelegationExpert.all[ id ];
  if( found )return found;
  this.id = id;
  this.delegations = [];
  this.count_votes = 0;
  agent.delegation_filters.push( this );
  DelegationExpert.all[ id ] = this;
  delegation_filter.experts.push( this );
  return this;
}


DelegationExpert.all = {};


DelegationExpert.register = function( agent, tags ){
  return new DelegationExpert( agent, tags );
};
...
function Delegation( options ){
  ...
  function update(){
    ...
    if( delta.changes ){
      ...
      this.delegation_expert = DelegationExpert.register(
        delegation.agent,
        delegation.tags()
      );
    }
