...
var basic_style_respond = function( question ){
  ...
  if( !head || head.indexOf( "<title>" ) === -1 ){
    title = '<title>Kudocracy test UI, liquid democracy meets twitter...</title>';
  }
...
function page_style( title ){
  ...
  if( title ){
    ...
    buf.push( "<title>Kudocracy</title" );
  }
  buf.push(
    '<link rel="stylesheet" href="http://simpliwiki.com/simpliwiki.css" type="text/css">',
...
PageBuilder.prototype.push_title_and_search_form = function( title ){
  ...
  if( filter_label ){
    filter_label = filter_label.replace( /#[A-Za-z0-9]+/g, function( tag_name ){
...
// The private page of a persona
  ...
  var persona = this.session.visitor;
  if( !persona )return this.error( "No Persona" );
  ...
  var old_filter = this.session.filter;
  if( old_filter.indexOf( " #new " ) !== -1 ){
    this.session.set_filter( old_filter.replace( " #new ", "" ) );
  }
  
  // Don't display votes unless some delegateable filter exists, too much
  var display = !!this.session.has_delegateable_filter();
  if( !display && this.session.filter.indexOf( " #tag ") !== -1 ){
    // Do display tags however
    display = true;
  }
...
// This is the "public" aspect of a persona
  ...
  // Display each vote, not too much, only when a delegateable filter exists
...
// The private page of a persona's delegations
  ...
  var title = link_to_persona_page( persona );
...
// Focus on one proposition
  ...
  if( !proposition ){
    return this.error( "Proposition not found: ", proposition_name );
  }
  ...
  if( tmp || this.session.visitor ){
    ...
    if( this.session.visitor && !this.session.visitor.is_abuse() ){
      buf.push(
        ...
        '<input type="text" placeholder="', i18n( "additional tag" ), '" name="i4" />',
