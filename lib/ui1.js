...
function handle_basic_style_request_input( req, res ){
  ...
  if( redir ){
    ...
    req.query = querystring.parse( redir );
    req.query.i = req.query.i.replace( /\//g, " " ).trim();
    return req.query.i;
  }
...
function page( name ){
  ...
  result.session = Session.current;
  result.session.page_builder = result;
  ...
  if( !redir && result.session.can_pushState ){
    ...
    if( state ){
      redir = "?i=" 
      + querystring.escape( "page " + params.join( " " ) + " " + state )
      .replace( /%20/g, "/" );
    }
  }
  if( redir ){
...
function link_to_command( cmd, title ){
  ...
  var r = '<a href="?i=' + url_code + '">' + ( title || cmd ) + '</a>';
  return r;
}
...
function link_to_persona_page( persona ){
  ...
  var title = persona.label;
  return link_to_page( "persona", title, title );
}
...
function page_header( left, center, right ){
  ...
  + ( left ? " " + left : "" );
  if( PageBuilder.current.session.visitor ){
    right = ( ( right && ( right + " " ) ) || "" )
    + link_to_page(
      "visitor",
      "",
      PageBuilder.current.session.visitor.label
    );
  }else{
    right = ( ( right && ( right + " " ) ) || "" )
      + link_to_page( "login" );
...
function vote_menu( vote, proposition, orientation, options ){
  ...
  // vote parameter is either a vote or a persona
  if( vote.type === "Vote" ){
    vote_id = vote.id;
  ...
  if( len ){
    ...
    recommendations = "other " + recommendations.sort().join( " " );
    if( len > 7 ){
      recommendations += "...";
    }
  }
  ...
  var buf = [
    ( len ? '\n<br>' + recommendations + '\n<br>' : "" ),
    '\n<form name="vote" url="/">',
    ( orientation !== Vote.agree    ? link_to_command( "change_vote " + vote_id + " agree", emoji( "agree" ) ) : "" ),
    ( orientation !== Vote.disagree ? link_to_command( "change_vote " + vote_id + " disagree", emoji( "disagree" ) ) : "" ),
    " or ",
...
function filter_and_sort_menu( can_propose, tag_page ){
  ...
  return [
    ...
    '<input type="search" autosave="filter" autofocus name="i3" value="',
      tags_label,
    '"/> ',
...
function push_title_and_search_form( buf, title ){
  ...
  buf.push( filter_and_sort_menu( can_propose, tag_page ) );
...
// The private page of a persona
  ...
  if( !persona )return this.set( page_style(), "No Persona not" );
  ...
  var tag_set = push_title_and_search_form( this, persona.label );
  ...
  votes.forEach( function( entity ){
    ...
    that.push( '<br><br>',
      ...
      vote_menu( entity, _, _, { with_twitter: true } )
    );
...
// The private page of a persona's delegations
  ...
  var tag_set = push_title_and_search_form( this, title );
  ...
  delegations.forEach( function( entity ){
    ...
    var votes = votes_by_delegation[ entity.id ];
...
// propositions, filtered.
  ...
  // Change #new filter into #hot if there is no persona logged in
  // because nothing is #new when persona is unknown and displaying
  // nothing is not a good idea
  var persona = this.session.visitor;
  if( !persona && filter === " #new " ){
    filter = this.session.set_filter( "#hot" );
  }
  ...
  var tag_set
  = push_title_and_search_form( this, tag_page ? "Tags" : "Propositions" );
  ...
  list.forEach( function( proposition ){
    ...
    if( that.session.visitor ){
      var vote_entity
      = Vote.find( that.session.visitor.name + "." + proposition.name );
      if( vote_entity ){
        that.push( '\n<br>you ',
          ...
          ( vote_entity.half_life() ? " for " + duration_label( vote_entity.expire() - Kudo.now() ) : "" ),
          vote_menu( vote_entity ),
          '<br>'
        );
      }else{
        that.push( vote_menu( that.session.visitor, proposition ), '<br>' );
      }
    }else{
      that.push( '<br>' );
    }
  ...
  // Change default filter if nothing was displayed, ie widen the narrow filter
  if( !count ){
    if( this.session.filter === " #new " ){
      this.session.set_filter( "#hot" );
      this.redirect( tag_page ? "tags" : "propositions" );
    }else if( this.session.filter === " #hot " ){
      this.session.set_filter( "#recent" );
      this.redirect( tag_page ? "tags" : "propositions" );
    }else if( this.session.filter === " #recent " ){
      this.session.set_filter( "all" );
      this.redirect( tag_page ? "tags" : "propositions" );
    }
    return;
  }
...
// This page let's visitor ask for results about tags by specified personas
  ...
  var tag_set = push_title_and_search_form( this, "Ballot" );
  ...
  if( false
  //&& ( !tags.length && !personas.length && this.session.has_filter() )
  //||  ( tags.length  && !personas.length )
  ){
    
    var filter = !tags.length && this.session.filter;
    var added_personas = {};
    ...
    if( !valid_query && filter ){
      this.session.set_filter( "" );
    }
    valid_query = date.toISOString() + " " + valid_query;
    
    return page_ballot.call( this, pagename, "results", valid_query );  
  }
  ...
  this.push(
    ...
    '<input type="hidden" value="page ballot2" name="i"/>',
...
// This is the votes page of the application, filtered.
  ...
  var tag_set = push_title_and_search_form( that, "Votes" );
  ...
  valid_votes.forEach( function( vote_value ){
    ...
    that.push( link_to_page( "proposition", proposition.label ) );
...
function page_leaders( page_name ){
  ...
  var tag_set = push_title_and_search_form( this, "Leaders" );
  ...
  while( ii-- ){
    ...
    if( count_propositions < 200 && !seen_propositions[ entity.proposition.id ] ){
      seen_propositions[ entity.proposition.id ] = true;
      tag_set.add_proposition( entity.proposition );
      count_propositions++;
    }
    ...
    if( !seen_agents[ agent_id ] ){
      ...
      count_votes_by_agent[ agent_id ] = 0;
      agent_ids.push( agent_id );
      count_agents++;
    }
    
    delegations_by_agent[ agent_id ].push( delegation );
    count_votes_by_agent[ agent_id ]++;
  }
  // I now get a big list of delegations for the 200 last seen agents
  
  // Sort agent for number of delegated votes
  agent_ids = agent_ids.sort( function( a, b ){
    return count_votes_by_agent[ b ].length - count_votes_by_agent[ a ].length;
  });
  ...
  agent_ids.forEach( function( agent_id ){
    ...
    that.push(
      ...
      " ",
      "" + ( Math.round( 1000 * ( count_votes_by_agent[ agent_id ] / count_votes ) ) / 10 ) + "%",
      "<br>"
    );
    ...
    Ephemeral.each( agent_delegations, function( delegation ){
      ...
      // Count number of votes involved
    ...
    // Display seven most seen tag sets
    var len = tag_strings.length;
    for( var ii = 0 ; ii < len && ii < 7 ; ii++ ){
      that.push(
        link_to_page( "persona", agent_id + " " + tag_strings[ ii ], tag_strings[ ii ] ),
        " "
      );
    }
    that.push( "<br>" );
...
// Focus on one proposition
  ...
  if( this.session.visitor ){
    var vote_entity = Vote.find( this.session.visitor.name + "." + proposition.name );
    if( vote_entity ){
      buf.push(
        '<br>you ',
        ...
        vote_menu( vote_entity, true /* with comment */, _, { with_twitter: true, nofocus: true } )
      );
    }else{
      buf.push( vote_menu( this.session.visitor, proposition ) );
    }
    buf.push( "<br>" );
  }
  ...
  this.set(
    ...
    + '</script>',
    page_header(
      link_to_page( "ballot", proposition.name, "ballot" ),
...
http_repl_commands.change_vote = function( vote_entity, privacy, orientation, duration, comment ){
  ...
  // Special change_vote vote_id "agree" / "disagree" case
  if( privacy === "agree"
  ||  privacy === "disagree"
  ){
    vote_id = vote_entity;
