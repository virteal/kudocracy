...
var config = {
  ...
  index_style: "http://simpliwiki.com/style.css",
  icon:  "http://simpliwiki.com/yanugred16.png",
  ueb_icon: "http://simpliwiki.com/ueb16.png"
};
...
Session.prototype.remove_new_filter = function(){
...
var basic_style_respond = function( question ){
  ...
  PendingResponse.end( [
    ...
    '\n<link rel="shortcut icon" href="', config.icon, '" type="image/png">',
    title,
    head,
    '\n</head>\n',
    '\n<body>',
...
function link_to_wiki( page, title ){
  ...
  var img = '<img src="' + config.ueb_icon + '"/>';
  var encoded_page = page;
  if( encoded_page[ 0 ] === "#" ){
    encoded_page = "on_" + encoded_page.substring( 1 );
  }else if( false && encoded_page[ 0 ] === "@" ){
    encoded_page = "User" + encoded_page;
  }
  var href = config.wiki + encoded_page;
  if( !title ){
...
// Header with Kudocracy/propositions/tags/votes/ballot ... login help
  ...
  var m = left || "";
  return page_header(
    m      + link_to_page( "leaders", "", "delegations" )
    + " "  + link_to_page( "votes" )
    + " "  + link_to_page( "ballot" ),
    center,
    right
...
PageBuilder.prototype.push_vote_menu = function( proposition, options ){
  ...
  if( persona ){
    ...
    if( vote_entity ){
      ...
      this.push( vote_menu( persona, proposition, options ), '<br>' );
    }
  }else{
    this.push( '<br>' );
  }
...
function get_sparkline_data( proposition ){
  ...
  // Add fake data for continuity to time when proposition was last touched
  graph_serie.push( [ touched, balance ] );
...
Sparklines.prototype.add = function( proposition ){
  var data = get_sparkline_data( proposition );
  if( !data.span )return;
  this.page_builder.push(
    '<br><canvas id="spark_', data.label,
    '" width=100 height=15></canvas>'
  );
  this.list.push( data );  
};
...
function sparkline( data ){
  if( !window.HTMLCanvasElement )return;
	...
	var xfactor = width  / duration;
	var yfactor = height / ( max - min );
	var delta_y = ( - min ) * yfactor;
	ctx.beginPath();
	ctx.strokeStyle = 'rgba(170,170,170,1)';
	ctx.moveTo( width, height - delta_y );
	ctx.lineTo( 0, height - delta_y );
	ctx.stroke();
	ctx.beginPath();
...
Sparklines.prototype.push = function(){
  ...
  this.list.forEach( function( data ){
    // All lines starts and ends at the same date
...
// The private page of a persona
  ...
  this.session.remove_new_filter();
...
// This is the "public" aspect of a persona
  ...
  if( this.session.novice ){
    this.push( i18n( 
      "This page lists informations about a person, her votes, her delegations, etc."
    ) );
  }
    ...
    for( var eii = 0 ; eii < elen && eii < 10 ; eii++ ){
      ...
      if( can_delegate ){
        that.push(
          ...
          '<input type="submit" value="', i18n( "Delegate" ), '"/> ',
          link_to_persona_page( expertize.agent ), " ", link_to_wiki( expertize.agent )
        );
      }
      that.push(
        link_to_page(
...
// The private page of a persona's delegations
  ...
  if( this.session.novice ){
    this.push( i18n( 
      "This page lists your delegations to delegate/agents that vote for you on propositions that match some specified tags."
    ) );
  }
...
function page_ballot2( /* pagename, ...query */ ){
  ...
  if( this.session.novice ){
    this.push( "<br>", i18n( 
      "You can change the limit date, the propositions and the authorized voters: "
    ), "<br>" );
  }
  this.push(
...
function page_leaders( page_name ){
  ...
  this.set( page_style( "leaders" ), page_header_left() );
  
  if( this.session.novice ){
    this.push( i18n( 
      "This page lists popular delegates/agents and associated tags. If logged in, you can delegate."
    ) );
  }
  ...
  // People. Display each agent
  this.push( "<br><br><h2>", i18n( "People" ), "</h2>" );
  if( persona ){
    this.push( " - ", link_to_page( "delegations" ) );
  }
  this.push( "<br>");
  ...
  if( last_vote ){
    this.push(
      ...
      "<br>", i18n( "indirect vote" ), " ",count_votes, 
...
function page_index(){
  ...
  var lang = this.session.lang;
  var auto_lang = this.session.auto_lang;
  this.session.clear();
  this.session.lang = lang;
  this.session.auto_lang = auto_lang;
  this.session.needs_twitter = true;
...
function page_help(){
  ...
  var msg = [];
  msg.push( link_to_wiki( this.session.domain + "/HomePage", "wiki" ), "<br>" );
...
//      '<li>You can share your votes or hide them.</li>',
  ...
  // French
    ...
    msg.push(
      ...
      'les gens peuvent ', emoji( "agree" ) + 'approuver ou ', emoji( "disagree" ),
      ...
      '<h2>En quoi est-il diff&eacute;rent ?</h2><br>',
...
// Focus on one proposition
  ...
  // Agents, actually inserted later
  ...
  // Header, rebuilt because it now contains data in head of html page
  buf = this._body;
  this.set(
    this.head()
