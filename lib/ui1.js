...
// options: with_twitter
  ...
  if( with_comment && vote ){
    ...
    if( comment ){
      ...
      with_comment += 'placeholder="comment vote"';
    }
    if( size !== 20 ){
      if( size > 100 ){ size = 100; }
      with_comment += ' size="' + size + '" ';
    }
    with_comment += '/> <input type="submit" value="Comment"/><br>';
  }else{
    with_comment = "";
  }
...
function filter_and_sort_menu( can_propose, title ){
  ...
  && PageBuilder.current.session.filter.indexOf( " #but " ) === -1
    ...
    propose_clause
    = '<br>or <input type="text" placeholder="new proposition" name="i3">'
...
// The private page of a persona
  ...
  if( list.length ){
    ...
    list.forEach( list, function( delegation ){
...
// The private page of a persona's delegations
  ...
  delegations.forEach( function( delegation ){
    ...
    that.push(
      ...
      " for " + duration_label( delegation.expire() - Kudo.now() ),
...
// propositions, filtered.
  ...
  if( !count ){
    if( this.session.filter === " #new " ){
      ...
      return this.redirect( tag_page ? "tags" : "propositions" );
    }
    if( this.session.filter === " #hot " ){
      this.session.set_filter( "#recent" );
      return this.redirect( tag_page ? "tags" : "propositions" );
    }
    if( this.session.filter === " #recent " ){
      this.session.set_filter( "+age" );
      return this.redirect( tag_page ? "tags" : "propositions" );
    }
  }
