...
// Return , separated list of tags extracted from filter
  ...
  .replace( /[+\-][a-z_]*/g, "" )
  .replace( / /g, "," ).replace( /#/g, "" );
  this.sort_criterias.forEach( function( criteria ){
    text += " " + criteria;
  });
  return text;
};
...
var input = l8.Task( function( question ){ this
  ...
  .step( function( req, res ){
    ...
    Session.current.timestamp = l8.now;
    PendingResponse = res;
...
// The private page of a persona
  ...
  votes.forEach( function( entity ){
    ...
    buf.push( '<br><br>'
      ...
      + ( entity.is_direct()
        ? ""
        :  "<dfn>(via " + link_to_persona_page( entity.delegation().agent ) + ")</dfn>" )
...
// propositions, filtered.
  ...
  // Parse filter
  if( filter ){
    filter = Array.prototype.slice.call( arguments, 1 ).join( " " );
  }
  ...
  buf.push( [
    ...
    '"if( this.value !== 0 ){ ',
      'this.form[0].value = \'change_proposition Search\';',
      'this.form.submit();',
    '}">',
      o( "sort" ),
      ...
      if( vote_entity ){
        buf.push( 'you: '
          ...
          + ( vote_entity.is_direct()
            ? ""
            :  "<dfn>(via " + link_to_persona_page( vote_entity.delegation().agent ) + ")</dfn>" )
  ...
  ||  ( tags.length  && !personas.length )
    ...
    for( topic_name in topics ){
      ...
      if( filter ){
        ...
        if( filter && topic.result.total() < 2 )continue;
    ...
    personas.forEach( function( persona ){
      ...
      if( orientation && orientation !== Vote.neutral ){
        ...
        || vote_value.privacy === Vote.private
          ...
          var delegation = Delegation.find( vote_value.delegation );
          var agent      = Persona.valid( delegation && delegation.agent );
          if( agent ){
            buf2.push( '<dfn>(via ' + link_to_persona_page( agent )
            + ')</dfn> ');
          }else if( vote_value.delegation !== Vote.direct ){
            buf2.push( '<dfn>(indirect)</dfn>' );
          }
  ...
  if( votes_log.length ){
    ...
    &&  last_vote_entity.privacy() === Vote.public
      ...
      if( last_vote_value.delegation !== Vote.direct ){
        var agent_label = last_vote_entity.agent_label();
        if( agent_label ){
          buf.push( ' <dfn>(via '
          ...
          buf.push( ' <dfn>(indirect)</dfn>' );
          if( Delegation.valid( last_vote_entity.delegation )
          &&  last_vote_entity.delegation.agent === persona
          ){
            buf.push( " (via you) ");
          }
        }
  ...
  votes.forEach( function( vote_value, index ){
    ...
    if( !idem && ( !gap || index >= votes.length - 200 ) ){
      ...
      &&  ( valid_vote && valid_vote.privacy() === Vote.public )
        ...
        if( vote_value.delegation !== Vote.direct ){
          if( valid_vote && ( agent_label = valid_vote.agent_label() ) ){
            persona_text += ' <dfn>(via '
            + link_to_page( "persona", agent_label )
            + ')</dfn>';
          }else{
            persona_text += ' <dfn>(indirect)</dfn>';
            if( Delegation.valid( valid_vote.delegation )
            &&  valid_vote.delegation.agent === persona
            ){
              buf.push( " (via you) ");
            }
          }
        ...
        if( vote_value.delegation !== Vote.direct ){
          if( valid_vote && ( agent_label = valid_vote.agent_label() ) ){
            persona_text += " via " + link_to_page( "persona", agent_label );
          }else{
            persona_text += ' <dfn>(indirect)</dfn>';
            if( Delegation.valid( valid_vote.delegation )
            &&  valid_vote.delegation.agent === persona
...
http_repl_commands.help = function(){
  var tmp = [
    ...
    "change_proposition verb text #tag text #tag... -- change proposition",
...
http_repl_commands.change_proposition = function( name, proposition_name ){
  ...
  // The form that leads here sometimes constains a default "sort" criteria
  if( text.indexOf( "sort " ) === 0 ){
    text = text.substring( "sort ".length );
  }
    
  // Search
  if( name === "Search" ){
    Session.current.set_filter( text || "all" );
    redirect_back( 1, text );
    return;
  }
