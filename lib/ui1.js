...
function encode_ref( ref ){
  ...
  return query ? path + "?" + querystring.escape( query ): path;
}
...
// Set HTTP response to 302 redirect, to redirect to specified page
  if( !PendingResponse )return;
  var query_style = true ? "?i=" : "";
  if( !page ){
    PendingResponse.redirect = query_style + "/";
  }else{
    PendingResponse.redirect = query_style + "/page/" + encode_ref( page );
  }
}
...
// propositions, filtered.
  ...
  Session.current.filter_tags.forEach( function( tag_entity ){
    tag_set.add( tag_entity.label );  
  });
  ...
  // Place holder for clickable list of tags
  var tag_set_index = buf.length;
  buf.push( "" );
  buf.push( '<hr>' );
  ...
  tag_set.sort( function( a, b ){
    ...
    // computed tags come first, tags with multiple propositions next, etc
    if( entity_a ){
      a = "zzzz" + a; 
