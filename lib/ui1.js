...
// This page let's visitor ask for results about tags by specified personas
  ...
  if( !tags.length && !personas.length && Session.current.has_filter() ){
    ...
    for( topic_name in topics ){
      ...
      topic.votes_log().forEach( function( vote_value ){
        ...
        if( vote_value.entity.expired() )return;
        var persona = vote_value.entity.persona;
        if( persona.expired() )return;
  ...
  tags.forEach( function( tag ){
    ...
    personas.forEach( function( persona ){
      ...
      var orientation = vote && !vote.expired() && vote.orientation();
...
// Focus on one proposition
  ...
  var author = comment ? comment.vote.persona : proposition.persona;
  if( author.expired() ){
    author = null;
  }
  if( comment && comment.expired() ){
...
http_repl_commands.change_proposition = function( name, proposition_name ){
  ...
  // Collect list of tags, inject user's name as first tag if "propose"
  var tags = is_tagging ? [] : [ "#"
  ...
  // If tagging or propose & not tags at all but some space, assume all tags
  if( is_tagging
  || ( tags.length ===  1 && text.indexOf( " " ) !== -1 )
  ){
    text = text.replace( /[A-Za-z][_0-9A-Za-z]*/g, function( tag ){
      // if( tag === "tag")return "";
      tags.push( "#" + tag );
