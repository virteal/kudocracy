...
function page( name ){
  ...
  var redir = Session.current.response.pushState;
  if( redir ){
    body = body
    + '\n<script>history.replaceState( null, "", "' + redir + '" );</script>';
    Session.current.response.pushState = null;
  }
  set_body( body );
}
...
function vote_menu( vote, proposition, orientation, with_twitter ){
  ...
  if( agents ){
    Ephemeral.each( agents, function( agent ){
      var orientation = agent.get_public_orientation_on( proposition );
      if( orientation === Vote.neutral )return;
      recommendations += " " + emoji( orientation ) + link_to_persona_page( agent );
    });
  }
  return [
...
function page_visitor( page_name, name, filter ){
// The private page of a persona
  ...
  var tag_set = push_title_and_search_form( buf, persona.label );
...
function page_persona( page_name, name,filter ){
...
function page_delegations( page_name, name, filter ){
// The private page of a persona's delegations
  ...
  buf.push( "<br><div><h2>Delegations</h2>" );
...
// This page let's visitor ask for results about tags by specified personas
  ...
  buf.push( [
    '\n<br><form name="ballots" method="POST" url="/">',
