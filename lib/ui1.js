...
function start_http_repl( kudo_scope ){
  ...
  if( dev_mode ){
    browserified = fs.readFileSync( 
      browserified_pathname = "browserified.js",
      "utf8"
    );
  }else{
    browserified = fs.readFileSync(
      browserified_pathname = "browserified.min.js.gz"
    );
  }
  ...
  function handler( req, res ){
    ...
    if( url.indexOf( "browserified" ) !== -1 ){
      if( !with_koa ){
        ...
        res.set( "Content-Encoding", dev_mode ? "identity" : "gzip" );
        res.sendfile( browserified_pathname );
      }else{
        res.writeHead( 200, {
...
var kudo_dir = main.module.filename.replace( "/lib/main.js", "" );

main.start( start_http_repl );
