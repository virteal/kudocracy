...
var HttpQueue = l8.queue( 1000 ); // in process_kudo_imports()
var input = l8.Task( function( question ){ this
  ...
  .step( function( req, res ){
    ...
    = req.post_query_data || url.parse( req.url, true).query;
    // Collect ?i=...&i2=...&i3... into space separated command + arg list
    var data = PendingResponse.query.i;
    var more;
    if( data ){
      more = PendingResponse.query.i2;
      if( more ){ data += " " + more; }
      more = PendingResponse.query.i3;
      if( more ){ data += " " + more; }
      more = PendingResponse.query.i4;
      if( more ){ data += " " + more; }
      more = PendingResponse.query.i5;
      if( more ){ data += " " + more; }
...
// Set HTTP response to 302 redirect, to redirect to specified page
  ...
  PendingResponse.redirect = "?i=page%20" + page;
}
...
function link_to_command( cmd ){
  ...
  return '<a href="?i=' + url_code + '">' + cmd + '</a>';
}
...
function page_header( left, center, right ){
  left = link_to_page( "index" ) + " " + link_to_page( "propositions" )
  + ( left ? " " + left : "" );
...
function page_index(){
  ...
  [
    ...
    '<input type="search" placeholder="all" name="i2" value="new"/>',
...
function vote_menu( vote, proposition, orientation ){
  ...
  if( with_comment && comment ){
    comment = encodeURIComponent( comment.substring( 0, remain ) );
  }else{
...
// The private page of a persona
  ...
  buf.push( [
    '\n<form name="proposition" url="/">',
    '<input type="hidden" name="i" value="page visitor ' + persona.label + '"/>',
    '<input type="search" placeholder="tags" name="i3" value="',
      Session.current.has_filter() ? Session.current.filter.trim().replace( /#/g, "" ) + " " : "",
    '"/>',
...
// This is the "public" aspect of a persona
  ...
  buf.push( [
    '\n<form name="proposition" url="/">',
    '<input type="hidden" name="i" value="page persona ' + persona.label + '"/>',
    ' <input type="search" placeholder="tags" name="i3" value="',
      Session.current.has_filter()
      ? Session.current.filter.trim().replace( /#/g, "" ) + " "
      : "",
    '"/>',
    ' <input type="submit" name="i2" value="Search"/>',
    '</form>\n'
...
// The private page of a persona's delegations
  ...
  buf.push( [
    ...
    '<input type="search" placeholder="tags" name="i3" value="',
      Session.current.has_filter()
      ? Session.current.filter.replace( /#/g, "" ) + " "
      : "",
    '"/>',
    ' <input type="submit" name="i2" value="Search"/>',
    '</form><br>\n'
...
// propositions, filtered.
  ...
  buf.push( [
    '\n<form name="proposition" url="/">',
    '<input type="hidden" name="i" value="change_proposition"/>',
    '<input type="search" placeholder="tags" name="i3" value="',
      Session.current.has_filter()
      ? Session.current.filter.trim().replace( /#/g, "" ) + " "
      : "",
    ...
    + ( Session.current.visitor
      && Session.current.has_filter()
      ? ' <input type="submit" name="i2" value="Delegate"/>' : "" )
    + (  Session.current.visitor
      && Session.current.has_filter()
  ...
  list.forEach( function( proposition ){
    ...
    proposition.tags_string().split( " " ).forEach( function( tag ){
      ...
      buf.push( link_to_page( page_name, tag ) + count );
    });
...
// This page let's visitor ask for results about tags by specified personas
  ...
  var r = [
    ...
    [ page_header(
      _,
      link_to_twitter_tags( "#vote #ballot" ),
      _
    ) ]
  ];
  var buf = [];

  buf.push( "<h2>Ballots</h2><br><br>" );
  
  // Query for names and tags
  query = Array.prototype.slice.call( arguments, 1 ).join( "" );
  var valid_query = "";
  var tags     = [];
  var personas = [];
  var tag_entity;
  ...
  query.replace( /[#A-Za-z0-9_]+/g, function( tag ){
    if( ( tag_entity = Topic.find( tag ) )
    && query.indexOf( "@" + tag ) === -1
  ...
  query.replace( /@[A-Za-z0-9_]+/g, function( name ){
  ...
  // If nothing, use filter to find propositions & voters
  if( !tags.length && !personas.length && Session.current.has_filter() ){
    
    var filter = Session.current.filter;
    var added_personas = {};
    ...
    for( topic_name in topics ){
      ...
      // Skip propositions with less than 2 votes, ie orphan/errors/noise
      if( topic.result.total() < 2 )continue;
      ...
      if( !topic.filtered( filter ) )continue;
  ...
  buf.push( [
    ...
    '<textarea name="i2" autofocus cols=40 rows="' + ( 4 + tags.length + personas.length )
    + '">\n',
  ...
  tags.forEach( function( tag ){
    ...
    personas.forEach( function( persona ){
      ...
      if( orientation && orientation !== Vote.neutral ){
        total++;
        buf2.push( '\n<br>'
          + link_to_page( "persona", persona.label, persona.label )
          + ' '
        );
...
// This is the votes page of the application, filtered.
  ...
  buf.push( [
    ...
    '<input type="search" placeholder="tags" name="i3" value="',
      ...
      : "",
    '"/>',
    ' <input type="submit" name="i2" value="Search"/><br>'
  ].join( "" ) );
...
// Focus on one proposition
  ...
  var comment = Comment.valid( proposition.comment() );
  ...
  if( comment || author ){
    ...
    + '</h3>'
    + ( author
    ? ' <dfn>(' + link_to_page( "persona", author.name, author.label ) + ')</dfn>'
    : "" )
    + '<br><br>' ) 
  }
  ...
  if( votes_log.length ){
    ...
    if( last_vote_entity
    &&  last_vote_persona
    &&  last_vote_entity.privacy() === "public"
    ){
      buf.push( ' <em>' + emojied( last_vote_entity.orientation() ) + '</em>' );
      buf.push( ' ' + link_to_page( "persona", last_vote_persona_label ) );
      if( last_vote_value.delegation !== Vote.direct ){
        var delegation_entity = Delegation.find( last_vote_value.delegation );
        var agent = delegation_entity && Persona.valid( delegation_entity.agent );
        if( agent ){
          buf.push( ' <dfn>(via '
            + link_to_page( agent.label, "persona" )
            + ')</dfn>'
          );
        }
      }
  ...
  votes.forEach( function( vote_value, index ){
    ...
    // Display only non expired non neutral non private direct votes
    if( vote_value.delegation          === Vote.direct
    && vote_value.privacy              !== Vote.private
    && vote_value.orientation          !== Vote.neutral
    && Vote.valid( vote_value.entity )
    && vote_value.entity.privacy()     !== Vote.private
    && vote_value.entity.orientation() !== Vote.neutral
    && Persona.valid( vote_value.entity.persona )
    && (!gap || index >= votes.length - 200 )
    ){
      count++;
      buf.push( "<br>" );
      var orientation
      = (vote_value.privacy === Vote.secret || vote_value.entity.privacy === Vote.secret )
      ? "secret"
...
http_repl_commands.change_delegation = function( delegation_entity, privacy, duration ){
...
http_repl_commands.change_proposition = function( name, proposition_name ){

  // Sanitize, extract tags, turn whole text into valid potential tag itself
  var text = Array.prototype.slice.call( arguments, name === "Tag" ? 2 : 1 ).join( " " );

  // Could be a search, a delegate or a propose coming from page_propositions
  // Or from page_proposition when visitor add tags
  ...
  // Propose new proposition or tag an existing proposition
  
  // Remove all tags that look like something from the current filter
  ...
  text = text.trim();
  if( !text && !is_tagging ){
    printnl( "Missing proposition" );
    return;
  }

  // Collect list of explicit #tags
  text = text.trim();
  var tags = [];
  ...
  // Add as tags words that are existing tags
  if( is_tagging
  || ( !hash_found && text.indexOf( " " ) !== -1 )
  ){
    text = text.replace( /[A-Za-z][_0-9A-Za-z]*/g, function( tag ){
      // if( tag === "tag")return "";
      if( Topic.find( "#" + tag ) ){
  ...
  // Add all tags from the current filter
  if( Session.current.has_filter() ){
    Session.current.filter_tags.forEach( function( tag ){
      tags.push( tag.name );
    })
  ...
  if( !is_tagging && text.length < 2 && tags.length ){
    if( ( text = tags.shift() ).length < 2 ){
    ...
    if( Topic.reserved( text ) ){
      printnl( "Cannot propose reserved tag " + text );
      return;
    }
  }
  ...
  if( !is_tagging && Topic.reserved( text ) ){
    printnl( "Not a valid proposition: " + text );
    return;
  }
  ...
  var proposition = Topic.find( text || ( is_tagging && proposition_name ) );
  if( !proposition ){
    changes.push( function(){
      Ephemeral.inject( "Topic", {
        label:   text,
