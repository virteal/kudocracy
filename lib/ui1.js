...
// Side effect:
//   Collect tags into session.filter_tags array.
...
// Set HTTP response to 302 redirect, to redirect to specified page
  if( !PendingResponse )return;
  if( !page ){ page = "index"; }
  // Change space sepator into . (nicer looking) and encode
  page = encodeURIComponent( page.replace( / /g, "." ) );
  PendingResponse.redirect = "?i=page." + page;
}
...
http_repl_commands.proposition_propose = function( text ){
  ...
  // Add all tags from the current filter, including those remover earlier
  if( Session.current.has_filter() ){
    Session.current.filter_tags.forEach( function( tag ){
      tags.push( tag.name );
    })
