...
function TwitterUser( twitter_user_data, persona ){
  ...
  if( id && !user ){
    ...
    && user.screen_name !== screen_name
      ...
      trace( "BUG, inconsistent screen_name & id, keep screen name" );
  ...
  if( user ){
    ...
    if( twitter_user_data.name ){
      ...
      user.time_twitter_user_data = now;
      changed = true;
    }
...
TwitterUser.prototype.collect_friends = function(){
  ...
  function reschedule( delay ){
    ...
    setTimeout( function(){ that.collect_friends(); }, delay );
    return false;
  }
  ...
  if( !this.id || !this.screen_name )return reschedule();
  
  // Don't do too often for same user, 5 minutes
  var age_collected = now - this.time_friends_collected;
  if( age_collected < 5 * 60 * 1000 )return reschedule();
  ...
  twit.get(
    ...
    function( err, data, response ){
      ...
      for( var ii = 0 ; ii < list.length ; ii++ ){
        ...
        // Skip if not part of the community
        if( !friend_user )continue;
        that.friends[ id ] = friend_user;
        // Detect new friend
        if( !old_friends[ id ] ){
...
// Data are saved in some disk based cache to respect Twitter rate limits
  ...
  for( var user_idx in AllTwitterUsers ){
    ...
    var user_data = {
      ...
      twitter_user_data : user.twitter_user_data,
    ...
    if( !changed && LastSavedData ){
      ...
      }else if( JSON.stringify( old.twitter_user_data )
        !== JSON.stringify( user.twitter_user_data )
      ){
        changed = true;
  ...
  ||    json.substring( json.indexOf( '"users":' ) ) 
    ...
    if( LastSavedJson ){
      for( var ii = json.indexOf( '"users":' ) ; ii < json.length ; ii++ ){
        if( json.substr( ii, 1 ) !== LastSavedJson.substring( ii, 1 ) ){
          var start = ii - 30;
          if( start < 0 ){
            start = 0;
          }
          trace( "JSON diff:", 
            "\n" + json.substring( start, 79 ),
            "\n" + LastSavedJson.substring( start, 79 )
          );
          break;
        }
      }
    ...
    trace( 
      "Twitter, save in", l8.update_now() -now, "ms, user count:", users.length
    );
...
TwitterUser.load = function(){
  ...
  for( var ii = 0 ; ii < users.length ; ii++ ){
    ...
    user.twitter_user_data = user_data.twitter_user_data;
    user.time_twitter_user_data = user_data.twitter_user_data;
    ...
    Object.keys( user_data.friends ).forEach( function( friend_id ){
      if( !friend_id ){
        trace( "BUG, bad friend_id in save()" );
        return;
      }
      user.friends[ friend_id ]
      = TwitterUser.register( { id: friend_id } );
      count_friendships++;
    } );
    Object.keys( user_data.followers ).forEach( function( follower_id ){
      if( !follower_id ){
        trace( "BUG, bad follower_id in save()" );
        return;
      }
      user.followers[ follower_id ]
      = TwitterUser.register( { id: follower_id } );
    } );
  }
  trace( 
    "File 'twitter_users.json', time to load:", l8.update_now() - now,
    "ms for", users.length, "users with", count_friendships, "friendships"
  );
};
...
// Get list of friend ids. This is first called for the user stream "preambule"
  ...
  if( start === 0 ){
    if( !this.kudocracy_list_queried ){
  ...
  var that = this;
  trace( "Twitter friends to process:", start, "until", friends.length );
