...
if( SaveScheduled )return;
  ...
  for( var user_idx in AllTwitterUsers ){
    ...
    list = new Array( user.friends_count );
    for( var friend_idx in friends ){
      var friend = friends[ friend_idx ];
      /*
      if( !friend.id ){
        trace( "BUG, missing friend.id in save()" );
        continue;
      }
      // CHeck validity
    ...
    user_data.friends = list;
    friendships_count += list.length;
  
    var followers = user.followers;
    list = new Array( user.followers_count );
