...
function TwitterUser( twitter_user_data, persona ){
  ...
  var id = twitter_user_data.id_str;
  var user;
  if( screen_name ){
...
TwitterUser.prototype.collect_friends = function(){
  ...
  function reschedule( delay ){
    ...
    if( TimeRateExcess ){
      delay = ( 15 + 5 * Math.random() ) * 60 * 60 * 1000;
    }
  ...
  if( age_busy < 10 * 1000 )return reschedule( 10 * 1000 );
  TimeFriendCollectorBusy = now;
  
  // Sleep if to much queries
  if( TimeRateExcess ){
   var age_excess = 0;
   age_excess = l8.update_now() - TimeRateExcess;
    if( age_excess < 15 * 60 * 60 * 1000 )return reschedule();
  }
  ...
  if( age_collected < 5 * 60 *0 )return reschedule();
...
// Data are saved in some disk based cache to respect Twitter rate limits
  ...
  for( var user_idx in AllTwitterUsers ){
    ...
    try{ 
      ...
      try{ 
        ...
        trace( "BUG? not json twitter data" );
      }
      if( !ok )continue;
    }
  ...
  var json = JSON.stringify( { meta: { time_saved: now }, users: users } );
  if(   json.substring( json.indexOf( "users:" ) ) 
    !== LastSavedData.substring( LastSavedData.indexOf( "users:" ) )
  ){
