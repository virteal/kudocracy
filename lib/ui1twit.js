...
var AllTwitterUsers = map();
var CachedListOfUsers = null; // AllTwittersUser stored as an array
var TwitterUsersByScreenName = map();
var TwitterFriendsByScreenName = map();
...
TwitterUser.prototype.get_followers = function( bool_community ){
  ...
  if( this.follower{s_count !== list.length ){
    trace( 
      "BUG? inconsistant number of followers.",
  ...
  if( twitter_data ){
    ...
    if( this.followers_count > nfollowers ){
      trace(
        ...
        this.followers_count, "vs", nfollowers
      );
...
TwitterUser.prototype.add_friend = function( twitter_user ){
  ...
  // Duplicate addition
  if( friends[ new_friend_id ] )return;
  ...
  if( twitter_user.followers[ this.id ] ){
    ...
    if( twitter_user.twitter_user_data ){
      ...
      {
        trace( 
          "BUG, local followers count greater thant twitter's for",
          "" + twitter_user + ",",
          twitter_user.twitter_user_data.followers_count
        );
        twitter_user.followers_count--;
      }
    }
...
var AllMonitoredPersonasById = map();
