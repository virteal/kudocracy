...
Proto.open_user_stream = function(){
  ...
  console.log( "Listen to Twitter CLI events for domain " + this.id );
  return this;
};
  ...
  if( start === 0 ){
    if( !this.kudocracy_list_queried ){
      ...
      this.twit.get(
        ...
        Proto.process_kudocracy_list_members_response.bind( that, event )
      );
      return;
    }
  }
...
Proto.process_kudocracy_list_members_response
= function( friends_event, err, data, response ){
  var users = [];
  if( err ){
    if( err.code !== 34 )return this.process_error_response( err );
    users = this.friends;
    // When there are more than 2500 friends, a "kudocracy" list is mandatory
    if( users.length > 2500 ){
      trace(
...
Proto.process_error_response = function( err ){
...
exports.start = function( ui1_server ){
  ...
  Ephemeral.each( Persona.all, function( persona ){
    ...
    if( !persona_topic ){
      console.warn( "Twitter, missing persona topic for " + domain );
      return;
    };
    var domain = persona_topic.get_data( "domain" );
    if( !domain ){
      console.warn( "Twitter, no 'domain' data for domain " + domain );
      return;
    }
    if( !domain.twitter_consumer_key ){
      console.warn( "Twitter, no consumer key for domain " + domain );
      return;
    };
    trace( "Twitter, new monitored persona " + domain );
