...
TwitterUser.prototype.add_friend = function( twitter_user ){
  ...
  var all_friends = this.all_friends;
  all_friends[ twitter_user.id ] = twitter_user;
};

var TimeRateExcess = 0;

var FriendCollectorIsBusy = false;
...
TwitterUser.prototype.collect_friends = function(){
  ...
  function reschedule(){
    setTimeout( 
      ...
      60 * 1000 * ( 15 + 2 * Math.random() )
    );
    return false;
  }
  
  // Only one collect at a time
  if( FriendCollectorIsBusy )return reschedule();
  ...
  twit.get(
    ...
    function( err, data, response ){
      if( err ){
        if( err.code === 88 ){
          if( !TimeRateExcess ){
            console.warn( "Twitter, rate excess collecting friends", err );
          }
      ...
      // Process list of ids
      trace( "ids received in collect_friends()" );
      TimeRateExcess = 0;
...
// Data are saved in some disk based cache to respect Twitter rate limits
  var age = l8.update_now() - TimeLastSaved;
  var period = 10 * 1000;
  if( age < period ){
  ...
  for( var user_idx in AllTwitterUsers ){
    ...
    var user_data = {
      ...
      time_last_change: user.time_last_change,
  ...
  if( json !== LastSavedData ){
    trace( "Twitter, save, user count:", users.length );
