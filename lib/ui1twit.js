...
TwitterUser.prototype.collect_friends = function(){
  var that = this;
  if( TimeRateExcess 
  && ( l8.update_now() - TimeRateExcess ) < 10 * 1000 
  ){
    // Try later
    setTimeout( 
      function(){
      ...
      60 * 1000 * ( 1 + 2 * Math.random() )
    );
    return;
  }
  // Don't try until profile is filled
  if( !this.id || !this.screen_name )return;
  // Don't do too often
  var now = l8.update_now();
  var delay = now - this.time_friends_collected;
  if( delay < 5 * 60 * 1000 )return;
  var twit =  AllMonitoredPersonas[ 0 ].twit;
  twit.get( "friends/ids", function( data ){
    if( !data ){
      trace( "Missing data in .collect_friends()" );
      return;
    }
    var err = data.code || data.errno;
    if( err ){
      if( err === 88 ){
        if( !TimeRateExcess ){
...
// Get list of friend ids. This is first called for the user stream "preambule"
  ...
  friends_slice.forEach( function( id ){
    var user = TwitterUser.register( { id: id } );
...
MonitoredPersonaProto.process_tweet = function( event ){
  ...
  if( !for_cli ){
    nde&&debug( "Twitter, tweet from " + twitter_user, "text:", event.text );
    return;
  }
