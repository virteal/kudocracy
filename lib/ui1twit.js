...
TwitterUser.prototype.deregister = function(){
// Called when user is removed by twitter or leaves the community
  ...
  // TODO: fix this
  if( CommunityUser.has_friend( this ) ){
    trace( "Attempt to deregister community member " + this );
...
TwitterUser.prototype.add_friend = function( twitter_user ){
  ...
  if( followers[ this.id ] ){
    ...
    twitter_user.cached_follower_ids;
  }
  ...
  // Simplify bidirectionnal friendships, keep unidirectional
  if( false && twitter_user.friends[ this.id ] ){
    // trace( "Bidirectional " + this + " <-> " + twitter_user );
    var this_count = this.followers_count;
    var other_count = twitter_user.followers_count;
    if( this.twitter_user_data ){
      this_count = this.twitter_user_data.followers_count;
    }
    if( twitter_user.twitter_user_data ){
      other_count = twitter_user.twitter_user_data.followers_count;
    }
    if( this_count >= other_count ){
      this.remove_friend( twitter_user );
    }else{
      twitter_user.remove_friend( this );
    }
  }
...
TwitterUser.prototype.remove_friend = function( twitter_user ){
  ...
  if( twitter_user.is_member && this === TwitterUser.get_community() ){
    trace( "Attempt to remove a community member " + twitter_user );
...
TwitterUser.prototype.collect_friends = function( dont_schedule ){
  ...
  if( !dont_schedule ){
    ...
    for( var ii = 0 ; ii < list.length ; ii++ ){
      ...
      var was_scheduled = candidate.collect_scheduled;
  ...
  twit.get( "friends/ids", params,
    ...
    function( err, data, response ){
      ...
      for( var ii = 0 ; ii < list.length ; ii++ ){
        ...
        if( !friend_user && !that.is_community() )continue;
        that.add_friend( friend_user );
        new_friends[ id ] = friend_user;
        // Detect new friend
        if( !old_friends[ id ] ){
      ...
      for( id in old_friends ){
        var friend = new_friends[ id ];
        if( friend )continue;
        friend = AllTwitterUsers[ id ];
        removed_friends.push( friend );
        // trace( "Removed friend " + friend, "of " + that );
        that.remove_friend( friend );
      }
      
      // Check inconsistency, friends
      if( that.twitter_user_data
      &&  that.twitter_user_data.friends_count < that.friends_count 
      ){
        trace( 
          "BUG? inconsistant friends count of " + that,
          "is", that.friends_count,
          "vs twitter's", that.twitter_user_data.friends_count
        );
      }

      // Check inconsistency, followers
      if( that.twitter_user_data
      &&  that.twitter_user_data.followers_count < that.followers_count 
      ){
        trace( 
          "BUG? inconsistant followers count of " + that,
          "is", that.followers_count,
          "vs twitter's", that.twitter_user_data.followers_count
        );
      }
      
      that.time_friends_collected = l8.update_now();
      trace( 
        "Done collecting friends of " + that + ".",
        that.friends_count, "friends,",
  ...
  twit.get( "followers/ids", params,
    ...
    function( err, data, response ){
      ...
      for( var ii = 0 ; ii < list.length ; ii++ ){
        ...
        follower_user.add_friend( that );
        new_followers[ id ] = follower_user;
        // Detect new follower
        if( !old_followers[ id ] ){
...
// Called at startup to restart using data from disk cache
  ...
  for( ii = 0 ; ii < users.length ; ii++ ){
    ...
    if( user.twitter_user_date && user.twitter_user_data.name ){
