...
TwitterUser.prototype.get_followers = function(){
  var list = [];
...
TwitterUser.prototype.collect_friends = function(){
  ...
  // Don't do too often for same user, twice a day at most
  var age_collected = now - this.time_friends_collected;
  if( age_collected < 12 * 60 * 60 * 1000 )return reschedule();
...
// Data are saved in some disk based cache to respect Twitter rate limits
  ...
  if( BacklogSize ){
    trace( "Still", BacklogSize, "users in twitter backlog" );
  }
  if( TimeRateExcess ){
