...
var AllTwitterUsers = {};
var CachedListOfUsers = null; // AllTwittersUser stored as an array
...
function TwitterUser( twitter_user_data, persona ){
  ...
  if( twitter_user_data.name ){
    ...
    this.time_twitter_user_data = now;
  }else{
...
TwitterUser.prototype.get_followers = function( bool_community ){
  ...
  return this.cached_followers = list;
};
...
TwitterUser.prototype.add_friend = function( twitter_user ){
  ...
  if( !friends[ new_friend_id ] ){
    ...
    if( twitter_user.followers[ this.id ] ){
      trace( 
        "BUG? follower issue while adding friend " + twitter_user,
        "to " + this
      );
    }else{
...
TwitterUser.prototype.collect_friends = function( dont_schedule ){
  ...
  if( age_collected < 7 * 24 * 3600 * 1000 ){
  ...
  // If there exists a user with more followers, update it first
  var list = TwitterUser.get_all();
  list.sort( function( a, b ){
    var a_count = a.get_followers().length;
    var b_count = b.get_followers().length;
  ...
  for( var ii = 0 ; ii < list.length ; ii++ ){
    ...
    var done = candidate.collect_friends( true /* don't reschedule */ );
    if( done ){
      trace( 
        "Prioriy update of " + candidate,
...
// Data are saved in some disk based cache to respect Twitter rate limits
  ...
  for( var user_idx in AllTwitterUsers ){
    ...
    var user_data = {
      ...
      twitter_user_data : {}, // not full user.twitter_user_data,
      time_twitter_user_data: user.time_twitter_user_data,
      friends: [],
      followers: []
    };
    
    // Cache some data from the user profile
  ...
  ||    json.substring( json.indexOf( '"users":' ) ) 
    ...
    if( Kudo.TrustActor ){
      Kudo.TrustActor.get_ranked();
    }
    trace(
      "Twitter, save in", now2 -now, "ms,",
      " user count:", users.length,
      " ranking in", l8.update_now() - now2, "ms"
    );
  }
...
TwitterUser.load = function(){
  ...
  for( var ii = 0 ; ii < users.length ; ii++ ){
    ...
    user_data.followers.forEach( function( follower_id ){
      if( !follower_id ){
        trace( "BUG, bad follower_id in load()" );
        return;
      }
  ...
  if( count_friendships !== count_followships ){
    trace( "BUG, followships not equal to friendships", count_followships );
  }
};
