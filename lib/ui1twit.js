...
TwitterUser.deregister = function(){
// Called when user is removed by twitter
...
TwitterUser.prototype.collect_friends = function( dont_schedule ){
  ...
  twit.get( "followers/ids", params,
    ...
    function( err, data, response ){
      ...
      // Process list of ids
      trace( "followers ids received in collect_friends() for user " + that );
      TimeRateExcess = 0;
...
MonitoredPersonaProto.process_tweet = function( event ){
  ...
  if( !twitter_user ){
    ...
    if( twitter_user ){
      ...
      twitter_user = TwitterUser.register( event.user );
      trace(
        "BUG? twitter tweet from unknow (new?) user:",
        from, "text:",
        event.text
      );
    }
