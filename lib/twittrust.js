...
TrustActor.get_ranked = function( max, depth ){
  ...
  for( var ii = 0 ; ii < list.length ; ii++ ){
    ...
    if( user.twitter_user.screen_name === "suvranu" )continue;
    // Skip inactive users
    if( !user.twitter_user.is_active() )continue;
    // Skip actors with too few tweets
    if( user.twitter_user.twitter_user_data.statuses_count < 500 )continue;
    ...
    if(
    // Skip actors with very few subscribers
      user.twitter_user.followers_count > 30
    ){
      if( user.twitter_user.twitter_user_data.followers_count 
      < user.twitter_user.followers_count
      ){
        trace( 
          "BUG, more users than twitter reports for " + user + ",",
          user.twitter_user.followers_count,
          "vs", user.twitter_user.twitter_user_data.followers_count 
        );
      }
      // Skip if less than 30 followers
      if( user.twitter_user.twitter_user_data.followers_count >= 30 ){
  ...
  list.sort( function( a, b ){
     var a_score 
     = a.followers.length 
     * a.twitter_user.locality_factor
     * a.action_rate;
     var b_score 
     = b.followers.length 
     * b.twitter_user.locality_factor
     * a.action_rate;
