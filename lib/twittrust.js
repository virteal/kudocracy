...
TrustActor.pagerank = function(){
  ...
  // Or else compute it
  if( TimeCachedPagerank < TwitterUser.get_time_last_change()
  && !PagerankIsRunning
  ){
    ...
    setTimeout( function(){
      ...
      for( ii = 0 ; ii < actors.length ; ii++ ){
        actor = actors[ ii ];
        actor.pagerank_score = rank[ actor.id ];
      }
      
      PagerankIsRunning = false;
  
    }, actors.length * 3 + 1000 );
  }
...
TrustActor.get_ranked = function( max, depth ){
  ...
  for( var ii = 0 ; ii < list.length ; ii++ ){
    ...
    // Skip actors with not enough followers
