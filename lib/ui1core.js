 ...
 *    ToDo: use a cookie, a secure one. ip addresses are shared behind proxies
...
function kudo_signal_can_script(){
...
// client
  if( window.kudo_ctx
  && window.kudo_ctx.can_script
  & window.kudo_ctx.can_script !== "init"
  )return;
  var cookie = "kudo_can_script=true"; // session cookie, no max-age
...
// Client side setup, dynamic, different for each page
  ...
  function kudo_init_ctx( ctx, time_built, debug_mode ){
    kudo_ctx = ctx;
    kudo_time_built = time_built;
    kudo_time_offset = 0;
    kudo_touch_device = !!( "createTouch" in document );
    de = debug_mode;
    nde = false;
    bugC = function(){ console.log.apply( console, arguments ); }
    try{
      history.replaceState( kudo_ctx.url, kudo_ctx.title, kudo_ctx.url );
    }catch( err ){
      console.log( "Kudocracy: can't use history.replaceState()", err, err.stack );
      kudo_ctx.can_history = false;
    }
    kudo_ready = true;
  }
  ...
  $(document).ready( window.sw_when_ready = function sw_when_ready(){
    ...
    if( !window.kudo_ctx.is_slim ){
      $( "form" ).submit( function( e  ){
        if( !window.sw_magic_loader )return true;
    ...
    if( window.kudo_ctx.changes && window.kudo_ctx.changes.length ){
...
function page_style( title ){
  ...
  buf.push(
    ...
    session.htmlScript( once( onloadScript ) )
  );
...
function page_footer(){
  ...
  if( Session.current.can_script && Session.current.needs_twitter ){
...
function page_ballot2( /* pagename, ...query */ ){
  ...
  if( date && date.getTime() !== date.getTime() ){
    this.push( "<em>", i18n( "date"), "<em>" );
    date = null;
  }
  if( date2 && date2.getTime() !== date.getTime() ){
    this.push( "<em>", i18n( "date"), "<em>" );
    date2 = null;
  }
...
function page_delegates( page_name, optional_proposition_name ){
  ...
  Session.current.can_script && this.push(
    '<script type="text/javascript">'
...
// Focus on one proposition
  ...
  // Pie graph
  if( Session.current.can_script && proposition.result.total() ){
  ...
  if( type ){
    if( type === "Session" ){
      ...
      if( Session.current.visitor ){
        entities.push( Session.current.visitor.json_value() );
      }
    }else{
      var type_name = type;
