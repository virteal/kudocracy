...
ProtoBuilder.push_title = function( title, not_compact ){
  ...
  if( title[0] === "@" ){
    ...
    nuit_debout && buf.push( ", une personne/organisation" );
  }else{ 
...
function format_change( change, page_name, twitter_style, array, at_tail ){
  ...
  function show_proposition(){
    ...
    if( twitter_style ){
      ...
      + ( persona 
        ? link_to_persona_page( persona )
          + ( nuit_debout ? ", une personne/organisation" : "" )
        : link_to_proposition_page( p, icon( "zoom-in") )
          + ( nuit_debout ? ", une proposition" : "" )
      ) +  '</dfn>';
    }else{
      msg += persona 
      ? link_to_persona_page( persona )
...
function page_index( page_name ){
  ...
  Ephemeral.each( list, function( proposition, index ){
    ...
    that.push(
      ...
      link_to_page( "proposition", proposition.label ),
      " ", 
      !nuit_debout && link_to_wiki( proposition.label ),
      proposition.is_persona()
      && ( '<dfn>(' 
        + link_to_persona_page( proposition.get_persona() )
        + ")</dfn> "
...
// Focus on one proposition
  ...
  if( persona ){
    nuit_debout && buf.push( "<br>Il s'agit du nom d'une personne/organisation : " );
    buf.push( ' ' + link_to_persona_page( persona ) );
    nuit_debout && buf.push( "<br>Elle dispose d'une page sur twitter : " );
    buf.push( ' ', link_to_twitter_tag( proposition.label, false /* title */ ) );
    nuit_debout && buf.push( " et d'une page dans le wiki : " );
    buf.push( ' ', link_to_wiki_icon( persona.label ) );
  }
  ...
  if( !persona ){
    ...
    buf.push( ' ', link_to_wiki_icon( proposition.label ) );
  }
  if( author || comment ){
    buf.br();
  }
  ...
  if( tmp = proposition.persona() ){
    !nuit_debout && buf.push( l( "by" ), " ", link_to_persona_page( tmp ) );
  }
  ...
  tmp.split( " " ).forEach( function( tag ){
    ...
    if( comment ){
      if( comment[0] === "@" ){
        ...
        nuit_debout && ( comment += ", une personne/organisation" );
      }else{
        comment = l( comment );
      }
      buf.push( comment );
