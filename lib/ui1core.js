  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    if( ctx.changes ){
      ...
      var process_changes = function(){
        ...
        ui_engine.login(); // Cannot login actual visitor, not in database yet
  ...
  function twitter_redirect( firebase, redirected, name ){
    ...
    function process_result( auth_data ){
      if( auth_data ) {
        ...
        if( !same_name ){
          console.log( 
            "Firebase. Authentic user", username, "is not visitor", name
          );
          // ToDo: redirect to ?page=login maybe?
        }
        // Signal server the authenticated user
        $.ajax({
          url: "?i=authentic/" + username + "/" + name
        });
        // ToDo: should get back to previous page
...
http_repl_commands.authentic = function( auth_name, name ){
  ...
  name      = ( "@" + name ).replace( /@@/g, "@" );
  // For demo purposes, login as if authentic when I login with any name
  if( auth_name === "@Kudocracy" || auth_name === "@jhr" ){
    if( Session.current.visitor ){
