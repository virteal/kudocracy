...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    // Provide more visual feedbacks on touch devices or when using ajax
    if( kudo_ctx.touch_device || !window.ui1_server ){
      $("#progress_bar").show();
    }
...
// options: with_twitter
  ...
  var buf = [
    ...
    i18n( "or vote" ), '<h2>',
    ( orientation !== Vote.agree    ? "</h2>, <h2>" + link_to_command( "change_vote " + vote_id + " agree",    emoji( "agree" ), "agree" ) : "" ),
    ( orientation !== Vote.disagree ? "</h2>, <h2>" + link_to_command( "change_vote " + vote_id + " disagree", emoji( "disagree" ), "disagree" ) : "" ),
    ( orientation !== Vote.neutral
      && ( !vote || vote.delegation() === Vote.direct ) // neutral triggers delegations
      ? "</h2>, <h2>" + link_to_command( "change_vote " + vote_id + " neutral",  emoji( "neutral" ), "neutral" ) : ""
    ),
    "</h2> ",
    i18n( "or" )
  ];
  ...
  function select( buf, more, tags ){
    ...
    buf.push(
      ...
      '"if( this.value !== 0 ){ ',
        ...
        '$(this.form).submit();',
      '}">',
     '<option dir="rtl">', i18n( more ? "more" : "less" ), '</option>'
    );
