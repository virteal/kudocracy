...
// this is the entry point for all html pages. It's a kind of router.
  ...
  // domain if it is not started already. Whereas &domain= detected in
  ...
  if( !f ){
    ...
    if( args.length === 1 ){
      ...
      // Don't do it if &domain= was in the query, it was done already
...
function link_to_page( page, value, title, anchor ){
  ...
  return '<a href="?page=' + page + url_code + '">' + adjusted_title + '</a>';
...
// options: with_twitter, compact
  ...
  if( options && options.with_twitter && orientation ){
    ...
    */
    ...
    permalink += "&domain=" + session.domain_label();
    buf.push(
...
// The private page of a persona
  ...
  if( persona.is_domain() ){
    ...
    this.push(
      ...
      '&domain=', persona.label.substring( 1 ),
      '">', persona.label, '</a></h2>'
    );
...
// propositions, filtered.
  ...
  if( !count ){
    if( invalid_count ){
      msg1 += l( "empty" ) + " " + sort_label( true, true ) + ". ";
  ...
  Ephemeral.each( tags, function( tag ){
    ...
    if( total ){
      ...
      that.push(
        ...
        icon( "disagree" ), " ", l( "against" ), sp,
        "" + count_against, percent, "" + ratio_against, end_percent, ". ", 
...
// This is the votes page of the application, filtered.
  ...
  if( valid_votes.length ){
    ...
    if( comments_only ){
      ...
      this.push(
        link_to_page( "votes", "comments", l( "comments" ), "votes" )
      );
    }
...
// It is the .login() command to redirects here.
  ...
  // Client side
    ...
    if( iframe ){
      ...
      url = "/twitter2";
      window.open( url, "kudocracy login" );
      return;
    }
...
function page_propose( page_name ){
  ...
  this.push(
    ...
    '<br><label>', l( "new&nbsp;proposition" ), '</label><br>',
...
function page_badges(){
  ...
  this.set( page_style( "badges" ), page_header() );
  ...
  .open_div( "badges_generator" )
    ...
    .push(
      ...
      '<br><textarea id="code" cols="50" spellcheck=false>...</textarea>'
    ).br()
  ...
  this.push( page_footer( true /* framed */ ) );
...
function page_badge( /* page_name, etc */ ){
  ...
  this.set( page_style( page_name ) );
  var cmd = "badge_vote " + proposition_name + " ";
  var domain = session.domain_label();
  
  this.open_div( "badge")
  .push(
    '<a target="_blank" id="badge-domain" href="main&domain=', domain,
    '" title="', Config.host, " ", domain, '">',
    Config.img_icon, 
    "</a> ",
    '<a target="_blank" href="/proposition/',
    Wiki.htmlizeAttr( l( proposition_name ) ),
    "?domain=", domain,
...
function page_index( page_name ){
  ...
  var twitter_name = domain || Config.domain;
  ...
  if( !is_main ){
    ...
    this.set_body(
      ...
      ' ', titled( link_to( "propositions" ), l( "Search" ) ),
      ...
      ? link_to_page( "visitor", "", avatar( authentic_twitter_id, 32 ) )
        + titled( link_to( "signout" ), l( "sign out" ) )
      : titled( link_to( "login" ), l( "login" ) ),
      
      l8.client ? "" : ' - '
        + titled( link_to( "main", "light version", true ), l( "mobile" ) ),
      ' - ', titled( link_to( "help" ), l( "help" ) ),
  ...
  if( !count ){
    ...
    msg1 
    += " - " + icon( "zoom-in" ) 
    +  " "   + link_to_page( "proposition", "", l( "details" ) )
    +  " "   + icon( "delegations" )
    +  " "   + link_to_page( "delegates", "", l( "delegates" ) );
  }
...
function page_help(){
  ...
  if( this.session.lang !== "fr" ){
    msg.push(
      ...
      'success ',
      '(<a href="http://kudocracy.com/proposition/kudocracy?domain=kudocracy">',
      'vote</a> #kudocracy !), ',
...
// Focus on one proposition
  ...
  var page_name = arguments[ 0 ];
  var proposition_name = arguments[ 1 ];
  var display_what = arguments[ 2 ] || "comments";
  ...
  this.set(
    ...
    page_header(
      titled(
        link_to_page( "delegates", proposition.label, icon( "delegates" ) ),
        l( "delegates" )
      ) + " " 
      + titled(
        link_to_page( "ballot2", proposition.label, icon( "ballot" ) ),
        l( "ballot" )
      ),
  ...
  // Badge
  this.push( " ", link_to_page( "badges", "", l( "badge") ) );
  ...
  if( proposition.half_life() ){
    buf.push( "<br>",
      l( "end in" ), " ", duration_label( proposition.expire() - Kudo.now() )
    );
  }
  ...
  tmp.split( " " ).forEach( function( tag ){
    ...
    buf.push(
      '<tr><td>',
      link_to_page( "propositions", tag, l( tag ) ),
      count,
      '</td><td>'
    );
