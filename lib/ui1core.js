...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    if( new_location ){
      console.log( "Set new location", new_location );
...
function page_login( page_name ){
  ...
  // Query for name
  var auth = "";
  if( l8.server && session.can_script ){
  ...
  if( auth ){
    this.push(
      '\n<script>$(function(){',
      'console.log( "install checkbox handler for twitter" );',
...
function page_index( pagename ){
  ...
  function reload_if_cached( time_cached ){
    ...
    if( age > 2 * 60 * 1000 ){
      ...
      window.location.replace( "/?page=kudocracy" );
    }
  }
