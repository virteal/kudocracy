...
var set = ui.set = raw_object_maker();
...
function page_index( page_name ){
  ...
  if( !is_main ){
    ...
    if( valid_machines.length > 1 ){
      ...
      var after_menu = ui.avatar( domain || Config.domain, 48 ) + " ";
      menu_list.push(
        '\n<form id="domain" url="/">',
  ...
  if( is_main ){
    ...
    this.set( ui.page_style( "main" ), page_header_left() );
...
// Note: port/host are "local"/document.domain on the client side.
  ...
  require( "./pagetwittrust.js" ).start( kudo_scope );
