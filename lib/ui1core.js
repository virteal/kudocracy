...
// this is the entry point for all html pages. It's a kind of router.
  ...
  if( !f ){
    ...
    var is_domain = false;
    if( !session.request.kudo_query.domain ){
...
// 'text' is some text to add to build the target url.
  ...
  if( page_name === "delegates" && n === 1 ){ n = 2; }
  ...
  if( page_name === "votes" && n === 1 ){ n = 2; }

  // When going back...
  if( page_name === "visitor" && n === 1 ){ n = 2; }
  
  // When going back...
  if( page_name === "persona" && n === 1 ){ n = 3; }
...
function link_to_wiki( page, title, no_icon, no_twitter ){
  ...
  if( page[ 0 ] === "#" ){
    ...
    encoded_page = domain + "on_" + twitter_tag;
  ...
  if( !no_twitter ){
    if( twitter_tag ){
      r += " " + link_to_twitter_tag( twitter_tag, icon( "twitter" ) );
    }
    if( twitter_user ){
      r += " " + link_to_twitter_user( twitter_user, icon( "twitter") );
    }
  }
  return r;
}
  ...
  if( is_index ){
    ...
    if( !adjusted_title ){
      ...
      if( Session.current.domain ){
        adjusted_title
        += " <em>"
        + link_to_wiki( "HomePage", Session.current.domain, false, true )
        + "</em>"; 
      }
    }
  }
...
// client side
  ...
  function hide_it(){
    $(this).parent().addClass( 'was_hidden' ).hide();
    $(".show_button").show();
  }
...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    if( path && path !== current_path && path.indexOf( ":" ) !== -1 ){
  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    // Make header fixed unless it spans multiple lines and eats too much space
    ...
    document.title = ctx.title || "kudocracy";
    ...
    if( window.twttr && window.twttr.widgets ){
      ...
      if( twitter_buttons ){
        console.log( "Twitter rendering" );
        window.twttr.widgets.load( twitter_buttons );
        console.log( "Twitter rendering done" );
     }else{
...
function page_header( left, center, right, current_page ){
  ...
  if( visitor ){
    ...
    + titled( link_to_page( "signout" ), i18n( "Sign out" ) );
  }else{
    right = ( ( right && ( right + " " ) ) || "" )
    + titled( link_to_page( "login" ), i18n( "Login" ) );
  }
  ...
  if( recent_events_div( current_page, true ) ){
    container_style = "margin:auto; font-family:inherit;";
  }
  ...
  var domain_owner = "@" + ( session.domain || Config.domain );
  var owner_avatar = avatar( domain_owner, 26 );
  var owner_link = link_to_twitter_user( domain_owner, owner_avatar );
  var home_link = link_to_page( home, "", "home" );
  if( home === "main" ){
...
// Header with 'propositions', 'delegates', 'votes', 'ballot' ... login help
  ...
  if( !Session.current.too_much_propositions() ){
    if( current_page !== "votes" ){
      m += " " + titled( link_to_page( "votes" ), i18n( "Votes" ) );
    }
    if( false && current_page !== "ballot" ){
      m += " " + titled( link_to_page( "ballot" ), i18n( "Ballot" ) );
    }
  }
...
// options: with_twitter, compact
  ...
  var recommendations = compact ? "" : proposition_recommendations({
  ...
  if( len ){
    if( vote && session.is_novice ){
      recommendation_msg.push( i18n( "recommendations:" ), " " );
    }
  ...
  var buf = [
    ...
    : link_to_page(
      "delegates",
      proposition.label,
      compact ? icon( "indirect" ) : "Delegate"
    ),
...
function sort_label( style, no_by ){
  var label = "";
  Session.current.sort_criterias.forEach( function( c ){
    ...
    var i18n_label = sort_labels[ Session.current.lang + " " + c ];
    if( !i18n ){
      i18n_label = i18n( c );
    }
    if( i18n_label ){
...
function sort_menu( title, dont_hide ){
  var hide;
...
// to use to show/hide some propositions.
  ...
  var hide        = session.can_script && options.hide;
  ...
  function o2( v, l, m ){
    ...
    var k2 = ( reversed ? "-" : "+" ) + v;
    var k3 = ( reversed ? "+" : "-" ) + v;
    var label1 = o( k2, l ) + o( k3, " ______ " + more );
  ...
  // Search button
  if( !hide && options.float ){ r.push( "<br>" ); }
  if( with_filter ){
    r.push(
  ...
  if( can_sort ){
    ...
    r.push(
      sort_criteria === "-voters_today"
      ? "<h3>" + i18n( "24 hours" ) + "</h3>"
      : link_to_command( 
        "proposition_action Search -voters_today", i18n( "24 hours" ) 
      ), " ",
      sort_criteria === "-voters_this_week"
      ? "<h3>" + i18n( "7 days" ) + "</h3>"
      : link_to_command( 
        "proposition_action Search -voters_this_week", i18n( "7 days" ) 
      ), " ",
      sort_criteria === "-voters_this_month"
      ? "<h3>" + i18n( "a month" ) + "</h3>"
      : link_to_command( 
        "proposition_action Search -voters_this_month", i18n( "a month" ) 
      ), " ",
      sort_criteria === "-voters_this_year"
      ? "<h3>" + i18n( "a year" ) + "</h3>"
      : link_to_command( 
        "proposition_action Search -voters_this_year", i18n( "a year" ) 
      ), " ",
      ...
      o2( "-voters_today",       "votes today" ),
      o2( "-voters_this_week",   "votes this week" ),
      o2( "-voters_this_month",  "votes this month" ),
      o2( "-voters_this_year",   "votes this year" ),
...
function filter_change_links( tag_set, dont_hide ){
  ...
  // Special "force" value forces hidding. In "main" page for example
  if( dont_hide === "force" ){
    hide = true;
  }
...
PageBuilder.prototype.push_title = function( title, not_compact ){
  ...
  if( filter_label ){
    ...
    if( comment ){
      ...
      buf.push( wikify_comment( comment.text ), '<br>' );
    }else{
      buf.push( '<br>' );
    }
  }else{
...
PageBuilder.prototype.push_title_and_search_form = function( title, hide ){
  ...
  buf.push( '', '</div></div>' ); // ToDo: why ''? without it the <div> is erased...???
...
PageBuilder.prototype.push_delegations = function( persona, br, misc ){ 
  ...
  if( persona === visitor ){
    this.push(
      icon( "indirect" ), " ", i18n( "Your delegations" ), '</h2>',
    ...
    this.push(
      icon( "indirect" ), " ", i18n( "Delegations" ),
      " ",
  ...
  var index = -1;
  var div = [
    '<div class="even"><div class="hide_button"></div>',
    '<div class="odd"><div class="hide_button"></div>'
  ];
...
function format_change( change, page_name, twitter_style, array, at_tail ){
  ...
  function show_proposition(){
    ...
    + "</dfn>";
    if( !twitter_style ){
      msg += " " + link_to_twitter_tag( p.label, icon( "twitter" ) );
    }
    if( persona && persona.is_domain() ){
      msg += ' <a href="/' + persona.label.substring( 1 ) + '">'
      + Config.img_icon
      + '</a>'
    }
  ...
  if( type === "Comment" ){
    msg += icon( "comment" ) + " ";
    show_proposition();
    msg += '<br>' + wikify_comment( params.text );
    if( proposition.get_comment_author() === persona ){
      RE_seen_comments[ proposition.id ] = true;
    }
    ...
    if( params.comment_text ){
      msg += '<br>' + wikify_comment( params.comment_text );
      if( proposition.get_comment_author() === persona ){
        RE_seen_comments[ proposition.id ] = true;
      }
    }
    ...
    var sub_msg = link_to_page( "propositions", str_tags, str_tags )
    + " " + link_to_delegation_page( delegation );
    ...
    &&  delegation.agent !== session.visitor
      ...
      + link_to_persona_page( delegation.agent, i18n( "delegate too") )
      // + '</div>
      + '</td></tr></table>';
    }else{
      msg += sub_msg;
  ...
  if( proposition && !RE_seen_comments[ proposition.id ] ){
    RE_seen_comments[ proposition.id ] = true;
    var comment = proposition.get_comment_text( proposition );
    if( comment ){
      msg += '<br>' + wikify_comment( comment );
    }
  }
...
function init_recent_events( page_name, twitter_style, span, time_limit ){
  ...
  RE_visitor = Session.current.visitor;
  RE_machine = Machine.current;
  RE_filter  = Session.current.filter;
  RE_query   = Session.current.filter_query;
  RE_span    = span;
}
...
function recent_events_div( page_name, check_only ){
  ...
  RE_size = RE_SIZE;
  var buf = [];
  buf.push( '<div id="recent_events"><div class="hide_button"></div>' );
  if( twitter_style ){
    buf.push(
      '<div class="recent_events_header">',
      link_to_page( "votes", "direct", i18n( "Votes" ) ),
      span === "all" ? "" : ' <small><dfn>(' + i18n( span ) + ')</dfn></small>',
      session.visitor ? ""
      : ' <span style="font-size:11px;">'
      + link_to_page( "login", "twitter_direct", "login to vote" )
      + "</span>",
      "</div>"
    );
  }else if( span !== "all" ){
...
// The private page of a persona
  ...
  if( display_votes ){
    ...
    this.push(
      ...
      '<br><div><a id="votes"></a><h2>',   // #votes anchor
      i18n( "Your votes" ),
      '</h2> <dfn>', filter, '</dfn>',
      !filter ? "" 
...
// The private page of a persona's delegations
  ...
  this.push(
    '<div><div class="hide_button"></div><h2>',
    i18n( "Your delegates" ), "</h2>",
  ...
  if( proposition_names.length ){ // && this.session.has_delegateable_filter() ){
    this.push(
      ...
      "<h2>Propositions</h2> - ",
      link_to_page(
        "propositions", "", icon( "zoom-in" ) + " " + i18n( "details" ) 
      ),
      "<br><br>"
    );
...
// propositions, filtered.
  ...
  if( session.is_novice ){
    ...
    if( ! visitor ){
      this.push_help(
        ...
        " ", link_to_page( "login", "twitter_direct", "login" )
      );
    }
  }
  ...
  this.push( '<div id="propositions_list">' );
  ...
  if( !count ){
    if( invalid_count ){
      msg1 += i18n( "empty" ) + " " + sort_label( true, true ) + ". ";
      msg1 += invalid_count + " " + icon( "propositions" ) + " ";
      msg1 += sort_menu( "", true /* dont_hide */ );
    }
  }else{
...
function page_ballot2( /* page_name, ...query */ ){
  ...
  this.push( '<br><div><div class="hide_button"></div>' );
  if( this.session.is_novice ){
    this.push_help(
      i18n( 
  ...
  Ephemeral.each( tags, function( tag ){
    ...
    if( total ){
      ...
      that.push(
        ...
        ""  , icon( "agree"   ), " ", i18n( "agree"    ), sp,
        count_for, bp, ratio_for, ep,
        
        ". ", i18n( "against" ), " ", i18n( "disagree" ), sp,
        count_against, bp, ratio_against, ep,
...
// This is the votes page of the application, filtered.
  ...
  if( !valid_votes.length ){
    ...
    if( valid_votes.length >= 200 ){
      msg1 = i18n( "among" ) + " " + msg1;
    }
    
    msg1 += count_propositions + " " + icon( "propositions" );
    
    if( count_propositions > 1 ){
      msg1 += sort_label( true ),
      msg1 += sort_menu();
    }
  ...
  valid_votes.forEach( function( vote_value ){
    ...
    if( Vote.valid( vote_value.entity ) ){
      ...
      if( orientation_now !== vote_value.orientation ){
        ...
        + i18n( "now" ) + " " + emoji( orientation_now );
...
function page_delegates( page_name, optional_proposition_name ){
  ...
  if( !count_votes ){
    ...
    this.push(
      ...
      span === "all" ? "" : " <dfn>(" + i18n( span ) + ")</dfn>",
      ". ", 
      icon( "direct" ), " ", i18n( "direct(s)" ), nbsp, count_direct_votes,
      nbsp, '<dfn>(', ratio_direct, "%)</dfn>. ",
      icon( "indirect" ), " ", i18n( "indirect(s)" ), nbsp, count_indirect_votes,
      nbsp, '<dfn>(', ratio_indirect, "%)</dfn>. "
    );
  }
  this.push( sort_menu( "", !count_votes /* show */ ) );
  
  // Delegates. Display agents
  if( agent_ids.length ){
    this.push(
      "<br><br><h2>",
      icon( "indirect" ), " ", i18n( "Delegates" ),
      "</h2>"
    );
    if( persona ){
  ...
  // Client side
    ...
    // Client side
      if( auth_data ) {
        ...
        $.ajax( "?i=authentic/" + username ).always( function(){
          go_back( username );
        });
      }else{
        // user is logged out
 ...
 *  'main' is the name when the page should look like all the other page,
...
function page_index( page_name ){
  ...
  var session = this.session;
  var is_main = ( page_name === "main" ) || session.magic_loader;
  
  // When logged in, it is always the 'main' page that is served
  var authentic_twitter_id
  = session.authentic && session.visitor.id.substring( 1 );
  if( false && authentic_twitter_id && !is_main )return this.redirect( "main" );
  ...
  if( !is_main ){
    ...
    // Domain info are store in the main ephemeral machine
    Ephemeral.Machine.main.activate();
    ...
    domains.forEach( function( machine ){
      ...
      valid_machines.push( machine.id || Config.domain );
    });
    ...
    if( Ephemeral.Machine.main !== current_machine ){
      valid_machines.unshift( Config.domain );
    }
    
    // First option in menu is current domain, noop
    valid_machines.unshift( domain || Config.domain );
    
    if( valid_machines.length > 1 ){
      
      var menu_list = [];
      var after_menu = avatar( domain || Config.domain, 48 ) + " ";
      menu_list.push(
        '\n<form id="domain" url="/">',
      ...
      valid_machines.forEach( function( label, index ){
        ...
        // Add the alias, if there is one
          ...
          if( persona ){
            i18n_label = persona_short_label( persona );
  ...
  if( !is_main ){
    ...
    this.set(
      ...
      ' ', titled( link_to( "propositions" ), i18n( "Search" ) ),
      
      ' - ', '<a title="Wiki" class="wiki" href=', Config.wiki,
        !domain ? "" 
        : domain + "/", 
        "HomePage",
      ...
      ? link_to_page( "visitor", "", avatar( authentic_twitter_id, 32 ) )
        + titled( link_to( "signout" ), i18n( "Sign out" ) )
      : titled( link_to( "login" ), i18n( "Login" ) ),
      
      l8.client ? "" : ' - '
        + titled( link_to( "main", "light version", true ), "Mobile" ),
      ' - ', titled( link_to( "help" ), i18n( "Help" ) ),
  ...
  if( is_main ){
    ...
    // 'main' page, normal style
    this.set( page_style( "main" ), page_header_left() );
  ...
  // Recent events, it floats on the left
  this.push(
  ...
  // Skip tags in "main" page, unless #tag is inside filter
  var visitor = session.visitor;
  var filter = session.filter;
  var skip_tags        = filter.indexOf( " #tag "     ) === -1;
  var without_orphans  = filter.indexOf( " #orphan "  ) === -1;
  var without_personas = filter.indexOf( " #persona " ) === -1;
  if( searching_domains ){
    skip_tags = without_orphans = without_personas = false;
  ...
  // Propositions list, it floats on the right
  this.push(
    '<div class="twitter_style">',
    ...
    link_to_page(
      "propositions", 
      "all",
      i18n( "Propositions" ) + "&nbsp;" + icon( "propositions" )
    ),
    '</div>'
  );
  ...
  if( !count ){
    ...
    msg1 
    += " - " + icon( "zoom-in" ) 
    +  " "   + link_to_page( "proposition", "", i18n( "details" ) )
    +  " "   + icon( "delegations" )
    +  " "   + link_to_page( "delegates", "", i18n( "delegates" ) );
  }
  ...
  Ephemeral.each( list, function( proposition, index ){
    ...
    if( comment ){
      that.push( wikify_comment( comment ), '<br>' );
    }
    ...
    that.push(
      proposition.is_tag() ? i18n( "Tag" ) : "",
      link_to_page( "proposition", proposition.label ),
      " ", link_to_wiki( proposition.label ),
      !proposition.is_persona() ? ""
      : ( '<dfn>(' 
        + link_to_persona_page( proposition.get_persona() )
        + ")</dfn> "
  ...
  this.at_insert(
    tag_set.insert_index, 
    filter_change_links( tag_set, "force" /* hide */ )
  );
...
function page_help(){
  ...
  if( this.session.can_script === true ){
    version_msg 
    = '<span class="action" ' + show_on_click + '>' 
    + icon( "help" ) + " " + i18n( "Language" )
    + '</span><div id="languages" class="kudo_collapse">';
  }
  ...
  if( this.session.lang !== "fr" ){
    msg.push(
      ...
      icon( "direct" ), ' each one of us can ', emoji( "agree" ), ' like or ',
      emoji( "disagree" ) + ' dislike ',
      icon( "propositions" ), ' propositions associated to ',
      icon( "tags" ), ' hashtags, or ',
      icon( "indirect" ), " delegate.",
      
      '<br><br>',
      
      '<h2>', icon( "tags" ), ' Hashtags?</h2><br>',
      'Hashtags are keywords used to categorize topics in social networks. ',
      '<br><br>',
      
      '<h2>', icon( "indirect" ), ' Delegate?</h2><br>',
      'On ', icon( "propositions" ), ' propositions that match ',
      icon( "tags" ), ' tags you specified, you tell who ',
      icon( "votes" ), ' votes for you ',
      icon( "indirect" ), ' unless you vote directly ', icon( "direct" ), '.',
      '<br><br>',
...
//      '<li>You can share your votes or hide them.</li>',
  ...
  // French
    ...
    msg.push(
      ...
      icon( "direct"), ' chacun peut ', emoji( "agree" ) + ' approuver ou ', emoji( "disagree" ),
      ' d&eacute;sapprouver des ', icon( "propositions" ), ' propositions',
      ' associ&eacute;es &agrave; des ', icon( "tags" ), ' hashtags',
      ', ou ', icon( "indirect" ), ' d&eacute;l&eacute;guer.',
      '<br><br>',
      ...
      'Sur des ', icon( "propositions" ), ' propositions, ',
      'associ&eacute;es &agrave; des ', icon( "tags" ), ' hastags',
      ', vous d&eacute;signez qui ', icon( "votes" ), ' vote pour vous ',
      icon( "indirect" ),
      ', sauf &agrave; voter directement ', icon( "direct" ), '.',
      '<br><br>',
      ...
      '(', icon( "vote"), ' <a href="proposition/kudocracy">votez</a> #kudocracy !), ',
      ...
      ' <a href="http://en.wikipedia.org/wiki/Delegative_democracy">',
      'D&eacute;mocratie d&eacute;l&eacute;gative</a>, j\'ai aim&eacute;. ',
  ...
  this.push(
    ...
    "<br>", icon( "propositions" ), " ", 
    " ",    i18n( "propositions" ), " ",  Topic.count,
    "<br>", icon( "votes"        ), " ", 
    " ",    i18n( "votes"        ), " ",  Vote.count,
    "<br>", icon( "votes"        ), "! ",
    " ",    i18n( "comments"     ), " ",  Comment.count,
    "<br>", icon( "indirect"     ), 
    " ",    i18n( "delegations"  ), " ",  Delegation.count,
    "<br><br>"
  );
  this.push( page_footer() );
...
emoji.table_ascii = {
  ...
  against:   '<span class="glyphicon glyphicon-thumbs-down"></span>',
...
function proposition_summary( proposition, div, options ){
  ...
  function cond_push( label, n, style ){
    if( n ){
      ...
      buf.push( icon( label ), "&nbsp;", i18n( label ) );
      buf.push( '&nbsp;', n );
      if( n !== total ){
  ...
  if( !no_counters ){
    ...
    if( total ){
      ...
      if( result.total() && result.direct() != result.total() ){
        ...
        buf.push(
          ' <span class="nobr">',
          icon( "votes" ), "&nbsp;", i18n( "total" ), "&nbsp;", result.total(),
          '.',
          '</span> <dfn class="nobr">', 
          icon( "direct" ), "&nbsp;", i18n( "direct" ), '&nbsp;',
          result.direct(),
          "&nbsp;(",
          ratio_direct,
          '%) ',
          icon( "indirect"), "&nbsp;", i18n( "indirect"), '&nbsp;',
...
// Focus on one proposition
  ...
  buf.push(
    ...
    : ( ' - ' + link_to_persona_page( persona ) ),
    ' - ',
    link_to_wiki_icon( persona ? persona.label : proposition.label ),
    ' - ',
    '#', link_to_twitter_tag( proposition.label )
  );
  ...
  if( persona && persona.is_domain() ){
    ...
    this.push( '">', persona.label, '</a></h2><br><br>' );
  ...
  if( comment.indexOf( "..." ) !== -1 ){
    buf.push(
      '<div class="comment">',
      wikify_comment( comment, true /* no trunctate */ ),
      '</div>'
    )
  }
  ...
  tmp.split( " " ).forEach( function( tag ){
    ...
    if( comment ){
      ...
      buf.push( wikify_comment( comment.text ));
    }
    buf.push( '</td></tr>' );
  });
  buf.push( '\n</table></div></div>\n' );
  ...
  votes.forEach( function( vote_value, index ){
    ...
    if( !idem && ( !gap || index >= votes.length - 200 ) ){
      ...
      && was !== now
        ...
        + emojied( was ) + " " + icon( "arrow right" ) + "</dfn> "
        + orientation
        + '</nobr>';
      }
  ...
  if( only_comments || only_direct_votes || only_indirect_votes ){
    buf_voters.push( 
      " ", link_to_page(
        "proposition", plabel + " all", "all(s)", "voters"
      )
    );
  }else{
    buf_voters.push( " <h3>", i18n( "all(s)" ), "</h3> " );
  }
  ...
  if( !only_comments ){
    buf_voters.push( 
      " ", link_to_page( 
        "proposition", plabel + " comments", "comments", "voters" // anchor
      )
    );
  }else{
  ...
  if( !only_direct_votes ){
    buf_voters.push( 
      " ", link_to_page( 
        "proposition", plabel + " direct", "direct(s)", "voters"
      )
    );
  }else{
  ...
  if( !only_indirect_votes ){
    buf_voters.push( 
      " ", link_to_page( 
        "proposition", plabel + " indirect", "indirect(s)", "voters"
      )
    );
  }else{
...
http_repl_commands.login = function( /* name, authentication, alias, more */ ){
  ...
  // Special "new xxx" alias is an admin command when issued by domain owner
  ...
  if( type ){
    ...
    if( type === "Session" ){
      ...
      if( path ){
        ...
        var entity = Kudo.Ephemeral[ type ].find( path );
        if( entity ){
          entities.push( entity.json_value() );
        }
      }else{
        Ephemeral.each( Kudo.Ephemeral[ type ].all, function( entity ){
          entities.push( entity.json_value() );
        });
      }
    }
