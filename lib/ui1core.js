...
http_repl_commands.change_vote = function( vote_entity, orientation, duration, comment ){
  ...
  // Figure out parameters, maybe from pending http query  var proposition = null;
  var query = get_query();
...
// Process http requests that starts with /api/
  ...
  test( "topic" ) || test( "proposition")
  test( "persona" ) || test( "vote") || test( "delegation");
  if( type ){
    var type_name = type[0].toUpperCase() + type.substring( 1 );
    Ephemeral.each( Ephemeral[ type_name ].all, function( entity ){
