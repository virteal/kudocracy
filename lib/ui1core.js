...
// options: with_twitter
  ...
  var buf = [
    ...
    : link_to_page(
      "delegates",
      proposition.label,
      compact ? icon( "delegates" ) + " " : "Delegate"
    ), "&nbsp;",
    compact ? "" : i18n( "or" ) + ' <h2>&nbsp;',
...
function format_change( change, page_name, twitter_style, array, at_tail ){
  ...
  if( type === "Comment" ){
    ...
    var sub_msg = link_to_page(
      "propositions", str_tags, str_tags.replace( / /g, "+" ) 
    ) + " " + link_to_delegation_page( delegation );
    if( session.visitor && !ReuseEvent.vote_menus[ delegation.id ] ){
      ReuseEvent.vote_menus[ delegation.id ] = true;
      msg_br = "";
      msg += '<table><tr><td>' + sub_msg + '</td><td class="timeline_vote delegate_too">'
      + icon( "delegates" ) + " "
...
function page_index( page_name ){
  ...
  // Will display list of matching propositions
  this.push(
