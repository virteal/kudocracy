...
// Init/reinit all session attributes. Init when ctor called (via login()).
  ...
  this.page_id         = ""; // of current page, see page_style()
...
Session.set_current = function( session ){
  if( session ){
    ...
    Builder.current = session.page_builder;
  }else{
...
console.log( "Fragment", id )
  var same = this.page_fragments[ id ];
  if( same ){
    same = same === content;
  }
  if( same )return "";
...
var ProtoBuilder = Builder.prototype;
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    // with $(document).ready(), simple!
      ...
      if( body === scripts ){
        ...
        var process_fragments = function( elements, $parent, is_reset ){
          
          var save_$local = $local;
          var save_$container = $container;
          $container = $parent;
          ...
          for( ii = 0 ; ii < len ; ii++ ){
            ...
            nde&&bugC( 
              "magic element", fragment_id, element.nodeName, magic_directive 
            );
            ...
            if( !fragment_id ){
              ...
              if( element.childElementCount ){
                process_fragments( element.children, $(element), is_reset );
              }
            ...
            if( $fragment && $fragment.length ){
              ...
              if( is_reset ){
                ...
                // console.log( "magic reset of", fragment_id );
                continue;
              }
              ...
              if( magic_directive === "upsert" ){
                ($local || $container).append( element );
                // console.log( "magic upsert of", fragment_id );
            ...
            // If new fragment
              ...
              if( is_reset ){
                ...
                // console.log( "magic reset, insert", fragment_id );
                continue;
              }
              ...
              if( magic_directive === "upsert" ){
                ...
                if( magic_directive === "update" ){
                  ...
                  process_fragments( dom_fragments, $main_container, true );
                  // console.log( "magic reset, due to", fragment_id );
                  done = true;
               }
              }
            }
          ...
          $local     = save_$local;
          $container = save_$container;
        };
        
        process_fragments( dom_fragments, $main_container, false );
    ...
    $.ajax( target_url, {
      ...
      error: function( data ){
        ...
        process_magic_response( null, err );
        window.kudo_new_location = url;
        window.location.replace( url );
      }
    });
...
// This defined the HEAD of a page. Most pages calls this.
  ...
  // "magic" means minimal content, because it was sent before
  if( session.magic_loader && !needs_firebase
...
// options: with_twitter, compact
  ...
  var recommendation_msg = new Builder();
...
ProtoBuilder.push_delegations = function( persona, br, misc ){ 
  ...
  if( persona === visitor ){
    this.push(
      ...
      ". ", 
      this.session.has_filter() && link_to_page( 
...
function update_recent_events( page_name, twitter_style, span, time_limit ){
  ...
  // Needs update?
  if( RE_last_change_index === Machine.current.changes.length - 1
  )return false;
...
function recent_events_div( page_name, check_only ){
  ...
  if( span_option ){
    ...
    + span_option 
    + '</span><div class="kudo_collapse">' + sort_menu() 
    + '</div></small>';
  }
  if( twitter_style ){
    buf.push(
...
// The private page of a persona
  ...
  if( !session.is_novice ){
    this.push(
      " ", link_to_command( "help_toggle", i18n( "help" ) ), "."
    );
  }
  ...
  if( display_votes ){
    ...
    this.push(
      ...
      i18n( "Your votes" ),
      '</h2> <dfn>', filter, '</dfn>',
      filter && " - " + link_to_page( "visitor", "all all", "all", "votes" ),
      "<br>",
...
function page_ballot2( /* page_name, ...query */ ){
  ...
  Ephemeral.each( tags, function( tag ){
    ...
    var count_against = 0;
    var count_blanks  = 0;
    var buf_proposition_votes = new Builder();
    index++;
    that.push(
      div[ index % 2 ],
      '<h3>',
...
function page_delegates( page_name, optional_proposition_name ){
  ...
  if( agent_ids.length ){
    ...
    if( session.can_script && ( l8.server || window.google ) ){
      ...
      = { all: [ [ i18n( "direct" ), count_direct_votes ] ] };
      this.push( 
        '<div><div class="hide_button"></div>',
...
function page_index( page_name ){
  ...
  if( Config.appcache_support && session.request.url === "/" ){
  ...
  var twitter_name = domain || Config.domain || "kudocracy";
  
  var twitter_timeline = new Builder(
  ...
  // if index page
    ...
    if( !full_query || full_query === "-heat" ){
      ...
      rename += 
      session.url
      .replace( /page=[^.&]*/, "page=main/" + encode_ref( full_query ) );
    } 
...
// Focus on one proposition
  ...
  buf.push(
    ...
    i18n( proposition.label ),
    '</h2>',
    persona && ( ' - ' + link_to_persona_page( persona ) ),
    ' - ',
