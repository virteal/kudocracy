...
// 'text' is some text to add to build the target url.
  ...
  if( !page || !page.length )return this.redirect( "home" );
...
function filter_change_links( tag_set, dont_hide ){
  ...
  tag_set.each( function( label ){
    ...
    if( !display 
    && new_category === "computed" 
    && tags_by_category.computed.length < max * 2
    ){
      display = true;
    }
...
PageBuilder.prototype.push_vote_menu = function( proposition, options ){
  ...
  if( session.can_script ){
    ...
    if( options.float ){
      this.push( '</div><div class="vote_menu_content kudo_collapse">' );
    }else{
      this.push(
...
function format_change( change, page_name, twitter_style, array, at_tail ){
  ...
  // Vote menu on index page for logged in visitor
  if( proposition ){
    if( session.visitor ){
      if( !ReuseEvent.vote_menus[ proposition.id ] ){
        ...
        + vote_menu(
          session.visitor,
          proposition,
          { compact: true, nofocus: true, index: twitter_style }
        )
        + '</div><div>'
...
function page_index( page_name ){
  ...
  // Recent event & twitter timeline
  this.push(
    '<div class="twitter_style">',
      recent_events_div( page_name ),
    '</div>',
    '<div id="main_twitter_timeline" class="hide_button">',
    twitter_timeline,
    '</div>'
  );
