...
// Header with 'propositions', 'delegates', 'votes', 'ballot' ... login help
  ...
  if( current_page !== "delegates" ){
    m += " " + titled( 
      link_to_page( "delegates", "all", i18n( "delegates" ) ),
      i18n( "Delegates" )
    );
  }
...
function recent_events_div( page_name, check_only ){
  ...
  if( twitter_style ){
    buf.push(
      ...
      + link_to_page( "login", "twitter_direct", i18n( "login to vote" ) )
      + "</span>",
      "</div>"
    );
  }else if( span !== "all" ){
...
// The private page of a persona
  ...
  if( display_votes ){
    ...
    this.open_div( "your_votes").push(
      ...
      '</h2> <dfn>', filter, '</dfn>',
      filter 
      && " - " + link_to_page( "visitor", "all all", i18n( "all" ), "votes" ),
      "<br>",
      
      icon( "votes" ), " ",
      !all_votes
      ? link_to_page( "visitor", "all", i18n( "all(s)" ), "votes" )
      : "<h3>" + i18n( "all(s)" ) + "</h3>",
      
      " ", icon( "direct" ), " ",
      ( all_votes || without_direct_votes )
      ? link_to_page( "visitor", "direct", i18n( "direct(s)" ), "votes" )
      : "<h3>" + i18n( "direct(s)" ) + "</h3>",
      
      " ", icon( "indirect" ), " ", 
      ( all_votes || without_indirect_votes )
      ? link_to_page( "visitor", "indirect", i18n( "indirect(s)" ), "votes" )
...
function page_ballot2( /* page_name, ...query */ ){
  ...
  this.open_div( "votes" )
    .push( "<br><h2>", i18n( "Votes" ), "</h2><br>", buf_votes.join() )
  .close_div();
...
// This is the votes page of the application, filtered.
  ...
  if( valid_votes.length ){
    ...
    this.push(
      ...
      filter 
      && " - " + link_to_page( "votes", "all all", i18n( "all" ), "votes" ),
      "<br>"
    );
    ...
    if( !comments_only && !direct_votes_only && !indirect_votes_only ){
      ...
      this.push(
        link_to_page( "votes", "all", i18n( "all(s)" ), "votes" )
      );
    }
