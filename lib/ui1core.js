...
function page_index( page_name ){
  ...
  // if index page
    ...
    this.push(
      ...
      '<br>(C) 2015 <a href="http://github.com/virteal">Virteal</a></div>',
...
// Focus on one proposition
  ...
  if( is_tag ){
    ...
    if( as_tag ){
      propositions = as_tag.propositions();
    }
  }
  ...
  // Proposition's name & twitter + wiki links
  buf.push(
    '<h2>', (is_tag ? l( "Tag" ) + " " : "" ),
    l( proposition.label ),
    '</h2>',
    persona && ( ' ' + link_to_persona_page( persona ) ),
    ' ', link_to_twitter_tag( proposition.label, false /* title */ ),
  ...
  // Picture of other personas who voted
  var recommendations = proposition_recommendations({
    proposition: proposition,
    persona: visitor,
  ...
  // Display full comment if it was truncated
  ...
  var tags_counts_by_agent = map(); // map of maps
  var all_agents = [];
  var seen_personas = set();
  var all_personas = [];
    ...
    for( var ii = 0 ; ii < len ; ii++ ){
      ...
      for( idx_tag = 0 ; idx_tag < tags_len ; idx_tag++ ){
        ...
        abuf.push(
          ...
          link_to_page( "persona", agent_id + " all " + tags, l( tags ) )
        );
        if( tags_len > 1 ){
          ratio = Math.round( 1000 * ( 
            tags_counts[ tags ] / c
