...
function page_delegates( page_name, optional_proposition_name ){
  ...
  if( agent_ids.length ){
    ...
    if( session.can_script && ( l8.server || window.google ) ){
      ...
      this.push( 
        '<div><div class="hide_button"></div>',
        '<div id="delegates_chart_div_all" class="chart_pie"></div>',
        '</div>'
      );
    }else{
      this.push( "<br><br>" );
  ...
  agent_ids.forEach( function( agent_id ){
    ...
    for( var ii = 0 ; ii < len && ii < 10 ; ii++ ){
      ...
      var pie = delegates_graph_pies && delegates_graph_pies[ agent_id ];
      if( pie ){
        pie.push( [ tags, delegation_counts_by_agent[ agent_id ][ tags ] ] );
      }
  ...
  if( all_tag_ids.length ){
    ...
    if( session.can_script && ( l8.server || window.google ) ){
      ...
      this.push(
        '<div><div class="hide_button"></div>',
        '<div id="tags_chart_div_all" class="chart_pie"></div>',
        '</div>'
      );
    }else{
      this.push( "<br><br>" );
...
// Runs client side
  ...
  function drawChart(){
    ...
    chart = new google.visualization.PieChart(
      document.getElementById( 'delegates_chart_div_all' )
    );
    ...
    // Draw the other smaller delegates pies
    options = {
      'width':  300,
      'height': 150,
      'chartArea': {'width': '100%', 'height': '100%'},
