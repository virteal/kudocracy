    ...
    if( query_str.indexOf( "page=login" ) !== -1 ){
      ...
      if( ui1_server ){
        ...
        cookie_val = encodeURIComponent(
          ui1_server.Session.current.filter
        );
        cookie
        = cookie_id + "=" + cookie_val + ";max-age=120";
        console.log( "set cookie", cookie );
...
function page_twitter( page_name ){
  ...
  var previous_page
  = redirected && session.get_cookie( "login_previous_page" );
  if( previous_page ){
    previous_page
  ...
  function twitter_redirect( firebase, redirected, name, previous_page ){
    ...
    if( !Firebase ){
      ...
      window.setTimeout( function(){
        twitter_redirect( firebase, redirected, name, previous_page );
      }, retry_delay );
      return;
    }
    ...
    function process_result( auth_data ){
      if( auth_data ) {
        ...
        if( !previous_page ){
          ...
          console.log(
            "Firebase auth, show previous page", name, previous_page
          );
...
function page_index( pagename ){
  ...
  this.set(
    ...
    + '\n<title>'
      + ( pagename === "index" ? "Kudocracy" : pagename ) // manifest.appcache
    + '</title>',
