...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    ctx.touch_device = !!Modernizr.touch;
...
// This is the votes page of the application, filtered.
  ...
  this.push( " " );
  if( direct_votes_only ){
    this.push( "<h3>", icon( "direct" ), " ", i18n( "direct(s)" ), "</h3>" );
  }else{
    this.push(
      link_to_page( 
        "votes",
  ...
  // Client side
    ...
    // Client side
      if( auth_data ) {
        ...
        // Avoid a race condition between ajax and reload where new page is
        // requested before ajax request.
        setTimeout( function(){
          go_back( username );
        }, 20 );
      }else{
...
function page_index( page_name ){
  ...
  this.push(
    '<div id="propositions_list" class="twitter_style hide_button">',
