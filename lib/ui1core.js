...
// client side
  ...
  $(".hide_button").prepend( hide_button );
  $(".even").prepend(        hide_button );
  $(".odd").prepend(         hide_button );
  $(".hide_button_icon").click( hide );
  $(".show_button_icon").click( show );
};
...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    if( header_height < 42 ){
      ...
      = "" + ( header_height + ( ctx.touch_device ? header_height : 2 ) ) 
      + "px";
    }
    ...
    if( ( ctx.screen_height > ctx.screen_width ) || ctx.touch_device ){
      $all_anchors.each( function(){
        var height = $(this).outerHeight();
        var width  = $(this).outerWidth();
        if( height < header_height * 0.8 ){
          // Need to increase the height
          var factor = ( header_height * 1 ) / height;
          console.log( "header h", header_height, "link", height, "x", width );
          var attr = "height" // "font-size"
          var cur_size = parseInt( $(this).css( attr ), 10 );
          var new_size = Math.round( cur_size * factor );
          if( attr === "height" ){
            var narrow = width < height * 2;
            if( narrow ){
              // Don't expand target for img/avatars
              if( $(this).children(":first").is( "img" ) ){
                narrow = false;
              }
            }
            if( narrow ){
              $(this).css( "padding", ( new_size - cur_size ) / 2 )
            }else{
              $(this)
              .css( "padding-top", ( new_size - cur_size ) / 2 )
              .css( "padding-bottom", ( new_size - cur_size ) / 2 )
            }
          }else{
            $(this).css( attr === attr, new_size )
          }
          console.log( "Increase size from ", cur_size, "to", new_size );
        }
      });
    }
...
// propositions, filtered.
  ...
  // Inject header, late, it depends on the filteredpropositions count
...
function page_delegates( page_name, optional_proposition_name ){
  ...
  if( session.can_script && ( l8.server || window.google ) ){
    buf2.push(
      '<div id="propositions_chart_div" class="hide_button" style="height:300px"></div>'
    );
  }else{
    this.push( "<br><br>" );
  }
  ...
  if( session.can_script && ( l8.server || window.google ) ){
    this.push( '<div id="delegates_chart_div" class="hide_button" style="height:300px"></div>' );
  }else{
    this.push( "<br><br>" );
  }
  ...
  if( session.can_script && ( l8.server || window.google ) ){
    this.push( '<div id="tags_chart_div" class="hide_button" style="height:300px"></div>' );
  }else{
    this.push( "<br><br>" );
  }
...
function page_propose( page_name ){
  ...
  this.push(
    ...
    '<br><label>', i18n( "new&nbsp;pproposition" ), '</label><br>',
...
//      '<li>You can share your votes or hide them.</li>',
  ...
  this.push(
    "<br>", icon( "personas"     ), " ", Persona.all._count,
    "<br>", icon( "propositions" ), " ", Topic.all._count,
    "<br>", icon( "votes"        ), " ", Vote.all._count,
    "<br>", icon( "votes" ),
    " ",    i18n( "comments"     ), " ", Comment.all._count,
    "<br>", icon( "delegations"  ), 
    " ",    i18n( "delegations"  ), " ", Delegation.all._count,
    "<br><br>"
  );
...
// Focus on one proposition
  ...
  if( session.can_script && proposition.result.total() ){
    buf.push( '<div id="orientation_chart_div" class="hide_button" style="height:300px"></div>' );
  }
  
  // Balance time serie graph
  if( session.can_script ){
    buf.push( '<div id="balance_chart_div" class="hide_button" style="height:100px"></div>' );
  }
  ...
  if( count_indirect_votes ){
    ...
    // pie
    if( Session.current.can_script ){
      abuf.push( '<div id="delegates_chart_div" class="hide_button" style="height:300px"></div>' );
    }
