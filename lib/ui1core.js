...
var config = {
  ...
  // The UEB html img tag icon (Universal Edit Button, a 'standard')
  img_ueb_icon: '<img src="http://simpliwiki.com/ueb16.png" type="image/png" height="16" width="16"/>',
...
// Compute config values that are derived from configurable ones.
  ...
  config.img_ueb_icon.replace( /src="([^"]*)/, function( src ){
    config.img_ueb_icon_src = src;
  });
}
...
function iconized( msg, html_a ){
  ...
  if( icon !== msg ){
    icon = '<a ' + html_a + ' title="' + i18n( msg ) + '">' + icon + '</a>';
  }else{
    var comment;
    if( icon[0] === "#" ){
...
function link_to_page( page, value, title, anchor ){
  ...
  if( adjusted_title[0] === "#" && adjusted_title.indexOf( " " ) !== -1 ){
    adjusted_title = adjusted_title.replace( /#a-z/g, function( tag ){
      return i18n( tag );
    });
  }else{
...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    var anchor;
    path = path.replace( /#[a-z]+$/, function( match ){
...
function page_header( left, center, right, current_page ){
  ...
  built_left
  = iconized( current_page, 'title="' + i18n( current_page ) + '"' )
  + " " + built_left;
  ...
  if( builder.session.visitor ){
    ...
    + link_to_page(
      "visitor",
      "",
      avatar( builder.session.visitor.label, 21 )
    );
  }else{
    right = ( ( right && ( right + " " ) ) || "" )
...
// to use to show/hide some propositions.
  ...
  if( with_filter ){
    ...
    r.push(
      ...
      '<input type="search" autosave="filter" name="i4" value="',
        tags_label,
  ...
  tag_set.each( function( label ){
    ...
    if( new_category != old_category ){
      ...
      if( true || Session.current.is_novice ){
        buf.push( iconized( new_category ), " " );
      }
    ...
    filter_label = filter_label.replace( /#[a-z0-9]+/gi, function( tag_name ){
      ...
      */
      var r = link_to_command( 
        ...
        "<h3>" + iconized( tag_name ) + "</h3>" + iconized( "Remove" )
      );
      var comment = Topic.reserved_comment( tag_name );
      if( comment )return titled( r,  i18n( comment ) );
      var tag_topic = Topic.find( tag_name );
      if( !tag_topic )return r;
...
PageBuilder.prototype.push_title_and_search_form = function( title, hide ){
  ...
  if( novice ){
    this.push(
      ' <span class="help"> ',
      i18n( "or click to select desired tags: " ),
      '</span>'
    );
  }
...
PageBuilder.prototype.push_vote_menu = function( proposition, options ){
  ...
  if( this.session.can_script ){
    ...
    if( options.float ){
      this.push( '</div><div style="display:none"> <span>' );
    }else{
      this.push(
        ' <span style="display:none">'
      );
  ...
  this.push(
    "<br>",
    !this.session.is_novice ? "" : i18n( "currently" ) + " ",
    i18n( "you" ), " ",
    emojied( orientation ),
    is_direct ? ""
...
PageBuilder.prototype.push_delegations = function( persona, br, misc ){ 
  ...
  if( persona === visitor ){
    this.push(
      iconized( "delegations" ), " ", i18n( "Your delegations" ), '</h2>',
    ...
    this.push(
      iconized( "delegations" ), " ", i18n( "Delegations" ),
      " ",
...
"This page lists informations about a person, her votes, her delegations (received and given), etc."
  ...
  if( delegateable_filter ){
    ...
    false && this.push(
      "<br>", iconized( "propositions" ), "&nbsp;",
      link_to_page(
        "propositions",
        delegateable_filter,
        delegateable_filter_label.replace( / /g, "&nbsp;+&nbsp;" )
      ),
      " ", iconized( "delegations" ), "&nbsp;",
      link_to_page(
  ...
  if( elen || not_done ){
    that.push(
      ...
      '<h2>',
      iconized( "delegations" ), " ", i18n( "Delegate" ), 
...
// propositions, filtered.
  ...
  list.forEach( function( proposition, index ){
    ...
    that.push( ' <span style="white-space:nowrap">' );
    tag_set.add_proposition( proposition, function( tag, label ){
      ...
      if( persona ){
        ...
        if( alias ){
          // Show label in title of link if there is an alias displayed
          toggle = titled( toggle, persona.label );
        }
        that.push(
          link_to_persona_page( persona, "@" ), // "@" => no label
          toggle,
          " "
        );
      }else{
        that.push(
          link_to_command( "filter_toggle " + tag, iconized( label ) ),
          " "
        );
      }
    }, tag_page );
  ...
  if( sparklines ){
    ...
    if( count > 1 ){
      msg1 += count + " " + iconized( "propositions" ) + " ";
      msg1 += ""
      + i18n( "between" )
      + " "
...
function page_ballot2( /* pagename, ...query */ ){
  ...
  this.push( 
    ...
    npersonas, " ", iconized( "personas" ), ". ",
    tags.length, " ", iconized( "propositions" ), ".<br>"
  );
...
// This is the votes page of the application, filtered.
  ...
  if( count_propositions ){
    msg1 += count_propositions + " " + iconized( "propositions" );
...
function page_delegates( page_name, optional_proposition_name ){
  ...
  buf2.push(
    '<br><br><h2>',
    iconized( "Propositions" ), " ", i18n( "Propositions" ),
    '</h2><br>'
  );
  ...
  this.push(
    '<br>',
    count_votes, nbsp, iconized( "votes" ), ". ", 
  ...
  this.push(
    "<br><br><h2>",
    iconized( "Delegates" ), " ", i18n( "Delegates" ),
    "</h2>"
  );
  if( persona ){
  ...
  // Display filters
  this.push( 
    "<br><h2>", iconized( "Tags" ), " ", i18n( "Tags" ), "</h2><br>"
  );
  ...
  if( last_vote ){
    this.push(
      ...
      "<br>", iconized( "propositions" ), " ",   count_propositions, ".",
      "<br>", iconized( "voters" ), " ",         count_personas, ".",
      "<br>", iconized( "votes" ), " ",          count_votes, ".",
      "<br>", iconized( "votes" ), " ",
      ...
      " ", ratio_indirect, "%", ".",
      "<br>", iconized( "delegates" ), " ",  
        i18n( "delegates" ), " ",                count_agents, ".",
      "<br>", iconized( "tags"), " ",            all_tag_ids.length, ".",
...
function page_propose( page_name ){
  ...
  this.push(
    ...
    ' required',
    '/>',
    '<br><br><label>', iconized( "tags" ), '</label><br>',
    ...
    ' autocapitalize="none" autocorrect="off"',
    '"/>',
    '<br><br><label>', iconized( "comment" ), '</label><br>',
...
function page_index( pagename ){
  ...
  this.set(
    ...
    l8.client ? "" : ' - '
      + titled( link_to( "propositions", "light version" ), "Mobile" ),
...
//      '<li>You can share your votes or hide them.</li>',
  ...
  this.push(
    "<br>",
    "<br>", iconized( "personas"     ), " ", Persona.all._count,
    "<br>", iconized( "propositions" ), " ", Topic.all._count,
    "<br>", iconized( "votes"        ), " ", Vote.all._count,
    "<br>", iconized( "comments"     ), " ", Comment.all._count,
    "<br>", iconized( "delegations"  ), " ", Delegation.all._count,
    "<br><br>"
  );
...
// Focus on one proposition
  ...
  var recommendations = proposition_recommendations({
    ...
    count: 150 // 32 + 2 px each, ToDo: fit in 960px?
  });
  if( recommendations.length ){
    buf.push( '<br><div><table>' );
    var by_orientation = {
      agree: [],
      disagree: [],
      protest: [],
        ...
        list.forEach( function( vote ){
          buf.push( titled(
            link_to_persona_page( 
              vote.persona,
              avatar( vote.persona.label, 64 )
            ),
            vote.persona.get_alias() || vote.persona.label
          ));
        });
        buf.push( "</td></tr>" );
      }
  ...
  // trace( "Tags: " + tmp );
  if( true || tmp || session.visitor ){
    buf.push( '<br><h2>', iconized( "Tags" ), " ", i18n( "Tags" ), '</h2>' );
    ...
    if( session.visitor
    && !session.visitor.is_abuse()
    && proposition.author_is( session.visitor )
    ){
