...
// this is the entry point for all html pages. It's a kind of router.
  ...
  session.previous_page = result.session.current_page;
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    if( path && path !== current_path && path.indexOf( ":" ) !== -1 ){
      ...
      && kudo_ctx.screen_width > 640
        ...
        $iframe_div.hide();
        // Change height from CSS 100% into actual size, to extand to bottom
        $iframe_div.css( "height", "100%" );
        // 60 px is for scroll arrows and hide button
        $iframe_div.height( $iframe_div.height() - 60 );
        $iframe_div.show();
        $("#kudo_iframe").attr( "src", url );
        return true;
      }
...
function page_header( left, center, right, current_page ){
  ...
  .open_div( "kudo_iframe_div", 'data-magic="once"' ).hide_button()
    ...
    .push( '<iframe id="kudo_iframe"></iframe>' )
  .close_div()
  .push(
...
"This page lists informations about a person, her votes, her delegations (received and given), etc."
  ...
  // What are the votes to display?
    ...
    if( !comments_only && !direct_votes_only && !indirect_votes_only ){
      ...
      this.push(
        link_to_page( "persona", p_label + " all", i18n( "all(s)" ), "votes" )
      );
    }
...
// The private page of a persona's delegations
  ...
  this.close_div().push( "<br>", page_footer() );
...
function page_index( page_name ){
  ...
  if( optional_twitter_timeline || !session.page_is_headless ){
    this.push(
      ...
      twitter_timeline,
      '<div class="hide_button">', i18n( "hide" ), ' </div>',
      '</div>'
    );
  }
...
function page_help(){
  ...
  if( this.session.lang !== "fr" ){
    msg.push(
      ...
      'success ',
      '(<a href="http://kudocracy.com/proposition/kudocracy">vote</a> #kudocracy !), ',
      ...
      'be a good thing to apply it broadly, using modern technologies, technologies ',
  ...
  // French
    ...
    msg.push(
      ...
      '(', icon( "vote"), ' <a href="http://kudocracy.com/proposition/kudocracy">votez</a> #kudocracy !), ',
...
// Focus on one proposition
  ...
  if( persona && persona.is_domain() ){
    ...
    this.push( '">', persona.label, '</a></h2><br><br></div>' );
  ...
  buf.push( 
    ...
    '<br>', proposition_summary( proposition, "div" ),
    '<br>'
  );
  ...
  if( votes_log.length ){
    ...
    buf.push( 
      '<div id="last_vote">',
      i18n( "last vote" ), " ",
      time_label( last_vote_value.snaptime )
    );
  ...
  if( count_indirect_votes ){
    ...
    .br().h2( icon( "Delegates" ), " ", i18n( "Delegates" ) )
    .push(
      " - ",
      link_to_page( 
        "delegates",
