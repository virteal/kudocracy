...
// Init/reinit all session attributes. Init when ctor called (via login()).
  ...
  this.page_init_done  = false; // magic loaded pages don't init the page
...
// Client side setup, dynamic, different for each page
  ...
  function kudo_init_ctx( ctx ){
    ...
    if( window.kudo_time_last_page && window.kudo_time_last_page > ctx.time ){
      console.log( "Older page, probably cached somewhere, refresh" );
      window.kudo_new_location = timeless_url;
      return;
    }
    var age = now - ctx.time;
    if( age > 1 * 60 * 1000 ){
...
var onloadScript = function kudo_onload(){
// Client side
...
// once the page is "ready"
  ...
  if( !done ){
    ...
    console.log( "BUG? useless double onload() include" );
    return;
  }
  var de  = true;
  var nde = false;
    ...
    if( window.ui1_server ){
      ...
      window.ui1_server(
        ...
        },
        {
         ...
         html = html.replace( /function kudo_onload[\s\S]*?<\/script>/, "<" + "/script>" );
