...
// Add a title="xxx" attribute to a <a xxxx link. i18n.
  ...
  return link.replace( "<a ", '<a title="' + i18n( title ) + '" ');
}
...
function link_to_wiki( page, title, no_icon ){
  ...
  if( !title ){
    return '<a class="wiki" href="' + href + '">' + img + '</a>';
  }else{
    return '<a class="wiki" href="' + href + '">' 
      + title + ( no_icon ? "" : img ) 
    + '</a>';
  }
}
...
function link_to_page( page, value, title, anchor ){
  ...
  &&  !title && adjusted_title[0] === "@"
  ){
    var user = adjusted_title.substring( 1 );
...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    if( path && path !== current_path ){
      ...
      // ToDo: offline mode
      // Let's put simpliwiki links into an iframe, if there is one
      var iframe_div = document.getElementById( "kudo_iframe_div" );
      if( url.indexOf( "?kudocracy=" ) !== -1 && iframe_div ){
...
// to use to show/hide some propositions.
  ...
  if( hide ){ 
    r.push(
      ...
      '">',
      i18n( with_filter ? "i-Search" : "i-Sort" ),
      '</a>',
...
PageBuilder.prototype.push_title = function( title, not_compact ){
  ...
  if( filter_label ){
    filter_label = filter_label.replace( /#[a-z0-9]+/gi, function( tag_name ){
      ...
      */
      return link_to_command( 
        "filter_less " + tag_name, 
        "</h2><h3>" + i18n( tag_name ) + "</h3>" + i18n( "i-Remove" ) + '<h2>'
      );
    });
    buf.push( '  <h2>'
      + filter_label
...
PageBuilder.prototype.push_vote_menu = function( proposition, options ){
  ...
  if( !visitor || visitor.is_abuse() ){
    ...
    this.push( link_to_page(
      "login",
      "", 
       '<h2>' + i18n( "vote" ) + '</h2><h3>' + "&nbsp;" + i18n( "Vote" ) + '</h3>'
    ));
    return;
  }
  ...
  if( this.session.can_script ){
    this.push(
      ...
      "this.style.display = 'none';",
      "this.nextElementSibling.style.display = 'block';",
      '"><h2>', i18n( "vote" ), "</h2>&nbsp;<h3>", i18n( "Vote" ), '</h3></a>',
      ' <span style="display:none">'
    );
  }else{
    this.push( " <span>" );
...
function page_ballot2( /* pagename, ...query */ ){
  ...
  this.push( 
    i18n( "votes" ),
    " ", time_label( time_start ), " ... ", time_label( time_limit ), ". ",
    npersonas, " ", i18n( "personas" ), ". ",
    tags.length, " ", i18n( "propositions" ), ".<br>"
  );
...
function page_delegates( page_name, optional_proposition_name ){
  ...
  this.push( "<br><br><h2>", i18n( "i-Delegates" ), "</h2>" );
  if( persona ){
...
// Visitor gets to this page during the twitter login, twice
  ...
  // Client side
    ...
    var before_redirection_request_result = needs_redirection;
    ...
    if( !needs_redirection ){
      console.log( "Firebase, set onAuth callback" );
...
function page_propose( page_name ){
  ...
  this.push(
    ...
    ' autocapitalize="none" autocorrect="off"',
    '"/>',
    '<br><br><label>', i18n( "comment" ), '</label><br>',
...
function page_index( pagename ){
  ...
  var menu = "";
  if( authentic_twitter_id ){
    menu += link_to_page( "visitor", "", "@" + authentic_twitter_id ) + "<br>";
  }
  ...
  this.set(
    ...
    ? titled( link_to_page( "signout", "", "sign out" ), "Sign out" )
...
// Focus on one proposition
  ...
  if( !only_direct_votes ){
    ...
    buf_voters.push( " <h3>", i18n( "direct votes" ), "</h3> " );
  }
  if( only_comments || only_direct_votes ){
    buf_voters.push( 
      " ", link_to_page(
        "proposition",
        plabel + " all",
