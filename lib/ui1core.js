...
http_repl_commands.login = function( /* name, authentication, alias, more */ ){
  ...
  // Create persona, first authentic visit, respect user provided case
  if( !persona ){
  ...
  // If ui1twit.js CLI twitter bot (or first authentic login), authenticate
  if( authentication === "cli" ){
    session.authentic = true;
  }
  
  // Redirect to page before page_login()
  var current_page = session.current_page[0];
...
// Called when firebase authentication succeeds, ajax called
  ...
  // Create visitor if first authentic login, as if from Twitter CLI
  if( !visitor ){
  ...
  if( visitor !== session.visitor ){
    if( session.visitor ){
      ...
      trace(
        "BUG? Twitter authentication with a different name",
        "authentic:", auth_name,
    ...
    trace(
      "BUG? Unexpected twitter authentication",
      "authentic:", auth_name,
      "session:", session.visitor.label
    )
  }
