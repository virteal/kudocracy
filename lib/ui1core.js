  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    if( !kudo_ctx.session.is_offline && query_str.indexOf( "page=login" ) !== -1 ){
      ...
      if( ui1_server ){
        ...
        + encodeURIComponent( ui1_server.Session.current.current_page.join( "," ) )
        + ";max-age=1"; // One second, transient cookie
        console.log( "set cookie", cookie );
        document.cookie = cookie;
        cookie
        = "kudo_login_filter="
        + encodeURIComponent( ui1_server.Session.current.filter )
        + ";max-age=1";
        console.log( "set cookie", cookie );
        document.cookie = cookie;
      }
      ...
      var process_changes = function(){
        ...
        ui1_server.load(
          ...
          function(){
            ...
            if( ctx.visitor ){
              console.info( "Client side login for " + ctx.visitor );
...
function page_twitter( page_name ){
  ...
  function twitter_redirect( firebase, redirected, name ){
    ...
    if( needs_redirection ){
      console.info( "Firebase. Signout", name );
      ref.unauth();
    }
...
// Runs client side
  ...
  google && google.load( 'visualization', '1.0', { 'packages': ['corechart'] } );
  google && google.setOnLoadCallback( drawChart );
