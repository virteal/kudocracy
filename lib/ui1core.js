...
// Also check local storage capabilities, also saved in a cookie.
  ...
  if( window.init_http_page_done )return;
  ...
  if( ctx.should_clear_local_storage && ctx.local_storage ){
...
// once the page is "ready"
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    if( !window.history || !window.history.pushState ){
      console.log( "No window.history.pushState" );
      return false;
    }
    ...
    || query_str.indexOf( "page=index" ) !== -1
      ...
      console.log( "No magic", query_str );
      return false;
    }
    ...
    // debugger;
    if( can_local && window.ui1_server ){
    ...
    console.log( "Ajax request", url, "ui1_server", !!window.ui1_server );
  ...
  $(document).ready( 
    window.kudo_when_ready = function kudo_when_ready(){
    ...
    if( ctx.changes ){
      ...
      var process_changes = function(){
        ...
        if( !ctx.changes ){
          //console.log( "BUG? no changes, revert to safe noscript mode" );
          //ctx.can_script = false;
          //debugger;
          return;
        }else{
          engine.load( ctx.changes );
          console.log(
...
function page_index(){
  ...
  this.session.clear();
  this.session.should_clear_local_storage = true;
  this.session.needs_twitter = true;
