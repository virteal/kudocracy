...
// domain on kudocracy.com is a sandbox.

  // On the client side, there is only one domain
  if( l8.client ){
...
// both the client and the server are in sync.
  ...
  if( l8.client ){
    ...
    if( !window.ui1_server || !window.ui1_server.inject ){
      console.log( "BUG? missing ui server" );
      debugger;
      return;
    }
...
// this is the entry point for all html pages. It's a kind of router.
  ...
  if( !f ){
    ...
    // }
    // return;
    console.log( "BUG? invalid page", name );
    debugger;
    name = "index";
    f = page_index;
  }
  ...
  if( f.length ){
    ...
    // Note: dummy "all" parameter clears the filter
      ...
      if( Session.current.filter ){
        args.push( Session.current.filter.trim() );
      }
    }
  }
...
function link_to_proposition_page( proposition ){
  ...
  return link_to_page( "proposition", name, i18n( name ) );
}
...
// Also check local storage capabilities, also saved in a cookie.
  ...
  function supports_local_storage(){
    if( window.kudo_ctx.can_local_storage )return true;
    if( window.kudo_ctx.can_local_storage === false )return true;
    // undefine, check it
    var storage;
    try{ // a try is required to avoid a weird bug in some firefox versions
      storage = 'localStorage' in window && window[ 'localStorage' ];
    }catch( e ){}
    window.kudo_ctx.local_storage = storage;
    window.kudo_ctx.can_local_storage = !!storage;
    return !!storage;
  }
  ...
  if( !ctx ){
    ...
    console.log( "Welcome to Kudocracy" );
    window.kudo_ctx = ctx = {};
  }
  if( ctx.session ){
  ...
  if( ctx.can_script === "init" || ctx.can_script === (void 0) ){
    if( window.kudo_signal_scheduled ){
      console.log( "BUG? duplicate 'can script' signal schedule" );
      debugger;
    }
    ctx.can_script = "init";
    setTimeout( function(){
      ...
      !local && $.ajax({
        ...
        error: function( data ){
          console.log( "signal can script ajax error", data );
          debugger;
        }
      } );
    }, 30 * 1000 );
...
// Client side setup, dynamic, different for each page
  ...
  if( this.url.indexOf( "offline" ) !== -1 ){
    console.log( '"offline" detected in page url' );
    // Hack to patch changes into constant "offline" for detection by client
    ctx.changes = "offline";
  }
  
  // Assume client will clear as instructed
  ...
  function kudo_init_ctx( ctx ){
    ...
    console.log( "Time offset (net delay, unless cached)", ctx.time_offset );
    console.log( "url", url );
    console.log( "timeless url", timeless_url );
    if( ctx.time_offset > 1 * 60 * 1000 ){
      console.log( "anormal delay, probably cached somewhere, refresh" );
      if( ctx.time_offset > 24 * 3600 * 1000 ){
        ...
        if( !ok_old && window.confirm(
          ...
          console.log( "reload at timeless location" );
          window.kudo_new_location = timeless_url;
          return;
        }
    ...
    if( window.kudo_time_last_page && window.kudo_time_last_page > ctx.time ){
      ...
      if( age > 10 * 1000 ){
        console.log(
          "Older page, probably cached somewhere, refresh. age ", age,
      ...
      console.log( "Ignored potential older page case, age:", age );
    }
    var age_old = now - ctx.time;
    if( age_old > 1 * 60 * 1000 ){
      console.log( "1 minute old page, probably cached somewhere, refresh" );
      window.kudo_new_location = timeless_url;
      return;
    }
    try{
    ...
    try{
      ...
      console.log( "Kudocracy: can't use history.replaceState()", err, err.stack );
    ...
    if( ctx.changes === "offline" ){
      offline = true;
      console.log( '"offline" change detected in context' );
...
// locally this is called the "client mode" or the "app mode".
  ...
  if( !done ){
    ...
    console.log( "BUG? useless double magic include" );
    return;
  }
  var de  = true;
  var nde = false;
  ...
  window.kudo_magic_loader = function kudo_magic_loader( url, back ){
    ...
    var idx = current_path.indexOf( "/offline/" );
    if( idx !== -1 ){
      current_path = current_path.replace( "/offline/", "" );
      console.log( "Removed /offline/ from current path", current_path );
      debugger;
    }
    ...
    if( !kudo_ctx.session.is_offline && query_str.indexOf( "page=login" ) !== -1 ){
      ...
      if( engine ){
        var cookie
        = "kudo_login_previous_page="
        + encodeURIComponent( engine.Session.current.current_page.join( "," ) )
        + ";max-age=1"; // One second, transient cookie
        console.log( "set cookie", cookie );
        document.cookie = cookie;
        cookie
        = "kudo_login_filter="
        + encodeURIComponent( engine.Session.current.filter )
        + ";max-age=1";
        console.log( "set cookie", cookie );
        document.cookie = cookie;
      }
      ...
      window.ui1_server(
        ...
        },
        {
          writeHead: function(){},
            ...
            if( !r ){
              console.log( "BUG? invalid empty response", url, query );
              debugger;
              r = "";
            }
            var duration = (new Date()).getTime() - time_local_request;
         console.log( "local request processed in", duration, "ms" );
         html = r;
    ...
    $.ajax( url, {
       ...
       complete: function( response, status ){
         ...
         $('body').empty().html( body ); // not [0].innerHTML = body;
         if( !window.kudo_ctx ){
           console.log( "BUG? Magic ajax result could not init kudo context" );
         }
  ...
  window.onpopstate = function( event ){
    ...
    if( event.state
    && kudo_magic_loader
    && kudo_magic_loader( event.state, true )
    )return;
    console.log( "BUG? cannot go back" );
  };
  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    if( new_location ){
      console.log( "Set new location, as instructed.", new_location );
      window.location.assign( new_location );
      return;
    }
    var ctx = window.kudo_ctx;
    if( !ctx ){
      console.log( "BUG? Missing Kudo context" );
      debugger;
    ...
    if( ctx.changes ){
      if( ctx.changes === "offline" ){
        console.log( "offline mode with no changes" );
      }else{
        console.log( "Ephemeral changes: " + ctx.changes.length );
      }
      var retry_count = 1000;
      var process_changes = function(){
        var ctx    = window.kudo_ctx;
        var ui_engine = window.ui1_server;
        if( !ui_engine ){
          ...
          console.log( "BUG? Ephemeral changes but no ui1_server" );
          return;
        }
        if( retry_count !== 999 ){
          console.log(
        ...
        ui_engine.set_config( ctx.config );
        ...
        if( ctx.changes === null ){
          console.log( "BUG? no changes, revert to safe server mode" );
        ...
        if( changes === "offline" ){
          if( !window.kudo_offline_loader ){
            console.log( "BUG? 'offline' change yet not kudo_offline_loader" );
            window.kudo_offline_loader = true;
            debugger;
          }
          changes = [];
        }
        ui_engine.load( changes, function(){
          console.log(
            "Restore: " + changes.length
            + " changes in " + ( (new Date()).getTime() - restore_start_time )
            + " ms"
          );
          // It is safe now to login the visitor
          if( ctx.visitor ){
            console.log( "Client side login for " + ctx.visitor );
            ui_engine.login( ctx.visitor, ctx.authentic );
          }
        });
        // Also sync local client session with server's one
        ui_engine.login();
        var session = ui_engine.Session.current;
        session.is_app = true;
        ...
        console.log( "Local UI server & local session are ready" );
      };
      setTimeout( process_changes, 0 );
    }
...
function page_style( title ){
  ...
  if( !session.page_init_done ){
    ...
    if( true ){ // l8.server ){
      ...
      l8.client && console.log( "Magic script is in response" );
      if( !session.is_slim
  ...
  if( session.is_app && !session.app_init_done ){
    ...
    if( l8.server ){
      ...
      buf.push(
        '\n<script>$("#progress_bar").show();console.log( "Loading engine...");</script>',
...
// The private page of a persona
  ...
  this.push( " ", link_to_page( "signout", "clear", i18n( "& clear" ) ), "." );
  ...
  Ephemeral.each( votes, function( entity ){
    ...
    var result_orientation = entity.proposition.result.orientation();
    that.push(
      div[ index % 2 ],
      //'<br>',
      '<h3>',
      ...
      '</h3>',
      " <em>", emojied( result_orientation ), '</em>',
      ' ',
...
// propositions, filtered.
  ...
  if( page_name === "offline" ){
    trace( "serving the offline version of proposition page" );
    if( l8.client ){
...
function page_login( page_name ){
  ...
  var previous_page = this.session.get_cookie( "login_previous_page" );
  if( previous_page ){
    previous_page
    = decodeURIComponent( previous_page ).replace( /[^A-Za-z0-9_ #+-,&]/g, "" );
    if( previous_page ){
      this.session.previous_page = previous_page.split( "," );
  ...
  // Query for name
  var auth = "";
  if( l8.server && session.can_script && config.firebase ){
    // Client side login page is served in offline mode only, hence...
    auth
    = '\n<br><br><label>' + i18n( "Twitter authentication" ) + '</label> '
    + '<input type="checkbox" id="checkbox" checked />'
    + '<input type="hidden" name="i3" id="twitter" value="auth" >';
  }
  this.push(
    '\n<form name="login" url="/">',
  ...
  if( auth ){
    this.push(
      '\n<script>$(function(){',
      'console.log( "Installing checkbox handler for twitter" );',
      '$("#checkbox").click( function(){',
      'console.log( "Change checkbox", $("#checkbox").is( ":checked" ) );',
      '$("#twitter").val( $("#checkbox").is( ":checked" ) ? "auth" : "noauth" );',
      '})});</script>'
    );
  }
  this.push( "\n<br>", page_footer() );
...
function page_signout( page_name, with_clear_storage ){
  ...
  if( l8.client ){
    // The client side must ask the server side to sign out too
...
function page_twitter( page_name ){
  ...
  if( !visitor )return this.redirect( "login" );
  
  // could be "facebook", "twitter", "google", "github", etc...
  this.set( page_style( "twitter" ), page_header() );

  function twitter_redirect( firebase, redirected, name ){
    ...
    if( !Firebase ){
      ...
      if( retry_count > 10 * ( 1000 / retry_delay ) ){
        console.log( "Oops ! cannot load Firebase?" );
      ...
      window.setTimeout( function(){
        twitter_redirect( firebase, redirected, name );
      }, retry_delay );
      return;
    }
    ...
    if( needs_redirection ){
      console.log( "Firebase. Signout", name );
      ref.unauth();
    }
    ...
    function process_result( auth_data ){
      if( auth_data ) {
        ...
        console.log( "Firebase authentic", username );
        // It is the name used to login in Kudocracy
        var same_name = name.toLowerCase() === username.toLowerCase();
        if( !same_name ){
          console.log( "Firebase. Authentic user", username, "is not visitor", name );
          // ToDo: redirect to ?page=login
        }
        // Signal server the authenticated user
        $.ajax({
          url: "?i=authentic/" + username + "/" + name
        });
        console.log( "Firebase auth, show visitor page", name );
        window.kudo_new_location = "?page=visitor";
        window.location.replace( "?page=visitor" );
      }else{
        // user is logged out
        console.log( "FIREBASE noauth", name );
        if( redirected ){
          console.log( "Firebase, no auth after redirection" );
          if( !before_redirection_result ){
            console.log( "Firebase, no auther, show login" );
            window.kudo_new_location = "?page=login";
            window.location.replace( "?page=login" );
          }  
        }
    ...
    needs_redirection && setTimeout( function(){
      ...
      ref.authWithOAuthRedirect(
        ...
        function( err, auth_data ){
          ...
          if( err ){
            console.log( "Firebase result, auth error", err );
            process_result( null );
          }else{
            console.log( "Unexpected Firebase result, auth", auth_data );
  ...
  // If redirected...
  var redirected = session.previous_page[ 0 ] === "twitter";
  
  // authenticate with Twitter
  var name_msg = Session.current.visitor ? Session.current.visitor.label : "";
  this.push(
    "<br><br><br><h1>", i18n( "login"), " ", "twitter", " ", name_msg, " ",
    redirected ? "(2/2)" : "(1/2)",
    '</h1><br><br>',
    "<script>",
...
function page_index( pagename ){
  ...
  function reload_if_cached( time_cached ){
    ...
    console.log( "age:", age / 1000, "sec.", age / ( 3600 * 1000), "hours." );
    if( age > 2 * 60 * 1000 ){
      console.log( "Cached, reload" );
  ...
  this.set(
    ...
    + "http://" + this.session.host + '&hashtags=kudocracy,democracy,opensource,vote" class="twitter-hashtag-button" data-related="Kudocracy,democracy,opensource">Tweet #kudocracy</a>'
    + ' <a href="https://twitter.com/Kudocracy" class="twitter-follow-button" data-show-count="true">Follow @Kudocracy</a>'
    + '<br><br><a class="twitter-timeline" data-dnt="true" href="https://twitter.com/Kudocracy" data-widget-id="299354291817287681">Tweets by @Kudocracy</a>',
    ...
    "\n<script>",
      ...
      "\n$(function(){",
        ...
        this.session.should_clear_local_storage,
      "};",
      "\nkudo_signal_capabilities();",
    "\n});</script>",
    // Twitter buttons
...
http_repl_commands.login = function( name, authentication, alias, more ){
  ...
  console.log( "Login", name, authentication, alias );
  ...
  Session.current.authentic = false;
  if( Session.current.visitor ){
    trace( "BUG? login while already logged in" );
    debugger;
  }
  ...
  var previous_page = Session.current.previous_page;
  Session.current.set_current_page( previous_page );
  
  // If twitter authentication, open a window for that
  if( authentication === "auth" ){
    Session.current.redirect( "twitter" );
    return;
  }
...
http_repl_commands.authentic = function( auth_name, name ){
  trace( "Authentic() called, auth_name", auth_name, "name", name );
  auth_name = ( "@" + auth_name ).replace( /@@/g, "@" );
  name      = ( "@" + name ).replace( /@@/g, "@" );
  if( auth_name === "Kudocracy" ){
    if( Session.current.visitor ){
      Session.current.authentic = true;
    }
    return;
  }
  var visitor = Persona.find( auth_name );
  var session = Session.current;
  if( !visitor ){
    printnl( "Invalid persona", auth_name, "in authentic()" );
    session.authentic = false;
    return;
  }
  if( visitor !== session.visitor ){
    if( session.visitor ){
      // ToDo: http_repl_commands.signout();
      trace(
        "Twitter authentication with a different name",
        "authentic:", auth_name,
        "expected:", name,
        "session:", session.visitor.label
      );
    }else{
      trace(
        "Twitter authentication on anonymous session",
        "authentic:", auth_name,
        "expected:", name
      );
    }
    ...
    trace(
      "Expected twitter authentication",
      "authentic:", auth_name,
      "expected:", name,
      "session:", session.visitor.label
    )
...
// This is not a normal command, it is called by the http server directly.
  ...
  if( req.method === "POST" ){
    ...
    if( !session.can_script || session.can_script === "init" ){
      console.log( "BUG? api call from noscript client" );
      session.can_script = true;
    }
...
function local_login( label, is_authentic ){
  ...
  if( label ){ session.set_visitor( label ); }
  session.authentic = !!is_authentic;
};
...
// Note: port/host are "local"/document.domain on the client side.
  ...
  if( l8.client ){
    ...
    window.kudo_is_offline = function(){
      try{ return !navigator.onLine; }catch(_){}
      return false;
    }
  }
