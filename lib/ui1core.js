...
function handle_basic_style_request_input( req, res ){
  ...
  if( !session ){
    ...
    session.is_app = was_magic && magic; 
    if( !magic ){
...
// Client side setup, dynamic, different for each page
  ...
  return [
    ...
    +   '})'
    + ' || (de = false)',
    'de&&bugC( "Starting " + sw_ctx.url + " [" + sw_ctx.title + "]" );',
...
// once the page is "ready"
    ...
    if( window.isMagicLoaded && window.ui1_server ){
      ...
      window.ui1_server(
        ...
        },
        {
         ...
         if( !back ){
           de&&bugC( "pushState", window.sw_ctx.url, "[" + window.sw_ctx.title + "]" );
           window.history.pushState(
             window.sw_ctx.url,   // data. onpopstate uses it
             window.sw_ctx.title,
             window.sw_ctx.url
           );
         }
  ...
  $(document).ready( window.sw_when_ready = function sw_when_ready(){
    ...
    document.title = window.sw_ctx.title;
    ...
    if( window.sw_ctx.changes ){
      setTimeout( function(){
        // Also sync local client session with server's one
        window.ui1_server.login( window.sw_ctx.visitor );
        window.ui1_server.Session.current.set_filter( window.sw_ctx.filter );
        Ephemeral.Change.process( window.sw_ctx.changes );
        window.sw_ctx.changes = null;
      });
    }
...
function page_style( title ){
  ...
  if( Session.current.is_app && !Session.current.app_init_done ){
    if( !l8.client ){
      buf.push( '\n<script type="text/javascript" src="http:?browserify"></script>' );
    }
...
exports.request.login = function( label ){
  var session = Session.login( "127.0.0.1" );
  session.set_visitor( label );
};


exports.request.Session = Session;
