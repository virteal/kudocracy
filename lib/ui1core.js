...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    window.kudo_set_touch_device = function( flag ){
      if( flag === undefined ){
        flag = !!Modernizr.touch;
      }
      window.kudo_is_touch_device = flag;
      ctx.touch_device = flag;
      if( ctx.touch_device ){
        $("html").addClass( "kudo_touch" ).removeClass( "kudo_no_touch" );
      }else{
        $("html").addClass( "kudo_no_touch" ).removeClass( "kudo_touch" );
      }
    }
    window.kudo_set_touch_device();
...
function page_header( left, center, right, current_page ){
  ...
  if( recent_events_div( current_page, true ) ){
    // 75 * 16 = 1200px
    container_style = "margin:auto; font-family:inherit;"; // max-width:75em; 
  }
...
"This page lists informations about a person, her votes, her delegations (received and given), etc."
  ...
  // What are the votes to display?
    ...
    if( direct_votes_only ){
      this.push( 
        "<h3>", icon( "direct" ) + " " + i18n( "direct(s)" ), "</h3>" 
      );
    }else{
      this.push(
        link_to_page(
    ...
    if( indirect_votes_only ){
      this.push( 
        "<h3>", icon( "indirect" ), " ", i18n( "indirect(s)" ), "</h3>"
      );
    }else{
      this.push(
        link_to_page(
...
// propositions, filtered.
  ...
  Ephemeral.each( list, function( proposition, index ){
    ...
    that.push( '</dfn></small></span><br><br></div><div>' );
...
//      '<li>You can share your votes or hide them.</li>',
  ...
  this.push(
    ...
    "<br>", icon( "propositions" ), " ", Topic.count,
    "<br>", icon( "votes"        ), " ", Vote.count,
    "<br>", icon( "votes" ),
