...
// Constructor, called by .login() only
  ...
  // Get initial set of changes from Ephemeral machine
  return this;
}
...
Session.prototype.pull_changes = function(){
  ...
  // Get initial set of Ephemeral changes if never done before
  if( !this.changes ){
...
Session.start_change_dispatcher = function(){
  Ephemeral.Change.fluid.tap( function( a_change ){
    ...
    for( var ii = 0 ; ii < len ; ii++ ){
      session = all[ ii ];
      session.dispatch_change( a_change );
    }
  } );
};


Session.prototype.dispatch_change = function( a_change ){
  if( this.changes )return this;
  // ToDo: if session is somehow inactive, it's better not to buffer anymore
  if( Kudo.now() - this.time_touched > 1 * 60 * 1000 ){
    this.changes = null;
    return this;
  }
  this.changes.push( {
    timestamp: a_change.ts,
    t:         a_change.t,
    o:         a_change.o,
    p:         a_change.p
  } );
...
var pn = function( msg ){
  ("" + msg).split( "\n" ).forEach( function( m ){ if( m ){ basic_screen.push( m ); } } );
};

var printnl   = function( msg ){ pn( msg ); pn( "\n" ); };
...
function link_to_page( page, value, title ){
  ...
  &&  title[0] === "@"
    ...
    if( idx_sp !== -1 ){
      user = user.substring( 0, idx_sp - 1 );
    }
    if( !user ){
...
// once the page is "ready"
  ...
  window.sw_magic_loader = function sw_magic_loader( url, back ){
    ...
    if( window.kudo_ctx.is_slim )return false;
    
    // Home page has a different style, cannot magic load it
    if( query_str.indexOf( "page=index" ) !== -1
    ||  !query_str
    ){
      return false;
    }
  ...
  $(document).ready( window.sw_when_ready = function sw_when_ready(){
    ...
    if( window.kudo_ctx.changes ){
      setTimeout( function(){
        ...
        Ephemeral.Change.process( ctx.changes );
        ctx.changes = null;
...
PageBuilder.prototype.push_title_and_search_form = function( title ){
  ...
  buf.push( '<div id="search"><br><h3>' + i18n( title ) + '</h3>' );
...
PageBuilder.prototype.push_delegations = function( persona ){ 
  ...
  var can_delegate = this.session.visitor && this.session.visitor !== persona;
...
"This page lists informations about a person, her votes, her delegations (received and given), etc."
  ...
  if( delegateable_filter ){
    that.push(
      ...
      link_to_page(
        "propositions",
        delegateable_filter,
        delegateable_filter.replace( / /g, "&nbsp;+&nbsp;" )
      ),
      "<br>", i18n( "delegations" ), " ",
      link_to_page(
        "leaders", "all " + delegateable_filter,
        delegateable_filter.replace( / /g, "&nbsp;+&nbsp;" )
      ),
      '<br>'
    );
  ...
  if( elen || not_done ){
    that.push(
      '\n\n<br><div class="odd"><h2>',
      i18n( "Delegate" ), " via ", link_to_persona_page( persona ),
      '</h2>'
    );
    if( !can_delegate ){
      that.push( '<br>' );
    ...
    for( var eii = 0 ; eii < elen && eii < 10 ; eii++ ){
      ...
      can_delegate
      = this.session.visitor && this.session.visitor !== expertize.agent;
      if( can_delegate ){
...
// The private page of a persona's delegations
  ...
  if( proposition_names.length && this.session.has_delegateable_filter() ){
    ...
    proposition_names.sort( function( a, b ){
      ...
      that.push( link_to_page( "proposition", label ), "<br>" );
    });
    this.push( "<br>" );
  }
  ...
  this.push(
    ...
    " - " + link_to_page( "leaders", "all", "all(s)" ),
    "<br>"
  );
...
// propositions, filtered.
  ...
  if( sparklines ){
    ...
    this._body[ date_range_insert_index ]
    = msg1
    + i18n( "between" )
    + " "
...
// This is the votes page of the application, filtered.
  ...
  if( count_propositions ){
    that.push(
      ...
      " - ", link_to_page( "leaders", "", "delegations" )
    );
  }
  ...
  valid_votes.forEach( function( vote_value ){
    ...
    if( comment ){
      that.push( '<br><h3>', format_comment( comment ), '</h3>' );
    }
    // buf.push( "</li>" );
  });
...
function page_leaders( page_name, optional_proposition_name ){
  ...
  Ephemeral.each( propositions, function( proposition ){
    ...
    that.push(
      div[ index % 2 ],
      '<h3>',
      link_to_page( "leaders", proposition.label ),
      '</h3>',
      ' ', 
      nbsp, count_votes, " ", 
      ...
      filter_label_div( 
        ...
        "leaders " + ( about_proposition ? about_proposition.label : "all" )
      ),
      '</div>'
    );
    shown_propositions++;
  ...
  agent_ids.forEach( function( agent_id ){
    ...
    that.push(
      ...
      '</h2>'
      //link_to_page( "leaders", "#" + agent.label.substring( 1 ) )
    );
    if( ratio !== 100 ){
      that.push(
      " <dfn>(",
...
function page_help(){
  ...
  if( this.session.lang !== "fr" ){
    msg.push(
      ...
      'Versi&oacute;n ', link_to_command( "lang es", "espag&ntilde;ol" ), ". ",
      'Protuguese ', link_to_command( "lang pt", "version" ), ". ",
      'German ',     link_to_command( "lang de", "version" ), ". ",
      'Italian ',    link_to_command( "lang it", "version" ), ". ",
...
// Focus on one proposition
  ...
  buf.push( '<br><div id="balance_chart_div" style="height:100px"></div>' );
  ...
  votes.forEach( function( vote_value, index ){
    ...
    if( !idem && ( !gap || index >= votes.length - 200 ) ){
      ...
      if( comment ){
        if( !seen_comments[ comment ] ){
          seen_comments[ comment ] = true;
          buf.push( '<br><h3>', format_comment( comment ), '</h3>' );
        }
      }
      buf.push( "</div>" );
    }
...
ui1server.set_config = function( new_config ){
  config = new_config;
}
