...
// locally this is called the "client mode" or the "app mode".
  ...
  window.kudo_when_ready = function kudo_when_ready( jquery_ready ){
    ...
    if( window.twttr && window.twttr.widgets ){
      ...
      var twitter_render = document.getElementsByClassName( "twitter_render" );
      if( twitter_render ){
        window.twttr.widgets.load( twitter_render );
      }
...
// This builds the content of the navigation bar at the top of the screen.
  ...
  if( visitor ){
    ...
    + titled( 
        link_to_page( "visitor", "", ui.avatar( visitor.label, 26 ) ), 
  ...
  var owner_avatar = ui.avatar( domain_owner, 26 );
    ...
    if( count_propositions ){
      ...
      if( count_propositions ){
        this.push(
          ...
          //'<br>',
          ui.proposition_summary( persona_topic, "", { avoid_author: persona } ),
          ""//"<br><br>"
        );
      }
      this.push( '</div>' );
...
// propositions, filtered.
  ...
  Ephemeral.each( list, function( proposition, index ){
    ...
    that.push(
      '<div>',
      ui.proposition_summary( proposition, "nocomment", {
...
function page_delegates( page_name, optional_proposition_name ){
  ...
  this.push( ui.sort_menu( "", !count_votes /* show */ ) );
...
// It is the .login() command to redirects here.
  ...
  if( previous_page ){
    previous_page = "?page=" + ui.encode_ref( previous_page.join( " " ) );
  }
...
ui.proposition_summary = function( proposition, div, options ){
...
// Note: port/host are "local"/document.domain on the client side.
  ...
  function s( m ){ m.start( kudo_scope ); }
  s( require( "./pageindex.js" ) );
  s( require( "./pageproposition.js" ) );
  s( require( "./pagetwittrust.js" ) );
