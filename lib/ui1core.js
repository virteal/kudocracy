...
// Client side setup, dynamic, different for each page
  ...
  function kudo_init_ctx( ctx, time_built, debug_mode ){
    window.kudo_ctx = ctx;
    window.kudo_ctx.time_built = time_built;
    window.time_offset = 0;
    window.touch_device = !!( "createTouch" in document );
    ...
    try{
      history.replaceState( ctx.url, ctx.title, ctx.url );
    }catch( err ){
