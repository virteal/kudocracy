...
// The private page of a persona
  ...
  if( display ){
    ...
    this.push( '<br><div><h2>', i18n( "Your votes" ), '</h2>' );
...
function page_ballot2( /* pagename, ...query */ ){
  ...
  Ephemeral.each( tags, function( tag ){
    ...
    var ratio_against = Math.round( 1000 * ( count_for / total) ) / 10;
    var ratio_blanks  = Math.round( 1000 * ( count_for / total) ) / 10;
    var sp = "&nbsp;";
    var bp = "&nbsp<dfn>";
    var ep = "%</dfn>";
    that.push(
...
// This is the votes page of the application, filtered.
  ...
  if( !comments_only ){
    this.push(
      link_to_page( "votes", "comments", i18n( "comments only") ),
      '<br><br>'
    );
  }else{
    this.push(
      link_to_page( "votes", "", i18n( "direct votes only") ),
      '<br><br>'
    );
  }
