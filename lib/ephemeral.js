...
// I use the "fast" version, without buffering, or else some updates fail
...
// Get/set global debug mode. Returns new current mode.
  if( arguments.length ){
    de = !!x;
  }
  return de;
...
// De&&mand() is like assert()
  if( b )return;
  var tmp = msg ? ": " + msg : "";
  bug( "ephemeral.js, assert error " + tmp );
  if( de && debugging )debugger;
  if( ! (de && debugging ) )throw new Error( "ephemeral.js assert" );
}
exports.assert = mand;
...
/*
 *  array_diff() makes it easy to detect changes in array. Such arrays are
 *  actually "sets" 
 ...
 *  When log of actions is replayed, time moves to follow the actions time.
 ...
 *  If the same entity is pushed multiple times into the same fluid, during
 *  the same step, only the first push is actually performed.
 */
...
function steps( list ){
  ...
  water.steps( queue ).boxon( function( err ){
    ...
    if( StepQueue.length ){
      // ToDo: trampoline to avoid stack overflow?
      steps();
    // If all done, signal result to all pending boxons
    }else{
      var signals = DoneQueue;
      DoneQueue = [];
      signals.forEach( function( boxon ){
        boxon( err );
      });
    }
  } );
 ...
 *  can vote (ToDo: not implemeted yet).
...
function Machine( options ){
  ...
  if( !MainMachine ){
    ...
    // Init shared attributes for all types. Including "all", "count" & "fluid"
    AllTypes.forEach( function( ctor ){
      // console.log( "New machine. Type clone", ctor.name );
      var ctor_clone = map();
      var keys = ctor.keys;
      var len = keys.length;
      var values = ctor.initial_values;
      var cloned_values = map();
      var key;
      var value;
      // Either copy or call constructor for each shared attribute
      for( var ii = 0 ; ii < len ; ii++ ){
        key = keys[ ii ];
        value = values[ key ];
        if( typeof value === "function" ){
          // trace( "Constructor for key", key );
          value = value();
        }else{
          // trace( "Primitive value for key", key, value );
        }
        if( value !== undefined ){
          cloned_values[ key ] = value;
        }
      }
      ctor_clone.ctor = ctor;
      ctor_clone.values = cloned_values;
  ...
  if( this.is_main ){
    Machine.by_id[ "main" ] = this;
  }
  this.label = label;
  this.id    = id;
  Machine.all.push( this );
  trace( "New Ephemeral.Machine", label + ( this.is_main ? "(main)" : "" ) );
}
...
Machine.prototype.add_alias = function( alias ){
  Machine.by_id[ alias ] = this;
  return this;
};
...
// Also detect forward reference ids and adjust NextId accordingly.
  ...
  if( id.entity_type )return id;
  if( id.prototype && id.prototype.entity_type )return id.prototype;
  if( typeof id === "string" ){
...
Entity.prototype.check_identity = function(){
  ...
  var typed_entity = null;
  if( !this.constructor.all ){
    trace( "BUG, missing .all[] for " + this );
    valid = false;
  }else{
    typed_entity = this.constructor.all[ this.id ];
  }
  ...
  if( typeof v === "function" || typeof v === "object" ){
    ...
    for( var attr in v ){
      if( attr !== "id" && v.hasOwnProperty( attr ) ){
        ...
        if( v.entity_type && attr === "label" )continue;
...
// is an optimization that speeds up method lookup a little.
  ...
  sub_proto.entity_type = name;
  sub_proto.constructor = ctor;
  
  // Access to super instance stuff, like instance methods
  sub_proto.super  = base_proto;  
  
  // Access to super static stuff, like class methods
  ctor.super = base;   
  
  // All constructors, from Entity, down to this new type
  ctor.ctors = [];     
  var a_ctor = ctor;
  while( a_ctor ){
 ...
 sub_proto.push = function( f ){
  ...
  // Patch it to make it run faster (unrolling)
  var buf = "if( true ){\n";
  var ii = 1;
  while( a_ctor = ctor.ctors[ ii++ ] ){
    buf += "a_ctor = list[ " + ( ii - 1 ) + " ];\n";
    buf += "r = a_ctor.call( obj, options );\n";
    buf += "if( r !== undefined ){\n";
    buf += "  if( !r || r.buried )return null;\n";
    buf += "  ctor.current = obj = r;";
    buf += "} else if( obj.buried )return null;\n";
  }
  buf += "}else";
  var definition = ( "" + ctor.inject ).replace( "if( false ){}else", buf );
  // console.log( "Definition", definition );
  var f;
  eval( "f = " + definition );
  ctor.inject = f;
  
  sub_proto.inject = ctor.inject;
  ...
  // Set up shared attributes. It is those just added to the ctor.
  // This is needed for the per machine types, when a new machine is created. 
  ctor.keys = [ "all", "fluid", "count" ];
  ctor.initial_values = map();
  ctor.initial_values.all = function(){ return map(); };
  ctor.initial_values.count = 0;
  ctor.initial_values.fluid = function(){ return fluid(); };
  
  ctor.share = function( key, value ){
  // Declares a new static attribute for the type, aka "class variable".
    ctor.keys.push( key );
    ctor.initial_values[ key ] = value;
    ctor[ key ] = typeof value === "function" ? value() : value;
  };
  ...
  ctor.valid = function( candidate ){
    ...
    if( candidate.entity_type !== this.name )return null;
  ...
  De&&mand( proto_entity.entity_type );
  De&&mand( proto_entity.id !== undefined );
...
function save_type_states(){
  var list = [];
  // Global list of all types
  var len = AllTypes.length;
  for( var ii = 0 ; ii < len ; ii++ ){
    var ctor = AllTypes[ ii ];
    // For each type, save each static attribute
    var keys = ctor.keys;
    var len2 = keys.length;
    // At least "all", "count" & "fluid"
    de&&mand( len2 >= 3 );
    var values = map();
...
function restore_type_states( list ){
  ...
  for( var ii = 0 ; ii < len ; ii++ ){
    var state = list[ ii ];
    var ctor = state.ctor;
    // console.log( "Restoring type", ctor.name );
    var values = state.values;
    var keys = ctor.keys;
    var len2 = keys.length;
    for( var jj = 0 ; jj < len2 ; jj++ ){
      var key = keys[ jj ];
      // console.log( "Static member", key );
      ctor[ key ] = values[ key ];
    }
  }
}
...
// Switches to another machine. Global.
  if( !next_machine ){
...
// Resolve id references into pointers
  ...
  var pure_map = !o.hasOwnProperty;
  for( var attr in o ){
    if( pure_map || o.hasOwnProperty( attr ) ){
      if( attr === "machine" ){
...
// Change pointers into id references for json storage
  ...
  json = {};
  for( var attr in o ){
    if( !o.hasOwnProperty || o.hasOwnProperty( attr ) ){
      if( attr === "machine" )continue;
      if( o[ attr ] ){
        if( o[ attr ].entity_type ){
...
function json_decode_resolve( id ){
  ...
  if( entity ){
    ...
    if( entity !== valid ){
      ...
      if( valid && !entity.buried )debugger;
    }
    return valid;
  }else{
...
var value_seen_entities;


function _value( x, force ){
// Entity's value is a snapshot of the entity's current state
  ...
  if( typeof x === "object" ){
    if( Array.isArray( x ) ){
      ...
      if( x.watered )return { watered: "water", error: value( x.error ) };
      if( x.entity_type ){
        if( x.buried )return undefined;
        if( value_seen_entities[ x.id ] )return value_seen_entities[ x.id ];
        if( !force )return x.id;
        value_seen_entities[ x.id ] = o;
      }
      // Scan all properties, including inherited ones
      for( var attr in x ){
        // Filter out some attributes, including _ prefixed ones
        if( value_dont_copy[ attr ] )continue;
    ...
    if( x._water )return _value( x._water.current );
    return undefined;
  }else{
    return x;
  }
...
function value( x, force ){
  // Init loop detection
  value_seen_entities = map();
  return _value( x, force );
}
exports.value = value;
...
// Process a json_encode() encoded change, like those in log files
  ...
  try{
    ...
    trace( "BUG? Change json_decode() error", err, err.stack );
    debugger;
    return;
  ...
  var r;
  try{
    Machine.current.changes.push( change );
...
// This is the mapping function applied on the fluid of Changes
  ...
  try{
    // If no id was provided for the new entity, reuse the change's id itself
...
// Changes to entities involves watering the original with an update.
  ...
  : function water_update( init_val ){
    // x = water( init_val );
    if( init_val !== undefined )return init_val;
...
// array with buried entities removed. Return true iff water was updated.
  ...
  for( ii = 0 ; ii < len ; ii++ ){
    ...
    if( entity.buried && is_water ){
      ...
      if( !entity.expired() ){
        r = functor.call( this, entity, ii, entities );
...
Ephemeral.prototype.expire = function(){
  // ToDo: avoid expiration for a while, buggy, suvranu
...
Ephemeral.prototype.bury = function(){
  ...
  // ToDo: per type flag to command clear.
  // ToDo: garbage collect at the log level. To do that, log the id of buried
  // entities and when restoring from log file, ignore entities that are in
  // the "buried entities" log file. Nota: when a buried entity is (re)created
  // after it was buried, the timestamp in the buried entities log is older
  // than then create time of the recreated entity.
  // This whole scheme is an optimization to reduce the amount of work done
  // when restoring the database. Another option, complementary, is to
  // compress the log files themselves in order to remove anything related to
  // obsolete entities. This could be done during the restore phase.
  ...
  if( this.buried ){
    ...
    for( var attr in this ){
      if( !this.hasOwnProperty( attr ) )continue;
      if( attr !== "entity_type" && attr !== "buried" ){
        var v = this[ attr ];
        if( v ){
          if( v._water ){ water.dispose( v ); }
        }
        if( should_clear ){
          // This is dangerous because some other entity may reference the
          // buried entity and wrongly access it's data without checking
          // if the entity is still alive or not. Much like dereferencing a
          // null pointer, somehow.
          this[ attr ] = _;
        }
      }
    }
 ...
 */
 ...
 function Expiration( options ){
   de&&bug( "Expired & buried: " + options.entity );
...
function persist( fn, a_fluid, filter ){
  ...
  .on( "error", function( err ){
    if( ("" + err ).indexOf( "Unexpected end" ) === -1 ){
      trace( "Restore, stream split error", err );
      trace( "Restore err around line number", all_changes.length + 1 );
...
// ToDo: should be per machine, not global.
var restoring = 0;
var restore_queue = [];
