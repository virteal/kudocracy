 ...
 sub_proto.push = function( f ){
  ...
  ctor.inject = sub_proto.inject = function( options ){
    ...
    while( a_ctor = list[ ii++ ] ){
      ctor.current = r = a_ctor.call( obj, options );
      if( r !== undefined ){
        obj = r;
...
// Entity's value is a snapshot of the entity's current state
  ...
  if( x ){
    ...
    if( x.is_entity && x.buried ){
      ...
      if( x._water ){
        return _value( x._water.current );
      }
      ...
      if( x.watered ){
        ...
        if( x.is_entity ){
          if( value_seen_entities[ x.id ] ){
            console.error( "BUG, value() for self referencing entity" );
            debugger;
            return _;
          }
          value_seen_entities[ x.id ] = true;
        }
        o = {};
