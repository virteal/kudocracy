...
Entity.prototype.measure_name = function(){ return "" + this.id; };
Entity.prototype.measure_name_is_alpha = true;
...
// When .inject() is called, Entity.created points to the being created obj
  var w = water();
...
Effect.prototype.measure_updates = function(){ return this.updates.length; };
...
// Make sure this entity won't get buried before some other one
  ...
  // OK if it will after the other one
...
// Returns null for 'created expired' entity, during 'restore' typically.
  ...
  if( entity ){
    ...
    if( entity.expired() ){
      ...
      // If entity already buried, nothing to be done
  ...
  // Check expiration, ie born expired entities, during 'restore' typically
  if( l8.update_now() > this.timestap + this.duration ){
    this.buried = true;
    return null;
  }
...
Ephemeral.each = function( source, functor ){
...
// This will call .bury(), now or later, depending on limit (a timestamp)
  // The scheduler works with 'real' now, not the 'frozen' one. This is safe
  // because dead entity are detected during "restore" phase and .schedule()
...
Ephemeral.prototype.measure_age = function(){ return this.age(); };


Ephemeral.prototype.measure_duration = function(){ return this.duration; };
...
Ephemeral.prototype.measure_age_modified = function(){ return this.age_touched(); };
...
var compare_measures = function( a, b, criterias ){
  ...
  var va = cmp.call( a );
  var vb = cmp.call( b );
  var is_alpha = a[ meter + "_is_alpha" ];
  var r = 0;
  if( !is_alpha ){
    r = va - vb;
  }else{
